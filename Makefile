...
# build-release-all; also runnable standalone to re-sign. No-op with a warning
# when MINISIGN_SECKEY is unset.
sign-release:
...
upload-release:
	...
	if [ -f "$$mounter" ]; then \
		...
		sig_arg=""; \
		if [ -f "$$mounter.minisig" ]; then sig_arg="-F minisig=@$$mounter.minisig"; fi; \
		curl -X POST "$$OAK_URL/api/releases" \
			-H "Authorization: Bearer $(OAK_ADMIN_API_KEY)" \
			-F "version=$(VERSION)" \
