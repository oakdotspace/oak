...
```toml
[dependencies]
oakvcs-core = { version = "0.103.0", default-features = false }
```
...
Oak is in **public beta** (v0.103.0). The quickest way in is the prebuilt
`oak` binary:
...
being logged in does not trigger object-store probes. If only the proof's
history/tree/path budget is exhausted, Oak prints an exact retry using
`--allow-unverified-integrity` while the pull still verifies transferred
hashes. `--allow-legacy-scope` is the separate, explicit compatibility waiver
