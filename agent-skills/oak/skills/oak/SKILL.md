...
- **Commits have no messages.** `oak commit` takes no `-m`. The *branch
  ...
  text on stdin.
- **Your work is isolated until you push/merge.** Commit as much as you want;
  nothing is shared until `oak push` (to the server) or `oak merge` (into
  `main`).
...
oak agent state --json --compact # lowest-token preflight with next commands
...
oak commit                 # snapshot the working tree — NO message, no -m
...
oak finish --desc-file /tmp/desc # desc + commit-if-needed + push; inside mounts also ends
...
`oak agent state --json --compact` for preflight state and next commands,
...
agent state --json` for full preflight state, `oak agent state --json
--refresh` when you explicitly want remote freshness fields, `oak log --json`
...
checkouts and mounts. To finalize work, prefer `oak finish --desc-file <file>`
from inside a checkout or mount; inside mounts it delegates to mount
finalization and ends the mount. From outside a mount, use `oak mount finish
[path] --desc-file <file>`.
