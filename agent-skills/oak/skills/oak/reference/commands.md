...
| `oak agent state --json --compact` | Lowest-token preflight for unattended agents: context, branch, dirty/conflict/progress state, blocked/finish state, one `recommended_action`, and compatibility `recommended_next_commands`. |
| `oak agent state --json [--refresh]` | Full preflight document for unattended agents: context, repo/branch, dirty changes, conflict/progress state, unpushed count, freshness fields, mount metadata when applicable, one structured `recommended_action`, and compatibility `recommended_next_commands`. Ordinary reads stay offline; `--refresh` explicitly contacts the remote to populate remote parent/current-branch heads where supported. |
...
| `oak commit [PATHS] [--no-verify]` | Create a local checkpoint only; it never auto-pushes. **No message** — descriptions carry intent. `--no-verify` skips hooks. |
| `oak commit --push [PATHS] [--no-verify]` | Create a checkpoint, then publish the current branch. |
| `oak commit --json --quiet [--push]` | Machine-readable checkpoint state; with `--push`, also reports publish state. |
...
| `oak finish --desc "<text>" [--json]` | Finish the current checkout or mount. It preflights unresolved state, empty descriptions, and a linked remote before mutating; then stepwise writes the branch description, checkpoints dirty work if needed, publishes committed work, and syncs branch metadata. In a mount it delegates to mount finalization and ends the mount after publish. It is not rollback-atomic. |
...
| Command | What it does |
| --- | --- |
| `oak push [-f] [--repo OWNER/NAME] [-r URL]` | Publish the current branch. `-f` overwrites diverged remote history. `--repo` links a fresh repo without the interactive org picker (useful for agents/CI). |
...
Plain `oak commit` creates a local checkpoint. `oak push --repo` persists the
local repo identity and creates the server repo on first push if needed, so
agents do not need the interactive organization picker.
...
| `oak mount finish [dest] --desc-file <file> [--json]` | Path-based mounted-agent finalization. Preflights a linked remote before mutating, reads the branch description from a UTF-8 file or `-` for stdin, checkpoints dirty overlay work if needed, publishes committed work, then ends the mount without `--force`. `--json` emits one result with repo, virtual branch, heads, checkpointed/published/ended flags, and branch URL. Defaults to the current directory when `dest` is omitted. Failure leaves the mount intact and names the next manual command to run. |
...
- `oak agent state --json --compact` for the lowest-token preflight document with one structured recommended action and compatibility next commands.
...
- `oak commit --json --quiet [--push]` for parseable checkpoint and optional publish state.
- `oak agent state --json` for one full offline preflight document with one structured recommended action and compatibility next commands.
