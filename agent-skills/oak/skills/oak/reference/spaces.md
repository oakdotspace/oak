...
that lives locally until you push. The mount runs as a detached background
daemon, so the command returns as soon as the mount is live.
...
virtual branch automatically. To wrap up, write the branch description to a
temp file and run `oak finish --desc-file <file>` from inside the mount. From
outside the mount, use `oak mount finish [path] --desc-file <file>`. Both set
the description, commit dirty overlay work if needed, push all committed work,
and tear the mount down without `--force`. You never need to know the virtual
branch name. If finish fails, it leaves the mount intact and names the next
...
- Commits are still messageless; `oak finish --desc-file <file>` inside a
  mount sets the virtual branch's merge message from a file, commits if
  needed, pushes, and ends the mount. Use `oak mount finish [path] --desc-file
  <file>` when you need to finalize by path from outside the mount. Use
  `oak desc --file <file>` inside the mount only when you need to update the
  description without finalizing yet.
