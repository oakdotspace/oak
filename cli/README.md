...
For headless agents or CI, `oak push --repo <org>/<name>` is the
non-interactive repo creation path. It links the local repo to that org/name
and creates the server repo on first push if it does not already exist.
