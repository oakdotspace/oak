...
mod review_budget;
pub(crate) use review_budget::ReviewPreparationBudget;
...
/// Review-specific transport bounds; legacy hydration callers keep their envelope.
#[cfg(test)]
pub(crate) async fn ensure_review_commits_local(
    repo: &SqliteRepository,
    remote_url: &str,
    owner: &str,
    repo_name: &str,
    ...
    ensure_commits_local_bounded(
        ...
        api_key,
        commit_hashes,
        (128 * 1024 * 1024, std::time::Duration::from_secs(30)),
    )
    .await
}

pub(crate) async fn ensure_review_commits_with_budget(
    repo: &SqliteRepository,
    remote_url: &str,
    owner: &str,
    repo_name: &str,
    ...
    if commit_hashes.len() > 256 {
        return Err(OakError::Server("review preparation incomplete: at most 256 exact commits per request; narrow the branch selection".into()));
    }
    tokio::time::timeout(
        budget.remaining(),
        ensure_commits_local_impl(
    ...
    .await
    .map_err(|_| OakError::Server("review preparation incomplete: time budget exceeded".into()))?
}

#[cfg(test)]
async fn ensure_commits_local_bounded(
    repo: &SqliteRepository,
    remote_url: &str,
    owner: &str,
    repo_name: &str,
    api_key: Option<&str>,
    commit_hashes: &[Hash],
    budget: (usize, std::time::Duration),
) -> Result<()> {
    let mut limits = ReviewPreparationBudget::with_limits(budget.0, budget.1);
    ensure_review_commits_with_budget(
        repo,
        remote_url,
        owner,
        repo_name,
...
async fn ensure_commits_local_impl(
    repo: &SqliteRepository,
    remote_url: &str,
    owner: &str,
    repo_name: &str,
    api_key: Option<&str>,
    commit_hashes: &[Hash],
    mut review_budget: Option<&mut ReviewPreparationBudget>,
) -> Result<()> {
    let mut missing: Vec<String> = Vec::new();
    for hash in commit_hashes {
        if let Some(budget) = review_budget.as_deref_mut() {
            if let Some(commit) = repo.get_commit(hash)? {
                match budget.inspect(&commit.manifest_hash, |hash| repo.get_tree(hash)) {
                    Ok(()) => continue,
                    Err(OakError::ManifestNotFound(_)) => {
    ...
    if !resp.status().is_success() {
        if let Some(budget) = review_budget.as_deref() {
            budget.check_deadline()?;
            // Error bodies are untrusted too. Review needs the failure status,
            // not an unbounded diagnostic body outside its response-byte budget.
            return Err(OakError::Server(format!(
                "review preparation incomplete: commit info request failed: HTTP {} (response body omitted)",
                resp.status()
            )));
        }
        return Err(OakError::Server(format!(
    ...
    let info: CommitInfoResponse = if let Some(budget) = review_budget.as_deref_mut() {
        ...
        while let Some(frame) = stream.next().await {
            ...
            if frame.len() > budget.bytes_left {
                return Err(OakError::Server(
                    "review preparation incomplete: commit response byte budget exceeded".into(),
                ));
            }
        ...
        budget.check_deadline()?;
        let parsed = serde_json::from_slice(&bytes).map_err(|e| OakError::Http(e.to_string()))?;
        budget.check_deadline()?;
        parsed
    } else {
        resp.json()
    ...
    if let Some(budget) = review_budget.as_deref_mut() {
        for commit in &commits {
            budget.inspect(&commit.manifest_hash, |hash| {
                Ok(tree_by_hash.get(hash).cloned())
            })?;
        }
        budget.check_deadline()?;
    }
    ...
    for commit in &commits {
        if review_budget.is_none()
            && repo.get_manifest(&commit.manifest_hash)?.is_none()
...
mod tests {
    ...
    fn shared_tree_graph(depth: usize) -> (Commit, Vec<oak_core::Tree>) {
        let mut trees = vec![oak_core::Tree::new(vec![oak_core::TreeEntry {
            name: "file".into(),
        ...
        .unwrap()];
        for _ in 0..depth {
            let child = trees.last().unwrap().hash.clone();
            trees.push(
                oak_core::Tree::new(
                    ["a", "b"]
                        .into_iter()
                        .map(|name| oak_core::TreeEntry {
                            name: name.into(),
    ...
    #[tokio::test]
    async fn cached_review_rejects_tiny_dag_with_exponential_expansion() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let (commit, trees) = shared_tree_graph(20);
        assert!(
            trees
                .iter()
                .map(|t| t.canonical_bytes().len())
                .sum::<usize>()
                < 10_000
        );
        for tree in &trees {
        ...
        let error = ensure_review_commits_local(
            &repo,
            "http://127.0.0.1:1",
            "oak",
            "oak",
        ...
        .await
        .expect_err("review must reject expanded work before flattening a tiny shared DAG");
        assert!(error.to_string().contains("work budget"), "{error}");
    }

    #[tokio::test]
    async fn fresh_review_rejects_expansion_before_publishing_any_tree() {
        review_shared_response(20, false).await;
    }

    #[tokio::test]
    async fn review_accepts_small_valid_shared_tree_graph() {
        review_shared_response(4, true).await;
    }

    async fn review_shared_response(depth: usize, accepted: bool) {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let server = MockServer::start().await;
        let (commit, trees) = shared_tree_graph(depth);
        let wire = CommitData {
        ...
        let body = serde_json::json!({"commits":[wire],"trees":trees.iter().map(oak_core::protocol::tree_to_wire).collect::<Vec<_>>()});
        assert!(serde_json::to_vec(&body).unwrap().len() < 20_000);
        Mock::given(method("POST"))
            ...
            .mount(&server)
            .await;
        let result = ensure_review_commits_local(
            &repo,
            &server.uri(),
            "oak",
            "oak",
        ...
        if accepted {
            ...
            assert!(result.unwrap_err().to_string().contains("work budget"));
        }
        assert_eq!(repo.get_commit(&commit.hash).unwrap().is_some(), accepted);
        for tree in &trees {
            assert_eq!(repo.get_tree(&tree.hash).unwrap().is_some(), accepted);
        }
    }

    #[tokio::test]
    async fn review_work_budget_is_shared_across_head_and_base_fetches() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let (commit, trees) = shared_tree_graph(18);
        for tree in &trees {
            repo.store_tree(tree).unwrap();
        }
        repo.store_commit(&commit).unwrap();
        let mut second_entries = trees.last().unwrap().entries.clone();
        second_entries[0].name = "c".into();
        let second_tree = oak_core::Tree::new(second_entries).unwrap();
        repo.store_tree(&second_tree).unwrap();
        let second = Commit::with_timestamp(
            "main".into(),
            None,
            None,
            second_tree.hash,
        ...
        .unwrap();
        repo.store_commit(&second).unwrap();
        ...
        .await
        .unwrap();
        let error = ensure_review_commits_with_budget(
            &repo,
            "http://127.0.0.1:1",
            "oak",
            "oak",
    ...
    #[tokio::test]
    async fn review_commit_error_does_not_wait_for_unbounded_response_body() {
        use tokio::io::{AsyncReadExt, AsyncWriteExt};

        let _ = rustls::crypto::ring::default_provider().install_default();
        for framing in ["Content-Length: 1073741824", "Transfer-Encoding: chunked"] {
            ...
            let server = tokio::spawn(async move {
                ...
                // A body-consuming client cannot finish until this is released.
                let _ = wait_for_release.await;
            });
            let temp = tempfile::TempDir::new().unwrap();
            let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
            let hash = oak_core::hash_bytes(b"missing commit");
            let fetch = ensure_review_commits_local(
                &repo,
                &url,
                "oak",
                "oak",
            ...
            server.await.unwrap();
            let error = result
                .expect("review must not consume an error body")
                .unwrap_err();
            assert!(error.to_string().contains("503"), "{error}");
            assert!(repo.get_commit(&hash).unwrap().is_none());
        }
    }

    #[tokio::test]
    async fn review_commit_budget_rejects_large_body_before_cache_publication() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(urlpath("/api/oak/oak/commits/info"))
            .respond_with(ResponseTemplate::new(200).set_body_string(" ".repeat(129)))
            .mount(&server)
            .await;
        let hash = Hash("ab".repeat(32));
        let error = ensure_commits_local_bounded(
            &repo,
            &server.uri(),
            "oak",
            "oak",
            None,
            std::slice::from_ref(&hash),
            (128, std::time::Duration::from_secs(30)),
        )
        .await
        .unwrap_err();
        assert!(
            error.to_string().contains("response byte budget"),
            "{error}"
        );
        assert!(repo.get_commit(&hash).unwrap().is_none());
    }
    ...
    #[tokio::test]
    async fn review_commit_deadline_leaves_cache_unpublished() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let server = MockServer::start().await;
        Mock::given(method("POST"))
        ...
        let error = ensure_commits_local_bounded(
            ...
            None,
            std::slice::from_ref(&hash),
            (128, std::time::Duration::from_millis(10)),
        )
        .await
        .unwrap_err();
