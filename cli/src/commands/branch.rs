...
pub fn new_branch(
    ...
    if let Some(description) = description {
        repo.edit_branch_description(name, description)?;
    }
...
pub(crate) fn store_remote_branch_metadata(
    ...
    for branch in branches {
        ...
        repo.upsert_branch_metadata_at_generation(
            &Branch {
...
async fn sync_branch_metadata_to_server(
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            {
                output::warning(&format!("Saved locally but {warning_action}: {e}"));
                for command in reconciliation_commands {
                    output::warning(&format!("Inspect remote state: {command}"));
                }
            } else {
...
mod tests {
    ...
    fn remote_metadata_refresh_replaces_stale_local_row_before_close() {
        ...
        store_remote_branch_metadata(
            ...
            repo.description_generation().unwrap(),
        )
        .unwrap();
