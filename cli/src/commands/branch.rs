...
pub fn list_branches_json(path: &Path) -> Result<()> {
    let ctx = crate::resolve::resolve(path)?;
    let repo = ctx.open()?;
    let branches = branch_json_rows(repo.as_ref())?;
...
fn branch_json_rows(repo: &dyn oak_core::Repository) -> Result<Vec<output::BranchJson>> {
    ...
    branches
        .into_iter()
        .map(|br| {
            let head = repo.get_branch_head(&br.name)?.map(|h| h.to_string());
            Ok(output::BranchJson {
                current: current.as_deref() == Some(&br.name),
                name: br.name,
                head,
                description: br.description,
                status: br.status.as_str().to_string(),
...
pub async fn edit_current_branch(path: &Path, description: &str) -> Result<()> {
    ...
    sync_description_to_server(repo.as_ref(), &name).await;

    Ok(())
}

/// Best-effort sync of a branch's row to the server, mirroring what
/// `oak desc` inside a mount already does. Without this, a description
/// set after the last push never leaves the machine: a commit-less
/// `oak push` returns "Already up to date" before sending branch
/// metadata, so the squash-merge message would silently be missing
/// server-side. No-op when the repo has no remote configured; warns
/// (never errors) on network/auth failure so the local update always
/// sticks.
async fn sync_description_to_server(repo: &dyn oak_core::Repository, branch_name: &str) {
    ...
    .await
    {
        Ok(()) => output::success("Description synced to server"),
