...
fn wait_summary_observation<'a>(run: &'a CiRun, at: &'a str) -> WaitSummaryObservation<'a> {
...
pub async fn wait_with_summary(
    ...
    if summary && (!json || expected_commit.is_some_and(|c| !summary_commit_valid(c))) {
        return Err(OakError::InvalidArgument(
    ...
    loop {
        ...
        if all_terminal || timed_out {
            ...
            let observations: Vec<_> = requested_run_ids
                .iter()
            ...
            if summary {
                output::print_json(&WaitSummaryJson {
                    ...
                    expected_commit,
                    state,
                    observations: requested_run_ids
                        .iter()
                        .filter_map(|id| {
                            Some(wait_summary_observation(
                                observed.get(id)?,
                                observed_at.get(id)?,
                            ))
                        })
