...
pub fn run_with_options(path: &Path, opts: CommitOptions) -> Result<()> {
    ...
    // publish preflight above runs first so a missing link/auth never changes
    ...
    let mut changes = old_manifest.diff_with_content_renames(
        &new_manifest,
...
fn preflight_push_after_commit(work_tree: &Path, repo: &dyn Repository) -> Result<()> {
    ...
    let has_token =
        super::credentials::effective_token(&remote, repo.get_metadata(MetadataKey::ApiKey)?)
            .is_some();
    if !has_token {
        return Err(OakError::Config(format!(
            "No Oak credentials found for {remote}. Run `oak login -r {remote}` before using `oak commit --push`."
        )));
    }
...
fn scan_changes_vs_manifest(
    ...
    let mut changes = old_manifest.diff_entries_with_content_renames(
        &scan.entries,
        |h| {
    ...
    if let Some(cone) =
        oak_core::SparseCone::from_metadata(repo.get_metadata(MetadataKey::SparsePaths)?.as_deref())
    {
...
fn manifest_for_head_or_empty(
    ...
    super::merge::manifest_for_commit_or_unavailable(repo, &head_commit, branch_name, "HEAD")
}
