...
pub fn run_with_options(path: &Path, opts: CommitOptions) -> Result<()> {
    ...
    if !std::io::stdout().is_terminal() {
        // The hash is the only information a piped reader can't already
        // know. The command name and success state are already present in the
        // tool call and exit code, so repeat neither on the data channel.
        output::print_line(commit_hash.short());
    } else {
    ...
    if let Ok(Some(remote_url)) = repo.get_metadata(MetadataKey::RemoteUrl) {
        ...
        match rt.block_on(push::run(&work_tree, &remote_url, false, None)) {
            Ok(()) => {}
            Err(OakError::ConflictDetected | OakError::RemoteCommitsNotInLocalHistory) => {
...
fn compute_changes_inner(
    ...
    let new_manifest = Manifest::new(scan.entries);
