...
use oak_core::{
    detect_renames_by_content, BlobSource, BranchStatus, ChangeType, CommitPhaseError, FileChange,
    FileMode, Hash, IgnorePatterns, Manifest, ManifestEntry, MetadataKey, OakError, RenameConfig,
    Result,
};
...
pub fn run_with_options(path: &Path, opts: CommitOptions) -> Result<()> {
    ...
    if push_after_commit {
        preflight_push_after_commit(repo.as_ref())?;
    }
...
fn preflight_push_after_commit(repo: &dyn Repository) -> Result<()> {
    let remote = push_remote_url(repo)?;
    let owner = repo.get_metadata(MetadataKey::RepoOwner)?.ok_or_else(|| {
        OakError::Config(
            "This repository is not linked to an Oak remote repo. Run `oak push --repo <org>/<repo>` once to link it before using `oak commit --push`."
                .to_string(),
        )
    })?;
    let _repo_name = repo.get_metadata(MetadataKey::RepoName)?.ok_or_else(|| {
        OakError::Config(
            "Repository metadata is missing the remote repo name. Re-clone with `oak clone <org>/<repo>` or re-link with `oak push --repo <org>/<repo>` before using `oak commit --push`."
                .to_string(),
        )
    })?;
    if owner.trim().is_empty() {
        return Err(OakError::Config(
            "Repository metadata has an empty remote owner. Re-link with `oak push --repo <org>/<repo>` before using `oak commit --push`."
                .to_string(),
        ));
    }
...
fn push_remote_url(repo: &dyn Repository) -> Result<String> {
    Ok(repo
