...
pub fn run(path: &Path, paths: &[std::path::PathBuf], stat: bool, name_only: bool) -> Result<()> {
    ...
    let (changes, head, _branch) =
        super::commit::compute_changes_for_status(repo.as_ref(), &work_tree)?;
    ...
    print_changes(repo.as_ref(), &work_tree, &head_files, &changes)
}
...
pub(crate) fn render_stat_with_limit(
    ...
    for (index, change) in changes.iter().enumerate() {
        let stat = stat_counts_for_change(repo, work_tree, head_files, change)?;
...
fn print_changes(
    ...
    for change in changes {
        let block = render_change(repo, work_tree, head_files, change)?;
...
pub fn render(repo: &dyn Repository, work_tree: &Path) -> Result<(Vec<FileChange>, String)> {
    ...
    let (changes, head, _branch) = super::commit::compute_changes_for_status(repo, work_tree)?;
...
fn stat_counts_for_change(
    ...
    let (old_bytes, new_bytes) = change_bytes(repo, work_tree, head_files, change)?;
...
fn change_bytes(
    ...
    match change.change_type {
        ChangeType::Added => Ok((Vec::new(), read_worktree_bytes(work_tree, path)?)),
        ChangeType::Deleted => Ok((
            old_bytes_for(repo, head_files, path, &change.old_blob_hash)?,
            Vec::new(),
        )),
        ChangeType::Modified => Ok((
            old_bytes_for(repo, head_files, path, &change.old_blob_hash)?,
            read_worktree_bytes(work_tree, path)?,
        )),
        ChangeType::Renamed => {
            let old_path = change.old_path.as_deref().unwrap_or(path);
            Ok((
                old_bytes_for(repo, head_files, old_path, &change.old_blob_hash)?,
                read_worktree_bytes(work_tree, path)?,
            ))
        }
    }
...
fn run_stat_with_row_limit(
    ...
    let head_manifest = head_manifest_for(repo.as_ref(), &head)?;
    let head_files = manifest_file_map(&head_manifest);
    for line in render_stat_with_limit(repo.as_ref(), &work_tree, &head_files, &changes, max_rows)?
    {
        output::print_line(&line);
    }
    Ok(())
