...
pub fn run(path: &Path, paths: &[std::path::PathBuf], stat: bool) -> Result<()> {
    let ctx = crate::resolve::resolve(path)?;
    let work_tree = ctx.work_tree.clone();
    let repo = ctx.open()?;

    let filters = resolve_filters(path, &work_tree, paths)?;
    let (changes, head, _branch) = super::commit::compute_changes(repo.as_ref(), &work_tree)?;
    ...
    if stat {
        for line in render_stat(repo.as_ref(), &work_tree, &head_files, &changes)? {
    ...
    for change in &changes {
        formatted.push_str(&render_change(
            repo.as_ref(),
            &work_tree,
            &head_files,
            change,
...
/// change and a totals line. Counts are derived from the very same
/// [`render_change_lines`] the full diff prints, so `--stat` and the hunk
/// view always agree (binary files show `bin` instead of counts).
pub(crate) fn render_stat(
    ...
    render_stat_with_limit(repo, work_tree, head_files, changes, None)
}
...
pub(crate) fn render_stat_with_limit(
    ...
    for (index, change) in changes.iter().enumerate() {
        let lines = render_change_lines(repo, work_tree, head_files, change)?;
        let mut added = 0usize;
        let mut removed = 0usize;
        let mut binary = false;
        let mut in_hunk = false;
        for l in &lines {
            if l.starts_with("Binary files ") || l.starts_with("Large files ") {
                binary = true;
        ...
        if binary {
            ...
            if show_row {
                rows.push(format!("{kind} {label}  +{added} -{removed}"));
            }
            total_added += added;
...
pub(crate) fn render_change(
    ...
    for line in render_change_lines(repo, work_tree, head_files, change)? {
...
pub(crate) fn render_change_lines(
    ...
    // it, so diff must too.
    if let Some(notice) = non_text_diff_notice(path, &old_bytes, &new_bytes) {
...
fn non_text_diff_notice(path: &str, old_bytes: &[u8], new_bytes: &[u8]) -> Option<String> {
    ...
    if old_bytes.len() > MAX_TEXT_DIFF_BYTES || new_bytes.len() > MAX_TEXT_DIFF_BYTES {
        return Some(format!(
            "Large files a/{path} and b/{path} differ ({} -> {} bytes; text diff omitted)",
            old_bytes.len(),
            new_bytes.len()
        ));
    }
...
pub fn run_tui(path: &Path, paths: &[std::path::PathBuf]) -> Result<()> {
    if !io::stdout().is_terminal() {
        return run_stat_limited(path, paths, 5);
    }
    ...
    let entries = build_entries(repo.as_ref(), &work_tree, &head_files, &changes)?;
    run_tui_entries(entries, "working tree")
}

fn run_stat_limited(path: &Path, paths: &[std::path::PathBuf], max_rows: usize) -> Result<()> {
    let ctx = crate::resolve::resolve(path)?;
    let work_tree = ctx.work_tree.clone();
    let repo = ctx.open()?;
