...
/// [`render_change_lines_with_options`] with an explicit `-U` context width —
/// the JSON `--hunks` path uses this so `-U 0` actually shrinks patches under
/// a `--max-bytes` budget.
pub(crate) fn render_change_lines_with_context(
    repo: &dyn Repository,
    work_tree: &Path,
    head_files: &HashMap<&str, &ManifestEntry>,
    change: &FileChange,
