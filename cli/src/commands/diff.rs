...
/// a textual hunk for it (binary files, a pure mode change, bytes that decode
/// lossily to identical text): it emits a "differ" notice instead. (A
/// trailing-newline-only change *does* render, as a marked replace of the
/// final line.) (It used to gate every file
...
fn present(
        ...
        if let Ok(tool) = std::env::var("OAK_DIFF_TOOL") {
            if !tool.trim().is_empty() {
                run_external_difftool(
                    ...
                    &base_files,
                    &changes,
                    BlobSource::Snapshot(&source.worktree_blobs),
                )?;
    ...
    if stat || (!print && !io::stdout().is_terminal()) {
        ...
        if !stat {
            ...
            let scope = if whole_branch && !filters.is_empty() {
                format!(
                    " -- {}",
                    filters
                        .iter()
                        .map(|filter| {
                            super::review::shell_quote_path(
                                &work_tree.join(filter).to_string_lossy(),
                            )
                        })
                        .collect::<Vec<_>>()
                        .join(" ")
            ...
            output::print_line(&format!(
                "# hunks: {stub} --print{scope} · structured: {stub} --json{scope}",
...
pub fn dispatch(path: &Path, request: DiffRequest) -> Result<bool> {
    ...
    if endpoints.is_empty() {
        ...
        if request.json {
            drop(repo);
            if request.branch {
                return super::review::current_branch_diff_json(path, &paths, request.json_options);
            }
...
fn branch_source(repo: &dyn Repository, work_tree: &Path) -> Result<DiffSource> {
    let source = branch_worktree_source(repo, work_tree)?;
    for caveat in &source.caveats {
        output::warning(caveat);
    }
    Ok(DiffSource {
        changes: source.changes,
        base_manifest: Some(source.base_manifest),
        worktree_blobs: source.worktree_blobs,
        scope_label: format!("branch {} vs {}", source.branch, source.parent),
        command_stub: "oak diff --branch".to_string(),
        conflict_paths: Vec::new(),
    })
}

/// One non-persisting contribution scan, shared by human and JSON output.
/// A missing fork is an error, never a fallback to a snapshot comparison.
pub(crate) struct BranchWorktreeSource {
    pub changes: Vec<FileChange>,
    pub base_manifest: oak_core::Manifest,
    pub worktree_blobs: HashMap<Hash, Vec<u8>>,
    pub branch: String,
    pub parent: String,
    pub fork: Option<Hash>,
    pub caveats: Vec<String>,
}

pub(crate) fn branch_worktree_source(
    repo: &dyn Repository,
    work_tree: &Path,
) -> Result<BranchWorktreeSource> {
    ...
    let (fork, base_manifest, caveats) =
        super::review::branch_fork_base_snapshot(repo, &branch, &parent)?;
    let (changes, worktree_blobs) =
        super::commit::compute_worktree_changes_vs_manifest(repo, work_tree, &base_manifest)?;
    Ok(BranchWorktreeSource {
        changes,
        base_manifest,
        worktree_blobs,
        branch,
...
pub(crate) fn render_stat(
    ...
    render_worktree_stat_with_limit(repo, work_tree, head_files, changes, None)
}

pub(crate) fn render_worktree_stat_with_limit(
    repo: &dyn Repository,
    work_tree: &Path,
    head_files: &HashMap<&str, &ManifestEntry>,
    changes: &[FileChange],
...
pub(crate) fn print_formatted(formatted: &str) {
    ...
    // Non-TTY or pager failed: print directly. Split on `\n` only so CRLF
    // content keeps its `\r` (`str::lines` would strip it, and the printed
    // patch would no longer apply to the file it describes).
...
fn print_changes(
    ...
    for change in changes {
        ...
        // `split_terminator`, not `lines`: keep the `\r` of CRLF content.
...
pub fn render(repo: &dyn Repository, work_tree: &Path) -> Result<(Vec<FileChange>, String)> {
    ...
    for change in &changes {
        formatted.push_str(&render_change_with_options(
            ...
            &head_files,
            change,
            BlobSource::Snapshot(&worktree_blobs),
            RenderOptions::default(),
        )?);
        formatted.push('\n');
    }
...
fn render_change_with_options(
    ...
    for line in render_change_lines_with_options(
        ...
        // Keep a CRLF line's `\r` outside the colour wrap, as git does:
        // `\r` followed by a reset code renders as a visible `^M` in
        // `less -R`, while `\r\n` does not. Bytes are identical when colour
        // is off, so piped patches are unaffected.
        let (body, cr) = match line.strip_suffix('\r') {
            Some(body) => (body, "\r"),
            None => (line.as_str(), ""),
        };
        block.push_str(&output::format_diff_line(body));
        block.push_str(cr);
        block.push('\n');
    }
    Ok(block)
...
/// Recorded comparisons use stored objects or bytes captured by their scan.
/// Live mount/working-tree helpers explicitly opt into reading each current
/// file, keeping stat memory bounded to one file instead of preloading a tree.
#[derive(Clone, Copy)]
enum BlobSource<'a> {
    Snapshot(&'a HashMap<Hash, Vec<u8>>),
    Worktree,
}
...
fn change_bytes<'a>(
    ...
    head_files: &HashMap<&str, &ManifestEntry>,
    change: &FileChange,
    worktree_blobs: BlobSource<'a>,
) -> Result<DiffBytes<'a>> {
    let path = &change.path;
    let current_bytes = || -> Result<Cow<'a, [u8]>> {
        let BlobSource::Snapshot(worktree_blobs) = worktree_blobs else {
            return Ok(Cow::Owned(read_worktree_bytes(work_tree, path)?));
        };
        if let Some(hash) = &change.new_blob_hash {
            if let Some(bytes) = worktree_blobs.get(hash) {
                return Ok(Cow::Borrowed(bytes.as_slice()));
            }
            // A recorded post-image must not be replaced with unrelated checkout
            // bytes. Worktree callers explicitly supply their scanned bytes.
            return Ok(Cow::Owned(read_blob_bytes(repo, hash)?));
        }
        Err(OakError::IncompleteBlobData {
            context: format!("diff post-image for {path}"),
            missing: "recorded blob hash".to_string(),
        })
    };
    match change.change_type {
...
fn read_blob_bytes(repo: &dyn Repository, hash: &Hash) -> Result<Vec<u8>> {
    // The canonical empty blob needs no stored payload; missing nonempty
    // objects, however, are unavailable content, never an empty diff side.
    if hash == &oak_core::hash_bytes(&[]) {
        return Ok(Vec::new());
    }
    repo.get_blob(hash)?
        .map(|b| b.content)
        .ok_or_else(|| OakError::IncompleteBlobData {
            context: "diff content".to_string(),
...
/// Render captured bytes through the same semantic headers and hunk formatter
/// as the human diff. Snapshot mode never falls back to the live filesystem.
pub(crate) fn render_change_lines_from_snapshot(
    repo: &dyn Repository,
    head_files: &HashMap<&str, &ManifestEntry>,
    change: &FileChange,
...
/// [`render_change`] colorizes these for `--print`; the TUI in
/// [`build_entries`] styles them per-line for the browser.
fn render_change_lines_with_options(
    ...
    if diff.has_changes {
        // Same shape as `FileDiff::to_unified_with_context`, taken straight
        // from the hunks (no render-then-resplit): the `diff --oak` header,
        // rename/mode headers, `---`/`+++`, then each hunk through the one
        // shared body renderer so prefixes and the no-newline marker cannot
        // drift between surfaces. Line bytes (including any `\r`) are intact.
        lines.push(format!("diff --oak a/{path} b/{path}"));
        for h in rename_header.iter().chain(mode_header.iter()) {
            lines.extend(h.lines().map(str::to_string));
        }
        lines.push(format!("--- a/{path}"));
        lines.push(format!("+++ b/{path}"));
        for hunk in diff.hunks(options.context) {
            lines.extend(hunk.unified_lines());
        }
    } else {
        // No textual hunk: a pure rename, a mode-only change, or a byte-level
        // change the line differ can't represent (e.g. distinct invalid UTF-8
        // sequences that decode lossily to the same text). The blob hash
        // still differs, so emit the header — never drop the path.
        lines.push(format!("diff --oak a/{path} b/{path}"));
        if let Some(h) = &rename_header {
            lines.extend(h.lines().map(str::to_string));
        }
        ...
        if rename_header.is_none() && mode_header.is_none() {
...
fn compute_line_emphasis(lines: &[String]) -> Vec<Vec<std::ops::Range<usize>>> {
    ...
    // `\ No newline at end of file` is metadata about the line before it,
    // not a line of its own: it must not split a removed run from the added
    // run that follows, or the final line of a file would lose its emphasis.
    // (Content lines always carry a ` `/`-`/`+` prefix, so a bare marker
    // string can only be the marker itself.)
    let is_marker = |i: usize| lines[i] == oak_core::diff::NO_NEWLINE_MARKER;
    let mut i = 0;
    while i < lines.len() {
        if !is_removed(i) {
            i += 1;
        ...
        while i < lines.len() && (is_removed(i) || is_marker(i)) {
            if is_removed(i) {
                removed.push(i);
            }
            i += 1;
        ...
        while i < lines.len() && (is_added(i) || is_marker(i)) {
            if is_added(i) {
                added.push(i);
            }
            i += 1;
        }
        for (&old_index, &new_index) in removed.iter().zip(&added).take(MAX_INLINE_PAIRS) {
            let old_line = &lines[old_index];
            let new_line = &lines[new_index];
            if let Some((old_ranges, new_ranges)) =
                oak_core::inline_diff_ranges(&old_line[1..], &new_line[1..])
            {
                // Shift past the +/- prefix byte.
                emphasis[old_index] = old_ranges
                    .into_iter()
                    .map(|r| r.start + 1..r.end + 1)
                    .collect();
                emphasis[new_index] = new_ranges
                    .into_iter()
                    .map(|r| r.start + 1..r.end + 1)
                    .collect();
            }
...
mod tests {
    ...
    #[test]
    fn historical_missing_bytes_never_become_empty_or_worktree_content() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        fs::write(temp.path().join("tracked.txt"), "unrelated checkout\n").unwrap();
        let missing = oak_core::hash_bytes(b"missing history\n");
        for change_type in [
            ChangeType::Added,
            ChangeType::Deleted,
            ChangeType::Modified,
            ...
            let mut change = change("tracked.txt", None);
            change.change_type = change_type;
            change.old_blob_hash = (change_type != ChangeType::Added).then(|| missing.clone());
            change.new_blob_hash = (change_type != ChangeType::Deleted).then(|| missing.clone());
            let result = render_change_lines_with_options(
                &repo,
                temp.path(),
                &HashMap::new(),
                &change,
                BlobSource::Snapshot(&HashMap::new()),
                RenderOptions::default(),
            );
            assert!(
                matches!(result, Err(OakError::IncompleteBlobData { .. })),
                "historical {change_type:?} must report unavailable, got {result:?}"
            );
        }
    }

    #[test]
    fn empty_blob_and_absent_sides_need_no_stored_payload() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        fs::write(temp.path().join("tracked.txt"), "unrelated checkout\n").unwrap();
        for change_type in [ChangeType::Added, ChangeType::Deleted, ChangeType::Renamed] {
            let mut change = change("tracked.txt", Some("old.txt"));
            change.change_type = change_type;
            change.old_blob_hash =
                (change_type != ChangeType::Added).then(|| oak_core::hash_bytes(&[]));
            change.new_blob_hash =
                (change_type != ChangeType::Deleted).then(|| oak_core::hash_bytes(&[]));
            let blobs = HashMap::new();
            let (old, new) = change_bytes(
                &repo,
                temp.path(),
                &HashMap::new(),
                &change,
    ...
    #[test]
    fn worktree_only_helpers_explicitly_read_current_content() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        fs::write(temp.path().join("tracked.txt"), "current checkout\n").unwrap();
        let mut change = change("tracked.txt", None);
        change.old_blob_hash = Some(repo.put_blob(b"old\n".to_vec()).unwrap());
        change.new_blob_hash = Some(oak_core::hash_bytes(b"current checkout\n"));
        let files = HashMap::new();
        assert!(render_change(&repo, temp.path(), &files, &change)
            .unwrap()
            .contains("+current checkout"));
        assert!(render_change_lines_with_context_and_text(
            &repo,
            temp.path(),
            &files,
            &change,
        ...
        .unwrap()
        .iter()
        .any(|l| l == "+current checkout"));
        assert!(
            build_entries(&repo, temp.path(), &files, std::slice::from_ref(&change)).unwrap()[0]
                .lines
                .iter()
                .any(|l| l == "+current checkout")
        );
        assert!(
            render_worktree_stat_with_limit(&repo, temp.path(), &files, &[change], Some(5))
                .unwrap()[0]
                .contains("+1 -1")
        );
    }

    /// Rendering must hand the applicator every byte of the line: CRLF
    /// content keeps its `\r`, and an unterminated final line is followed
    /// by the marker (fb521). Pre-fix, `str::lines` ate the `\r`.
    #[test]
    fn rendered_lines_keep_carriage_returns_and_mark_missing_final_newline() {
        let temp = TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let old = b"a\r\nb\r\nc".to_vec();
        let new = b"a\r\nB\r\nc".to_vec();
        let mut change = change("f.txt", None);
        change.old_blob_hash = Some(repo.put_blob(old).unwrap());
        let new_hash = oak_core::hash_bytes(&new);
        change.new_blob_hash = Some(new_hash.clone());
        let mut blobs = HashMap::new();
        blobs.insert(new_hash, new);
        let lines = render_change_lines_from_snapshot(
            &repo,
            &HashMap::new(),
            &change,
            &blobs,
            oak_core::DEFAULT_CONTEXT_LINES,
            false,
        )
        .unwrap();
        assert_eq!(
            ...
            vec![
                ...
                "+B\r",
                " c",
                oak_core::diff::NO_NEWLINE_MARKER,
            ]
        );
    }

    /// The marker sits between a removed run and its added run when the
    /// old final line changes; `--word-diff` must still pair those lines.
    #[test]
    fn word_diff_pairs_across_the_no_newline_marker() {
        ...
        .collect();
        apply_word_diff_markers(&mut lines);
        assert_eq!(lines[5], "-the quick [-brown-] fox");
        assert_eq!(lines[6], oak_core::diff::NO_NEWLINE_MARKER);
        assert_eq!(lines[7], "+the quick {+red+} fox");
        assert_eq!(lines[8], oak_core::diff::NO_NEWLINE_MARKER);
    }
