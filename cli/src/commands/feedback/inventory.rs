...
async fn read<T: serde::de::DeserializeOwned>(
    ...
    if status == reqwest::StatusCode::METHOD_NOT_ALLOWED {
        return Err(failed("feedback GET request rejected (HTTP 405 Method Not Allowed); check server feedback route/API compatibility. This response does not establish whether credentials are valid. No legacy fallback was attempted."));
    }
