...
/// updated and the fetch retried once — see `follow_remote_move`.
pub async fn run(path: &Path) -> Result<()> {
    match run_once(path).await {
...
async fn run_once(path: &Path) -> Result<()> {
    ...
    if let Some(plan) = plan {
        let (changes, _, _) = super::commit::compute_changes(&repo, &ctx.work_tree)?;
