//! `oak histedit` — reorder a branch's commits and split them into branches.
//!
...
//! histedit reports the conflicting paths and changes nothing rather than
//! producing a branch that can't stand on its own.
//!
//! Two front ends share one engine:
//!
//!   * **Interactive** (`oak histedit`) — a TUI to reorder / drop / split.
//!   * **Scriptable** (`oak histedit --plan <file|->`) — a `rebase -i`-style
...
/// The slice of history `histedit` operates on: the branch's own commits
    ...
    if update_worktree {
        if let Some(tip) = &source_tip {
            ...
            let manifest = repo
                .get_manifest(&commit.manifest_hash)?
                .unwrap_or_else(Manifest::empty);
            super::switch::update_working_dir(root, repo, &manifest)?;
            repo.set_head(tip)?;
        }
    }
...
fn print_plan_summary(source_branch: &str, history: &BranchHistory, plan: &Plan) {
    output::info(&format!(
        "histedit of '{source_branch}' ({} commit(s) on base {})",
        history.commits.len(),
        history
            .base_commit
            .as_ref()
...
pub fn run(path: &Path, from: Option<&str>, plan_path: Option<&str>, dry_run: bool) -> Result<()> {
    ...
    let plan_text = match plan_path {
        ...
        None => match tui::edit_plan(&branch, &history)? {
            Some(text) => text,
            None => {
                output::info("Histedit cancelled — nothing changed.");
                return Ok(());
            }
        },
    };
    ...
    let results = apply_plan(repo_ref, &root, &branch, &history, &plan, editing_current)?;
