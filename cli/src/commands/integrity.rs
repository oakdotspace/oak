...
fn proves_bounded_blob_history(report: &IntegrityReport) -> bool {
    !report.healthy
        && !report.complete
...
fn is_clone_budget_finding(finding: &IntegrityFinding) -> bool {
    matches!(
        finding.code.as_str(),
...
fn adjudicated_known_loss_hashes(
    ...
    // Negotiation support alone is not an adjudicated-loss disposition. The
    // server also echoes this protocol on healthy and budget-limited reports.
    if report.known_loss_protocol.as_deref() != Some(oak_core::protocol::KNOWN_LOSS_PROTOCOL)
        || !report
            .findings
            .iter()
            .any(|finding| finding.code == "known_lost_blob")
    {
        return Ok(None);
    }
...
async fn fetch_report(
    ...
    if report.repo != format!("{owner}/{name}") {
        return Err(OakError::Server(
            "invalid integrity response: repository identity did not match the request".to_string(),
        ));
    }
    if let Some(hash) = options.blob {
        if report.blob_evidence.iter().any(|blob| blob.hash != hash) {
            return Err(OakError::Server(
                "invalid integrity response: blob identity did not match the request".to_string(),
            ));
        }
        if report.blob_evidence.len() > 1
            || ((proves_complete_health(&report) || proves_bounded_blob_history(&report))
                && report.blob_evidence.len() != 1)
        {
            return Err(OakError::Server(
                "invalid integrity response: expected exactly one requested blob evidence record"
                    .to_string(),
            ));
        }
        if (proves_complete_health(&report) || proves_bounded_blob_history(&report))
            && report.blob_evidence.iter().any(|blob| {
                !blob.metadata_present
                    || !blob.mapping_present
                    || blob.chunks.iter().any(|chunk| {
                        !chunk.metadata_present
                            || chunk.object_present == Some(false)
                            || chunk.hash_verified == Some(false)
                    })
            })
        {
            return Err(OakError::Server(
                "invalid integrity response: successful blob proof contains negative evidence"
                    .to_string(),
            ));
        }
    }
    ...
    if (proves_complete_health(&report)
        || (options.blob.is_some() && proves_bounded_blob_history(&report)))
        && report.scope.depth != options.depth
    {
        return Err(OakError::Server(
            "invalid integrity scope: proved history depth did not match the request".to_string(),
        ));
    }
...
pub async fn doctor(
    ...
    .await?
    {
        FetchResult::Report(report) => finish_diagnostic(*report, json),
...
pub async fn blob_info_scoped(
    ...
    .await?
    {
        FetchResult::Report(report) => {
            if proves_bounded_blob_history(&report) {
                print_report(&report, json)?;
                if !json {
...
pub(crate) async fn preflight_clone_acquisition_with_policy_and_token(
            ...
            ClonePreflightCapability::LegacyAccessible => {
                ...
                if !scoped || policy.allow_legacy_scope {
                    ...
                    return Ok(ClonePreflightEvidence {
                        ...
                        selected_branch: None,
                        disposition: ClonePreflightDisposition::LegacyUnverified,
                    });
                }
                return Err(OakError::Server(
    ...
    let (snapshot_token, known_lost_blob_hashes, selected_branch, disposition) =
...
pub(crate) enum ClonePreflightDisposition {
    Verified,
    BudgetOverride,
    AdjudicatedKnownLoss,
    LegacyUnverified,
}

impl ClonePreflightDisposition {
    pub(crate) fn as_str(self) -> &'static str {
        match self {
            Self::Verified => "bounded_v1",
            Self::BudgetOverride => "bounded_v1_budget_override",
            Self::AdjudicatedKnownLoss => "bounded_v1_known_loss",
            Self::LegacyUnverified => "legacy_unverified",
        }
    }

    pub(crate) fn proof_scope(self) -> &'static str {
        match self {
            Self::Verified => "requested_history_and_materialization",
            Self::BudgetOverride => "snapshot_bound_budget_incomplete",
            Self::AdjudicatedKnownLoss => "requested_history_with_adjudicated_historical_loss",
            Self::LegacyUnverified => "downloaded_objects_only",
        }
    }
}
...
pub(crate) struct ClonePreflightEvidence {
    ...
    pub selected_branch: Option<SelectedBranchEvidence>,
    pub disposition: ClonePreflightDisposition,
}
...
async fn preflight_clone_with_policy_and_verification(
    ...
    let selected_evidence = match selected_branch {
        Some(name) => {
            if let Some(evidence) = report.selected_branch.as_ref() {
    ...
    // Inconclusive diagnostics need not have completed admission evidence.
    // Every path that actually permits pull still requires both the snapshot
    // and any requested selected-branch identity, including explicit overrides.
    let admission_token = || {
        let token = report.snapshot_token.clone().ok_or_else(|| {
            OakError::Server(
                "invalid integrity response: bounded preflight omitted its snapshot token"
                    .to_string(),
            )
        })?;
        if selected_branch.is_some() && selected_evidence.is_none() {
            return Err(OakError::Server(
    ...
    if proves_complete_health(&report) {
        return Ok((
            ...
            selected_evidence,
            ClonePreflightDisposition::Verified,
        ));
    }
    // A partial report can include loss findings without establishing the
    // complete historical-only disposition required to admit degraded clone.
    let known_loss = if report.complete && !report.truncated {
        adjudicated_known_loss_hashes(&report)?
    } else {
        None
    };
    if let Some(hashes) = known_loss {
        let snapshot_token = admission_token()?;
        output::warning(&format!(
        ...
        return Ok((
            ...
            hashes,
            selected_evidence,
            ClonePreflightDisposition::AdjudicatedKnownLoss,
        ));
    }
    let budget_only = !report.healthy
        ...
        && report.verification == Verification::Metadata
        && !report.findings.is_empty()
        && report.findings.iter().all(is_clone_budget_finding);
    if policy.allow_unverified_budget && budget_only {
        ...
        return Ok((
            ...
            std::collections::HashSet::new(),
            selected_evidence,
            ClonePreflightDisposition::BudgetOverride,
        ));
    }
    let first = report
        .findings
        .iter()
        .find(|finding| !is_clone_budget_finding(finding))
    ...
    if budget_only {
        ...
        if admission_token().is_ok() {
            arguments.push("--allow-unverified-integrity".to_string());
        }
        arguments.extend(["--remote".to_string(), remote.to_string()]);
...
mod tests {
    ...
    async fn blob_info_treats_target_history_window_as_verified_scope_not_corruption() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                ...
                "blob_evidence": [{
                    "hash": "a".repeat(64), "metadata_present": true, "mapping_present": true
                }],
                "findings": [{
    ...
    async fn clone_rejects_integrity_report_for_a_different_repository() {
        ...
        assert!(error
            .to_string()
            .contains("repository identity did not match"));
