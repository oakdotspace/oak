...
fn handle_key(app: &mut App, event: KeyEvent) {
    ...
    if app.search_active {
        // `C-g` (and `Esc`) back out of the search; `Enter` keeps the filter
        // and returns to the list. Chorded keys must never land in the query
        // as bare letters, which is what `keys::text_input` guards.
        ...
        match key {
            ...
            _ => {
                if let Some(c) = keys::text_input(&event) {
    ...
    if keys::is_search(&event) {
        app.search_active = true;
        return;
    }

    // Motions first, in every dialect (arrows / vi+less / emacs); pane-specific
    // keys below. `Space` pages in both panes here — neither has a competing
    // meaning for it.
    if let Some(motion) = keys::nav(&event, SpaceKey::Pages) {
        match app.focus {
            Focus::List => match motion {
                Nav::Top => app.select_first(),
                Nav::Bottom => app.select_last(),
                other => {
                    if let Some(d) = keys::delta(other, app.list_rows) {
                        app.move_selection(d);
                    }
                }
            },
            Focus::Detail => match motion {
                Nav::Top => app.detail_scroll = 0,
                Nav::Bottom => app.detail_scroll = usize::MAX,
                other => {
                    if let Some(d) = keys::delta(other, app.detail_rows) {
                        app.scroll_detail(d);
                    }
                }
            },
...
fn draw(f: &mut Frame, app: &mut App) {
    ...
    let help = if app.focus == Focus::List {
        " ↑↓/jk/^P^N move · Space/PgDn page · Enter/→/Tab details · c switch · r HEAD · / search · ? keys · q quit"
    } else {
        " ↑↓/jk/^P^N scroll · Space/PgDn/^V page · ^U^D half · g/G top/bottom · ←/Esc/Tab back · ? keys · q quit"
    };
    f.render_widget(
...
/// Built from [`keys::HELP_ROWS`] so it can't drift from the matcher.
fn draw_help_overlay(f: &mut Frame, area: Rect) {
    let lines: Vec<Line<'static>> = keys::HELP_ROWS
        .iter()
        .map(|(k, what)| {
            Line::from(vec![
...
fn draw_list(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    // Remember the visible row count so a page key advances one screenful, the
    // way `less` does, instead of a fixed 20 rows. Measured after the search
    // box is carved off, so paging shrinks with the list.
    app.list_rows = list_area.height.saturating_sub(2) as usize;
...
fn draw_detail(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    app.detail_rows = area.height.saturating_sub(2) as usize;
...
/// Key-binding wiring for the log viewer: the shared table in
/// [`crate::commands::keys`] is unit-tested there, so these cover what only
/// this viewer can get wrong — which pane a motion lands in, that paging
/// follows the pane height, and that search mode doesn't swallow chords.
#[cfg(test)]
mod key_tests {
    use super::*;
        ...
        let commits: Vec<Commit> = (0..n)
            .map(|i| {
                Commit::with_timestamp(
                    ...
                    None,
                    None,
                    oak_core::hash_bytes(format!("m{i}").as_bytes()),
                    format!("author-{i}"),
            ...
            .collect();
        let ctx = RepoContext {
            oak_dir: PathBuf::from("/nonexistent/.oak"),
            work_tree: PathBuf::from("/nonexistent"),
            backend: Backend::Sqlite,
        };
        App::new(commits, Some("main".to_string()), ctx)
    }
    ...
    /// Space advances by a screenful of the *current* pane height, so the
    /// viewer behaves like `less` on a tall terminal and a short one alike.
    #[test]
    fn space_advances_one_screenful_of_the_current_pane() {
        let mut app = app_with(200);
        app.list_rows = 30;
        plain(&mut app, ' ');
        assert_eq!(app.selected, 28, "one page = pane height minus the overlap");
        plain(&mut app, 'b');
        assert_eq!(app.selected, 0, "b pages back the same distance");

        app.list_rows = 10;
        plain(&mut app, ' ');
        assert_eq!(app.selected, 8, "paging shrinks with the pane");
    }

    /// The emacs dialect drives the same motions as vi and the arrow keys.
    #[test]
    fn emacs_motions_move_the_focused_pane() {
        ...
        assert_eq!(app.selected, 99, "M-> jumps to the last commit");
        press(&mut app, KeyCode::Char('<'), KeyModifiers::ALT);
        assert_eq!(app.selected, 0, "M-< jumps back to the first");

        // Move to the detail pane; the same keys now scroll it.
        press(&mut app, KeyCode::Enter, KeyModifiers::NONE);
        assert_eq!(app.focus, Focus::Detail);
        app.detail_rows = 40;
        ctrl(&mut app, 'v');
        assert_eq!(app.detail_scroll, 38);
        press(&mut app, KeyCode::Char('v'), KeyModifiers::ALT);
        assert_eq!(app.detail_scroll, 0, "M-v pages back");
    }

    /// `C-s` opens the search, typing filters, and `C-g` backs out — without
    /// the chord itself being typed into the query (the bug bare
    /// `KeyCode::Char(c)` matching used to have).
    #[test]
    fn search_opens_on_ctrl_s_and_ctrl_g_cancels_without_typing_a_g() {
        ...
        assert!(!app.filtered_indices.is_empty(), "matches author-N");

        ctrl(&mut app, 'g');
        assert!(!app.search_active);
        assert_eq!(app.search_query, "", "C-g clears rather than typing 'g'");
        assert_eq!(app.filtered_indices.len(), 20, "and restores the full list");
    }

    /// `?` raises the binding overlay and any key dismisses it — including one
    /// that would otherwise move the cursor.
    #[test]
    fn help_overlay_opens_on_question_mark_and_swallows_the_next_key() {
        let mut app = app_with(20);
        plain(&mut app, '?');
        assert!(app.show_help);
        plain(&mut app, 'j');
        assert!(!app.show_help);
        assert_eq!(app.selected, 0, "the dismissing key doesn't also move");
    }
    ...
    /// Drawing also records the pane heights paging depends on.
    #[test]
