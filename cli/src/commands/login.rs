...
pub fn run(remote: &str) -> Result<()> {
    ...
    let (token, username) = rt.block_on(run_browser_login(&remote))?;
    ...
    // Offer to wire a live `oak prompt` status segment into the user's shell
    // prompt. Interactive-only and best-effort — never fails the login.
    crate::commands::prompt::offer_shell_integration();
    Ok(())
}
