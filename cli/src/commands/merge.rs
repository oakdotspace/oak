...
pub async fn run(
    ...
    match run_once(path, continue_merge, abort_merge, branch, wait, force, json).await {
        Err(OakError::RemoteMoved { origin }) if crate::http::is_trusted_origin(&origin) => {
            ...
            run_once(path, continue_merge, abort_merge, branch, wait, force, json).await
        }
        result => result,
    }
}
...
async fn run_once(
    ...
    if parent_name == "main" {
        ...
        if matches!(ctx.backend, crate::resolve::Backend::Sqlite) {
            push_branch_for_merge(&ctx, &branch_name, json).await?;
        }
        return merge_to_main_via_server(
            &lock,
            repo.as_ref(),
            root,
            &branch_name,
    ...
    if force || json {
        return Err(OakError::InvalidArgument(
            "`oak merge --force` and actual `oak merge --json` apply only to server-side merges onto main"
                .to_string(),
        ));
    }
    ...
    } in conflicts
    {
        match (branch_entry, parent_entry) {
            (Some(be), Some(pe)) => {
                ...
                // conflict.
                let base_text = match base_manifest.entries.iter().find(|e| e.path == path) {
                    Some(entry) => {
                        let blob = repo.get_blob(&entry.blob_hash)?.ok_or_else(|| {
                            incomplete_blob_error(
                                &format!("conflict resolution for '{path}'"),
                                std::slice::from_ref(&entry.blob_hash),
                            )
                        })?;
...
/// the merge.
async fn push_branch_for_merge(
    ...
    if suppress_result {
        return super::push::push_async_suppressing_result(
            &repo,
            &ctx.work_tree,
            &remote,
            &owner,
...
/// Like [`find_lca_manifest_with_parent`], but doesn't require the LCA
/// manifest's blob *content* to be local. Mount pulls run over a lazy cache
/// that holds blob metadata (sizes, chunk refs) without content and hydrates
/// blobs on demand — demanding every base blob up front would force-hydrate
/// the whole repo (and made `oak pull` in a fresh mount fail outright). The
/// mount merge planner fetches exactly the blobs the 3-way merge needs (see
/// `mount::pull::plan_merge`), so manifest-level resolution is enough here.
pub(crate) fn find_lca_manifest_for_lazy_cache(
    repo: &dyn Repository,
    branch_a: &str,
    branch_b: &str,
    branch_b_head: Option<&Hash>,
) -> Result<Manifest> {
    match resolve_merge_base_impl(repo, branch_a, branch_b, branch_b_head, false)? {
        MergeBaseResolution::CommonAncestor(manifest)
        | MergeBaseResolution::DeclaredRoot(manifest) => Ok(manifest),
        MergeBaseResolution::Unavailable(reason) => Err(reason.to_error(branch_a, branch_b)),
    }
...
pub(crate) fn resolve_merge_base(
    ...
    resolve_merge_base_impl(repo, branch_a, branch_b, branch_b_head, true)
}

/// As [`resolve_merge_base`], but takes both heads and the source's declared
/// parent from an immutable remote snapshot rather than local branch refs.
pub(crate) fn resolve_merge_base_from_heads(
    repo: &dyn Repository,
    branch_a: &str,
    branch_a_head: Option<&Hash>,
    branch_a_parent: Option<&str>,
    branch_b: &str,
    branch_b_head: Option<&Hash>,
) -> Result<MergeBaseResolution> {
    resolve_merge_base_impl_from_heads(
        repo,
        branch_a,
        branch_a_head,
        branch_a_parent,
...
fn resolve_merge_base_impl(
    ...
    let branch_a_head = repo.get_branch_head(branch_a)?;
    let branch_a_parent = repo
        .get_branch(branch_a)?
        .and_then(|branch| branch.parent_branch);
    let branch_b_head_owned = match branch_b_head {
        Some(head) => Some(head.clone()),
        None => repo.get_branch_head(branch_b)?,
    };
    resolve_merge_base_impl_from_heads(
        repo,
        branch_a,
        branch_a_head.as_ref(),
        branch_a_parent.as_deref(),
        branch_b,
        branch_b_head_owned.as_ref(),
        require_blobs,
    )
}

fn resolve_merge_base_impl_from_heads(
    repo: &dyn Repository,
    branch_a: &str,
    branch_a_head: Option<&Hash>,
    branch_a_parent: Option<&str>,
    ...
    let hash = match resolve_merge_base_identity_from_heads(
        repo,
        branch_a,
        branch_a_head,
        branch_a_parent,
        ...
        MergeBaseIdentity::Commit(hash) => hash,
        MergeBaseIdentity::DeclaredRoot => {
            return Ok(MergeBaseResolution::DeclaredRoot(Manifest::empty()))
        }
        MergeBaseIdentity::Unavailable(reason) => {
            return Ok(MergeBaseResolution::Unavailable(reason))
        }
    };
    let commit = repo.get_commit(&hash)?.ok_or_else(|| {
        incomplete_ancestry_error(branch_a, branch_b, std::slice::from_ref(&hash))
    })?;
    let Some(manifest) = repo.get_manifest(&commit.manifest_hash)? else {
        return Ok(MergeBaseResolution::Unavailable(
            MergeBaseUnavailable::IncompleteManifestData {
                missing: vec![commit.manifest_hash],
            },
        ));
    };
    if require_blobs {
        let missing = missing_blobs_in_manifest(repo, &manifest, None)?;
        if !missing.is_empty() {
            return Ok(MergeBaseResolution::Unavailable(
                MergeBaseUnavailable::IncompleteBlobData { missing },
            ));
        }
    }
    Ok(MergeBaseResolution::CommonAncestor(manifest))
}

pub(crate) enum MergeBaseIdentity {
    Commit(Hash),
    DeclaredRoot,
    Unavailable(MergeBaseUnavailable),
}

/// Select the same proven base as merge without requiring its object closure.
/// Preparation may hydrate this identity, never invent one from missing ancestry.
#[allow(dead_code)] // retained local-ref wrapper for callers outside remote analysis
pub(crate) fn resolve_merge_base_identity(
    repo: &dyn Repository,
    branch_a: &str,
    branch_b: &str,
    branch_b_head: Option<&Hash>,
) -> Result<MergeBaseIdentity> {
    let branch_a_head = repo.get_branch_head(branch_a)?;
    let branch_a_parent = repo
        .get_branch(branch_a)?
        .and_then(|branch| branch.parent_branch);
    ...
    resolve_merge_base_identity_from_heads(
        repo,
        branch_a,
        branch_a_head.as_ref(),
        branch_a_parent.as_deref(),
...
/// Resolve a merge-base identity using immutable caller-supplied branch
/// evidence. Read-only remote analysis must use this instead of installing
/// remote branch rows or heads into the caller's repository.
pub(crate) fn resolve_merge_base_identity_from_heads(
    repo: &dyn Repository,
    branch_a: &str,
    branch_a_head: Option<&Hash>,
    branch_a_parent: Option<&str>,
    ...
    let a_walk = match branch_a_head {
        Some(head) => collect_ancestors_from_head(repo, head)?,
        None => AncestorWalk::default(),
    };
    let b_walk = match branch_b_head {
        Some(head) => collect_ancestors_from_head(repo, head)?,
        // This is an immutable-evidence resolver. An absent remote target
        // head is unavailable, never permission to consult a local ref.
        None => AncestorWalk::default(),
    };
    ...
    for hash in &a_walk.ancestors {
        if b_ancestor_set.contains(&hash.to_string()) {
            ...
            if !missing_edges.is_empty() {
                return Ok(MergeBaseIdentity::Unavailable(
            ...
            return Ok(MergeBaseIdentity::Commit(hash.clone()));
        }
    }
    ...
    if declared_root_base_with_parent(branch_a, branch_a_parent, branch_b, &a_walk.ancestors, repo)?
    {
        return Ok(MergeBaseIdentity::DeclaredRoot);
    }
    ...
    Ok(MergeBaseIdentity::Unavailable(
...
pub(crate) fn manifest_for_commit_or_unavailable(
    ...
    manifest_for_commit_or_unavailable_in_cone(repo, commit, left, right, None)
}

/// Resolve a commit manifest for a sparse working-tree operation. The manifest
/// itself remains complete; only blob-byte completeness is scoped to the
/// materialized cone. Three-way merge callers use the strict wrapper above.
pub(crate) fn manifest_for_commit_or_unavailable_in_cone(
    repo: &dyn Repository,
    commit: &Commit,
    left: &str,
    right: &str,
    cone: Option<&oak_core::SparseCone>,
) -> Result<Manifest> {
    ...
    let missing_blobs = missing_blobs_in_manifest(repo, &manifest, cone)?;
...
fn missing_blobs_in_manifest(
    repo: &dyn Repository,
    manifest: &Manifest,
    cone: Option<&oak_core::SparseCone>,
) -> Result<Vec<Hash>> {
    // Blobs the server withheld under path-based permissions are absent by
    // design, not a torn snapshot — don't fail HEAD/status/merge resolution
    // over them. (A content merge that actually needs such a blob still fails
    // at its own read, which is correct: you can't merge what you can't read.)
    let restricted = super::restricted::load_restricted_blobs(repo);
    let known_lost = super::known_loss::load_known_lost_blobs(repo);
    // The empty blob is reconstructible from its hash, so a snapshot missing
    // only that is not torn — it's a repo whose server never shipped the
    // zero-byte object. Materialize it instead of blocking status/commit/merge
    // with `IncompleteBlobData`, which is what left users unable to commit
    // even after recreating the empty file by hand.
    oak_core::ensure_empty_blobs_in_manifest(repo, manifest)?;
    let mut missing_blobs = Vec::new();
    for entry in &manifest.entries {
        if cone.is_some_and(|cone| !cone.covers(&entry.path))
            || restricted.contains(entry.blob_hash.as_str())
            || known_lost.contains(entry.blob_hash.as_str())
        {
            continue;
        }
...
fn declared_root_base(
    ...
    let parent = repo
        .get_branch(branch_a)?
        .and_then(|branch| branch.parent_branch);
    declared_root_base_with_parent(branch_a, parent.as_deref(), branch_b, a_ancestors, repo)
}

fn declared_root_base_with_parent(
    branch_a: &str,
    branch_a_parent: Option<&str>,
    branch_b: &str,
    a_ancestors: &[Hash],
    ...
    if branch_a_parent != Some(branch_b) {
        return Ok(false);
    }
...
#[allow(dead_code)] // local-ref helper retained with the compatibility wrapper
...
#[derive(Serialize)]
struct MergeSuccessJson<'a> {
    schema_version: u32,
    branch: &'a str,
    merged: bool,
    parent: &'static str,
    #[serde(skip_serializing_if = "is_false")]
    recovered_already_closed: bool,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    merge_parent_hash: Option<&'a str>,
}
...
fn validate_merge_receipt(
    ...
    submitted_branch_head: Option<&Hash>,
    reconciliation_commands: &[String],
) -> Result<()> {
    let is_native_hash = |hash: &str| hash.len() == 64 && Hash::from_hex(hash).is_ok();
    let invalid = !is_native_hash(&merged.commit_hash)
        || merged
            .manifest_hash
            .as_deref()
            .is_some_and(|hash| !is_native_hash(hash))
        || merged
            .parent_hash
            .as_deref()
            .is_some_and(|hash| !is_native_hash(hash))
        || merged
            ...
            .as_deref()
            .is_some_and(|hash| !is_native_hash(hash));
    let wrong_merge_parent = merged
        .merge_parent_hash
        .as_deref()
        .and_then(|hash| Hash::from_hex(hash).ok())
        .is_some_and(|acknowledged| {
            submitted_branch_head.is_some_and(|submitted| acknowledged != *submitted)
        });
    if invalid || wrong_merge_parent {
        return Err(crate::http::publication_unconfirmed(
            "merge publication",
            "the server returned a malformed or contradictory publication acknowledgement",
...
fn merge_publication_reconciliation_commands(branch_name: &str) -> Vec<String> {
    vec![super::push::publication_branch_reconciliation_command(
        branch_name,
    )]
}
...
pub(crate) async fn merge_to_main_via_server(
    ...
    let submitted_branch_head = repo.get_branch_head(branch_name)?;
    let reconciliation_commands = merge_publication_reconciliation_commands(branch_name);
    // Merges are CI-gated server-side: while CI for the branch head is
    // running (or after it failed) the merge endpoint answers 412. Without
    // `--wait` that's a self-describing error; with it we poll the CI runs
    // API until the gate resolves and then retry the merge.
    let deadline = wait.map(|timeout| std::time::Instant::now() + timeout);
    let resp = loop {
        let mut req = client.post(&url);
        if let Some(t) = &token {
            req = req.header("authorization", format!("Bearer {t}"));
        }
        let resp = crate::http::send_publication_request(
            req,
            Vec::new(),
            "merge publication",
        ...
        // 412 is an explicit CI-gate refusal, not an ambiguous mutation
        // outcome. Do not render the remote body: proxies and custom servers
        // may include private diagnostics in it.
        let gate_msg = "HTTP 412 Precondition Failed".to_string();
        let Some(deadline) = deadline else {
            return Err(OakError::Server(format!(
                "Merge failed: {gate_msg}\nCI gates this merge — check it with `oak ci status`, or run `oak merge --wait` to merge automatically when CI concludes."
            )));
        };
        let api = super::ci::CiClient {
            remote: remote.clone(),
            owner: owner.clone(),
            repo: repo_name.clone(),
            token: token.clone(),
        };
        wait_for_ci_gate(repo, &api, branch_name, deadline, &gate_msg).await?;
        // Recheck the authoritative merge gate after CI success or a paced
        // NoRuns poll; a no-workflow delivery may have drained meanwhile.
    };

    let status = resp.status();
    if crate::http::publication_status_is_unconfirmed(status) {
        return Err(crate::http::publication_unconfirmed(
            "merge publication",
    ...
    if status == reqwest::StatusCode::CONFLICT {
        let body = crate::http::bounded_publication_body(resp)
            .await
            .unwrap_or_default();
    ...
    if !status.is_success() {
        ...
        {
            return recover_already_closed_branch(lock, repo, root, branch_name, json).await;
        }
        return Err(OakError::Server(format!(
            "Merge publication was rejected with {status}; response body omitted to protect remote diagnostics"
        )));
    }

    let receipt = crate::http::bounded_publication_body(resp)
        .await
        .map_err(|error| {
            crate::http::publication_unconfirmed(
                "merge publication",
                error.to_string(),
                reconciliation_commands.clone(),
            )
        })?;
    let merged: MergeResponse = serde_json::from_slice(&receipt).map_err(|_| {
        crate::http::publication_unconfirmed(
            "merge publication",
            "the server returned a malformed publication receipt",
            reconciliation_commands.clone(),
        )
    })?;
    validate_merge_receipt(
        &merged,
        submitted_branch_head.as_ref(),
        &reconciliation_commands,
    )?;
    ...
    if !json {
        ...
        if let Some(subject) = compact_merge_message_subject(&merged.message) {
            output::detail("message", &subject);
        }
    }
    ...
    let new_branch_name = switch_to_fresh_main_branch_with_output(
        lock,
        repo,
        root,
        branch_name,
    ...
    if json {
        output::print_json(&MergeSuccessJson {
            schema_version: crate::work_state::SCHEMA_VERSION,
            branch: branch_name,
            merged: true,
            parent: oak_core::DEFAULT_BRANCH,
            recovered_already_closed: false,
            new_branch: Some(&new_branch_name),
            message: &merged.message,
            commit_hash: &merged.commit_hash,
            manifest_hash: merged.manifest_hash.as_deref(),
            parent_hash: merged.parent_hash.as_deref(),
            merge_parent_hash: merged.merge_parent_hash.as_deref(),
        })?;
    }
...
fn compact_merge_message_subject(message: &str) -> Option<String> {
    let subject = message.lines().find(|line| !line.trim().is_empty())?.trim();
    const MAX_SUBJECT: usize = 120;
    if subject.len() <= MAX_SUBJECT {
        return Some(subject.to_string());
    }
    let mut end = MAX_SUBJECT;
    while !subject.is_char_boundary(end) {
        end -= 1;
    }
    Some(format!("{}...", &subject[..end]))
}

/// Poll the CI runs API until CI for `branch_name`'s head commit concludes.
///
/// Returns `Ok(())` on a concluded-success run or after a paced no-run poll so
/// the authoritative merge endpoint can be retried. A concluded failure errors immediately, naming
/// `oak ci logs <id>` and `oak ci rerun <id>` as the next steps; passing
/// `deadline` errors with a timeout. Poll failures (a network blip) are
/// warned about and retried — the deadline still bounds the whole wait.
async fn wait_for_ci_gate(
    repo: &dyn Repository,
    api: &super::ci::CiClient,
    branch_name: &str,
    ...
    wait_for_ci_gate_with_interval(repo, api, branch_name, deadline, gate_msg, POLL_INTERVAL).await
}

async fn wait_for_ci_gate_with_interval(
    repo: &dyn Repository,
    api: &super::ci::CiClient,
    branch_name: &str,
    deadline: std::time::Instant,
    ...
    use super::ci::CiGateState;

    // The pre-merge push guarantees the server's branch head equals the
    // local one, so the local head is the commit the merge gate checks.
    let head = repo
        .get_branch_head(branch_name)?
        .ok_or_else(|| OakError::Server(format!("Merge failed: {gate_msg}")))?;
    let head = head.to_string();
    let short = &head[..head.len().min(12)];

    let started = std::time::Instant::now();
    loop {
        let mut retry_merge_after_poll = false;
        match api.latest_run_for_commit(&head).await {
            Ok(Some(run)) => match run.gate_state() {
                CiGateState::Success => {
                    output::success(&format!(
                        "CI passed for {branch_name}@{short} (run #{}) — merging.",
                        run.id
                    ));
                    return Ok(());
                }
                CiGateState::Failure => {
                    let detail = run
                        ...
                        .unwrap_or_default();
                    return Err(OakError::Server(format!(
                        "CI concluded failure for {branch_name}@{short} (run #{id}){detail}.\nInspect it with `oak ci logs {id}`; if it failed for infra reasons, `oak ci rerun {id}` and then `oak merge --wait` again.",
                        id = run.id,
                    )));
                }
                CiGateState::Running => {
                    output::info(&format!(
                        "CI run #{} is {} for {branch_name}@{short} — waiting ({}s elapsed)",
                        run.id,
                        run.status,
            ...
            Ok(None) => {
                output::info(&format!(
                    "No CI run visible for {branch_name}@{short} — retrying the merge gate after a paced poll ({}s elapsed)",
                    started.elapsed().as_secs(),
                ));
                retry_merge_after_poll = true;
            }
            Err(e) => {
                output::warning(&format!("Couldn't poll CI runs ({e}) — retrying"));
            }
        }

        if std::time::Instant::now() + poll_interval > deadline {
            return Err(OakError::Server(format!(
                "Timed out waiting for CI on {branch_name}@{short} ({gate_msg}).\nCheck `oak ci status`, then retry `oak merge --wait`."
            )));
        }
        tokio::time::sleep(poll_interval).await;
        if retry_merge_after_poll {
            return Ok(());
        }
    }
}
...
fn switch_to_fresh_main_branch(
    ...
    switch_to_fresh_main_branch_with_output(lock, repo, root, branch_name, new_main_head, true)
}

fn switch_to_fresh_main_branch_with_output(
    lock: &WorkdirLock,
    repo: &dyn Repository,
    root: &Path,
    branch_name: &str,
...
async fn recover_already_closed_branch(
    ...
    if current_branch.as_deref() != Some(branch_name) {
        ...
        if json {
            output::print_json(&MergeSuccessJson {
                ...
                merged: false,
                parent: oak_core::DEFAULT_BRANCH,
                recovered_already_closed: true,
                new_branch: None,
                message: "Branch is already merged onto main.",
                commit_hash: "",
                manifest_hash: None,
                parent_hash: None,
        ...
        if let Some(reconciled) = apply_remote_merge_reconcile(lock, repo, root, true, plan)? {
            if json {
                output::print_json(&MergeSuccessJson {
                    ...
                    parent: oak_core::DEFAULT_BRANCH,
                    recovered_already_closed: true,
                    new_branch: Some(&reconciled.new_branch),
                    message: "Branch was already merged onto main.",
                    commit_hash: "",
                    manifest_hash: None,
                    parent_hash: None,
                    merge_parent_hash: None,
    ...
    let new_branch_name =
        switch_to_fresh_main_branch_with_output(lock, repo, root, branch_name, None, !json)?;
    if json {
        output::print_json(&MergeSuccessJson {
            schema_version: crate::work_state::SCHEMA_VERSION,
            branch: branch_name,
            ...
            recovered_already_closed: true,
            new_branch: Some(&new_branch_name),
            message: "Branch is already closed on the server.",
            commit_hash: "",
            manifest_hash: None,
            parent_hash: None,
            merge_parent_hash: None,
...
/// Render a server-side squash-merge conflict (HTTP 409) with its bounded
/// conflict count and safe recovery advice. The server lists paths in its
/// error message ("… on both branches: a, b"); a structured
/// `conflict_paths` field wins when present. Remote path strings are not
/// echoed because this response is not independently verified against local
/// content.
/// Anything that doesn't match the server's envelope (a proxy error page,
/// an older server) falls back to status-only rendering so arbitrary remote
/// text cannot disclose private diagnostics.
fn server_merge_conflict_error(branch_name: &str, body: &str) -> OakError {
    ...
    let Ok(parsed) = serde_json::from_str::<ConflictBody>(body.trim()) else {
        return OakError::Server("Merge failed: HTTP 409 Conflict".to_string());
    };
...
mod tests {
    ...
    async fn no_visible_ci_run_yields_to_the_authoritative_merge_retry() {
        ...
        repo.set_branch_head("feature", &commit.hash).unwrap();
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let api = super::super::ci::CiClient {
            remote: server.uri(),
            owner: "oak".to_string(),
            repo: "oak".to_string(),
            token: None,
        ...
        wait_for_ci_gate_with_interval(
            &repo,
            &api,
            "feature",
        ...
        .await
        .expect("NoRuns must return control so the merge endpoint is retried");
    }

    #[tokio::test(flavor = "current_thread")]
    async fn merge_wait_retries_after_no_workflow_delivery_drains() {
        let temp = tempfile::TempDir::new().unwrap();
        ...
        let feature = oak_core::Commit::new(
            "feature".to_string(),
        ...
        let responder_calls = merge_calls.clone();
        let merged_head = "cd".repeat(32);
        let response_head = merged_head.clone();
        let response_manifest = manifest.to_string();
        let response_parent = main.hash.to_string();
        let response_feature = feature.hash.to_string();
        Mock::given(method("POST"))
            .and(path("/api/oak/oak/branches/feature/merge"))
            .respond_with(move |_request: &wiremock::Request| {
                if responder_calls.fetch_add(1, Ordering::SeqCst) == 0 {
                    ResponseTemplate::new(412).set_body_string("CI delivery is still in flight")
                } else {
                    ResponseTemplate::new(200).set_body_json(serde_json::json!({
                        "message": "feature",
                        "commit_hash": response_head,
                        "manifest_hash": response_manifest,
                        "parent_hash": response_parent,
                        "merge_parent_hash": response_feature
                    }))
                }
            })
            .expect(2)
        ...
        let lock = crate::workdir_lock::WorkdirLock::acquire(&oak_dir).unwrap();
        merge_to_main_via_server(
            &lock,
            &repo,
            temp.path(),
            "feature",
        ...
        .await
        .expect("NoRuns must yield to a second authoritative merge POST");

        assert_eq!(merge_calls.load(Ordering::SeqCst), 2);
        assert_eq!(
            repo.get_branch_head("main").unwrap(),
            Some(Hash(merged_head))
        );
    }
    ...
    #[test]
    fn server_conflict_without_paths_omits_server_message() {
        let body = r#"{"error":"Merge conflict: branch is read-only"}"#;
        match server_merge_conflict_error("feature", body) {
            OakError::Server(msg) => {
                assert_eq!(msg, "Merge failed: HTTP 409 Conflict")
            }
    ...
    #[test]
    fn malformed_merge_acknowledgement_is_unconfirmed_without_echoing_value() {
        let marker = "SYNTHETIC_PRIVATE_ERROR_MARKER";
        let merged = MergeResponse {
            message: "merged".to_string(),
            commit_hash: marker.to_string(),
            manifest_hash: None,
            parent_hash: None,
            merge_parent_hash: None,
        };
        let error = validate_merge_receipt(
            &merged,
            Some(&Hash("a".repeat(64))),
            &["oak branch show feature --remote --json".to_string()],
        )
        .expect_err("malformed successful acknowledgement must remain unconfirmed");
        assert!(matches!(error, OakError::PublicationUnconfirmed { .. }));
        assert!(!error.to_string().contains(marker));
    }

    #[test]
    fn merge_reconciliation_shell_quotes_valid_unsafe_branch_names() {
        assert_eq!(
            merge_publication_reconciliation_commands("qa;:"),
            ["oak branch show --remote --json -- 'qa;:'"]
        );
    }

    #[test]
    fn merge_acknowledgement_must_name_the_submitted_branch_head_when_present() {
        let submitted = Hash("a".repeat(64));
        let merged = MergeResponse {
            message: "merged".to_string(),
            commit_hash: "b".repeat(64),
            manifest_hash: None,
            parent_hash: None,
            merge_parent_hash: Some("c".repeat(64)),
        };
        assert!(matches!(
            validate_merge_receipt(
                &merged,
            ...
            Err(OakError::PublicationUnconfirmed { .. })
        ));
    }

    #[test]
    fn compact_merge_message_subject_uses_first_line_and_bounds_length() {
        let long = format!("{}\n\nbody", "x".repeat(160));
        let subject = compact_merge_message_subject(&long).unwrap();
        assert!(subject.ends_with("..."));
        assert!(subject.len() <= 123);
        assert_eq!(
            compact_merge_message_subject("\n  short subject  \nbody").as_deref(),
            Some("short subject")
        );
        assert!(compact_merge_message_subject(" \n\t").is_none());
    }
    ...
    #[test]
    fn lazy_cache_lca_tolerates_missing_blob_content() {
        // A mount's cache holds the base manifest's structure (trees) but not
        // its blob content — blobs hydrate on demand. The strict resolver must
        // refuse (a working-tree merge would read those blobs), while the
        // lazy-cache resolver returns the manifest for the mount merge
        // planner, which fetches only the blobs it needs.
        let temp = tempfile::TempDir::new().unwrap();
        let repo = oak_core::SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        ...
        // Base manifest references a blob whose content was never stored.
        let unhydrated = ManifestEntry {
            path: "lazy.txt".to_string(),
            blob_hash: Hash("ab".repeat(32)),
            mode: oak_core::FileMode::Regular,
        };
        let base_manifest_hash = repo.put_manifest(vec![unhydrated.clone()]).unwrap();
        let base_head = repo
            .put_commit(
                "main".to_string(),
                None,
                None,
                base_manifest_hash.clone(),
                "tester".to_string(),
                None,
                chrono::Utc::now(),
                vec![],
        ...
        let (feat_entry, _) = manifest_entry(&repo, "feat.txt", "feat\n");
        let feat_manifest_hash = repo.put_manifest(vec![unhydrated, feat_entry]).unwrap();
        let feat_head = repo
            .put_commit(
                "feat".to_string(),
            ...
            .unwrap();
        repo.set_branch_head("feat", &feat_head).unwrap();

        let err =
            find_lca_manifest_with_parent(&repo, "feat", "main", Some(&base_head)).unwrap_err();
        assert!(
            matches!(err, OakError::IncompleteBlobData { .. }),
            "strict resolver should refuse missing blob content, got {err}"
        );

        let manifest =
            find_lca_manifest_for_lazy_cache(&repo, "feat", "main", Some(&base_head)).unwrap();
        assert_eq!(manifest.hash, base_manifest_hash);
        assert_eq!(manifest.entries.len(), 1);
        assert_eq!(manifest.entries[0].path, "lazy.txt");
    }
    ...
    #[cfg(unix)]
    #[test]
    fn postmerge_equal_tree_refresh_preserves_regular_files_and_rotates_branch() {
        use std::os::unix::fs::MetadataExt;
        let temp = tempfile::TempDir::new().unwrap();
        let root = temp.path().join("work");
        fs::create_dir_all(&root).unwrap();
        let (repo, main_head, feature_head, manifest) = seed_main_and_feature(temp.path());
        let lock = crate::workdir_lock::WorkdirLock::acquire(temp.path()).unwrap();
        crate::commands::reset::reset_to_manifest(&lock, &root, &repo, &manifest).unwrap();
        let before = fs::metadata(root.join("README.md")).unwrap();
        let merged = repo
            .put_commit(
                "main".into(),
                Some(main_head),
            ...
            .unwrap();
        repo.set_branch_head("main", &merged).unwrap();
        let fresh =
            switch_to_fresh_main_branch(&lock, &repo, &root, "feature", Some(merged.clone()))
                .unwrap();
        let after = fs::metadata(root.join("README.md")).unwrap();
        assert_eq!(
            after.ino(),
            before.ino(),
            "equal-tree postmerge refresh must preserve the existing source file"
        );
        assert_eq!(after.modified().unwrap(), before.modified().unwrap());
        assert_eq!(fs::read(root.join("README.md")).unwrap(), b"base\n");
        assert_eq!(fs::read(root.join("feature.txt")).unwrap(), b"feature\n");
        assert_eq!(repo.get_current_branch_name().unwrap(), Some(fresh.clone()));
        assert_eq!(repo.get_branch_head(&fresh).unwrap(), Some(merged));
    ...
    #[test]
    fn switch_to_fresh_main_branch_can_suppress_human_output_for_json_merge() {
        let temp = tempfile::TempDir::new().unwrap();
        let root = temp.path().join("work");
        fs::create_dir_all(&root).unwrap();
        let (repo, main_head, _feature_head, feature_manifest) = seed_main_and_feature(temp.path());
        ...
        output::begin_capture();
        let new_branch = switch_to_fresh_main_branch_with_output(
            &lock,
            &repo,
            &root,
            "feature",
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn recover_already_closed_branch_json_is_single_json_document() {
        let temp = tempfile::TempDir::new().unwrap();
        let root = temp.path().join("work");
        fs::create_dir_all(&root).unwrap();
        let (repo, _main_head, _feature_head, feature_manifest) =
            seed_main_and_feature(temp.path());

        let lock = crate::workdir_lock::WorkdirLock::acquire(temp.path()).unwrap();
        crate::commands::reset::reset_to_manifest(&lock, &root, &repo, &feature_manifest).unwrap();

        output::begin_capture();
        recover_already_closed_branch(&lock, &repo, &root, "feature", true)
            .await
            .unwrap();
        let captured = output::end_capture();

        let json: serde_json::Value =
            serde_json::from_str(&captured).expect("captured output should be JSON only");
        assert_eq!(json["schema_version"], crate::work_state::SCHEMA_VERSION);
        assert_eq!(json["branch"], "feature");
        assert_eq!(json["merged"], false);
        assert_eq!(json["recovered_already_closed"], true);
        assert!(json["new_branch"].as_str().is_some());
    }
