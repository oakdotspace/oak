...
pub(crate) async fn merge_to_main_via_server(
    ...
    let resp = loop {
        ...
        // CI concluded success — loop around and retry the merge.
    };
...
///
/// Returns `Ok(())` only on a concluded-success run (the caller then retries
/// the merge). A concluded failure errors immediately, naming
...
async fn wait_for_ci_gate(
    ...
    loop {
        match api.latest_run_for_commit(&head).await {
            ...
            Ok(None) => {
                output::info(&format!(
                    "No CI run visible yet for {branch_name}@{short} — waiting ({}s elapsed)",
                    started.elapsed().as_secs(),
                ));
            }
            Err(e) => {
