...
pub fn read_repo_identity(repo: &dyn Repository) -> Result<(String, String)> {
    let owner = repo.get_metadata(MetadataKey::RepoOwner)?.ok_or_else(|| {
        OakError::Config("This repository is not linked to an Oak remote. Run `oak push --repo <org>/<repo>` to link and publish it, or re-clone with `oak clone <org>/<repo>`.".to_string())
    })?;
    let name = repo.get_metadata(MetadataKey::RepoName)?.ok_or_else(|| {
        OakError::Config("This repository is missing remote repo metadata. Run `oak push --repo <org>/<repo>` to repair and publish it, or re-clone with `oak clone <org>/<repo>`.".to_string())
    })?;
    Ok((owner, name))
}
...
#[cfg(test)]
mod tests {
    use super::{branch_web_url, repo_web_url};
