...
use oak_core::{FileMode, Hash, IgnorePatterns, Manifest, OakError, Result};
...
/// Load the repository's ignore rules from the mounted manifest. Prefers
...
fn load_mount_ignore(
    ...
    let entry = manifest
        .entries
        .iter()
        .find(|e| e.path == ".oakignore")
        .or_else(|| manifest.entries.iter().find(|e| e.path == ".gitignore"));
    let Some(entry) = entry else {
        return IgnorePatterns::from_patterns("");
    };

    // Hydrate the ignore-file blob if it isn't already in the local cache.
    if cache.get_blob(&entry.blob_hash).ok().flatten().is_none() {
        ...
        if let Err(e) = fetch {
            tracing::warn!(?e, path = %entry.path, "mount: could not hydrate ignore file");
            return IgnorePatterns::from_patterns("");
        }
    }
...
impl MountCore {
    ...
    pub fn new(
        ...
        let deleted: HashSet<&str> = overlay.deletions.iter().map(|s| s.as_str()).collect();
        // Apply renames (old path → new path) so the path we display is the
        // post-rename path. Simple flat rename; nested renames within a
        // renamed dir aren't tracked here.
