...
impl MountCore {
    ...
    fn node_attr(&self, node: &Node, overlay: &OverlayMeta) -> NodeAttr {
        let mtime = self.mtime_for_node(node, overlay);
        match &node.kind {
    ...
    fn mtime_for_node(&self, node: &Node, overlay: &OverlayMeta) -> SystemTime {
        if let NodeKind::File { .. } = &node.kind {
            let overlay_file = overlay
                .dirty
                .get(&node.path)
                .filter(|dirty| !dirty.in_place && !dirty.overlay_file.is_empty())
                .map(|dirty| dirty.overlay_file.clone())
                .unwrap_or_else(|| state::overlay_filename_for(&node.path));
            let overlay_path = self.overlay_dir.join(overlay_file);
    ...
    fn overlay_file_for_path(&self, path: &str) -> String {
        ...
        inner
            ...
            .filter(|dirty| !dirty.in_place && !dirty.overlay_file.is_empty())
            .map(|dirty| dirty.overlay_file.clone())
            .unwrap_or_else(|| state::overlay_filename_for(path))
    }

    fn overlay_path_for(&self, path: &str) -> PathBuf {
        self.overlay_dir.join(self.overlay_file_for_path(path))
    }
    ...
    pub fn attr(&self, ino: u64) -> Option<NodeAttr> {
        ...
        inner
            .nodes
            .get(&ino)
            .map(|n| self.node_attr(n, &inner.overlay))
    }
    ...
    pub fn read_full(&self, ino: u64) -> std::io::Result<Option<Vec<u8>>> {
        ...
        // 1. Overlay
        let overlay_path = self.overlay_path_for(&path);
    ...
    pub fn materialize_to_overlay(&self, ino: u64) -> std::io::Result<PathBuf> {
        ...
        let overlay_name = self.overlay_file_for_path(&path);
    ...
    pub fn unlink(&self, parent: u64, name: &OsStr) -> std::result::Result<(), i32> {
        let (path, removed_overlay_file) = {
            ...
            let removed_overlay_file = inner
                .overlay
                .dirty
                .remove(&child.path)
                .filter(|dirty| !dirty.in_place && !dirty.overlay_file.is_empty())
                .map(|dirty| dirty.overlay_file);
            if was_in_base && !inner.overlay.deletions.contains(&child.path) {
                inner.overlay.deletions.push(child.path.clone());
            }
            (child.path, removed_overlay_file)
        };
        let overlay_path = self
            .overlay_dir
            .join(removed_overlay_file.unwrap_or_else(|| state::overlay_filename_for(&path)));
        if overlay_path.exists() {
    ...
    pub fn rename(
        ...
        let (old_path, new_path, child_ino) = {
            ...
            if let Some(de) = inner.overlay.dirty.remove(&old_path) {
                let new_overlay = if de.in_place {
                    de.overlay_file.clone()
                } else {
                    state::overlay_filename_for(&new_path)
                };
                if !de.in_place && !de.overlay_file.is_empty() {
    ...
    /// startup size map, but always fall back to the local cache when a hash is
    /// missing — including when the whole startup map is empty (a cold lazy
    /// mount that skipped all-repo blob metadata prefetch). `blob_size` reads
    /// the chunk-length index (or the blob record) and returns `None` only on a
    /// genuine cache miss, so a wide-directory listing degrades to a cheap
    /// indexed probe per file rather than the old hard `size 0`.
...
fn io_err(e: OakError) -> std::io::Error {
    // Keep permission-withheld content distinguishable through the io::Error
    // seam so it can surface as EACCES instead of a generic I/O fault.
    if matches!(e, OakError::RestrictedContent(_)) {
...
/// Errno for a failed blob hydration: content the server withheld under
/// path-based permissions reads as "permission denied" (the file IS
/// restricted — ask an org admin for access), everything else as an I/O
/// fault. Backends log the full error alongside this.
pub fn hydrate_errno(e: &OakError) -> i32 {
    match e {
        OakError::RestrictedContent(_) => libc::EACCES,
        _ => libc::EIO,
    }
}
...
mod tests {
    ...
    fn directory_hydration_tolerates_deferred_base_sizes() {
        ...
        assert_eq!(
            foo.size, 5,
            "an empty startup size map falls back to the cached blob size \
             rather than statting the file as size 0"
        );
        assert_eq!(
    ...
    #[test]
    fn dirty_overlay_paths_do_not_alias_separator_lookalikes() {
        let (core, _cache, _rt, _dir) = build_core(&[("a/b", b"------"), ("a__b", b"------")]);
        let nested = ino_of(&core, "a/b").expect("nested file");
        let flat = ino_of(&core, "a__b").expect("flat file");

        core.write(nested, 0, b"NESTED").expect("write nested");
        core.write(flat, 0, b"FLAT!!").expect("write flat");

        assert_eq!(core.read_full(nested).unwrap().unwrap(), b"NESTED");
        assert_eq!(core.read_full(flat).unwrap().unwrap(), b"FLAT!!");

        let inner = core.inner.lock().unwrap();
        let nested_overlay = &inner.overlay.dirty.get("a/b").unwrap().overlay_file;
        let flat_overlay = &inner.overlay.dirty.get("a__b").unwrap().overlay_file;
        assert_ne!(
            nested_overlay, flat_overlay,
            "overlay filenames must be injective for distinct logical paths"
        );
    }
