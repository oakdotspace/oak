...
fn token_for_mount(cfg: &MountConfig, state_dir: &Path) -> Option<String> {
    match SqliteRepository::open_relaxed(&cache_db_path(state_dir)) {
        Ok(cache) => super::credentials::effective_token_for_repository(&cfg.remote_url, &cache),
        Err(_) => super::credentials::effective_token(&cfg.remote_url, None),
    }
}

fn token_for_mount_remote(remote: &str, state_dir: &Path) -> Option<String> {
    match SqliteRepository::open_relaxed(&cache_db_path(state_dir)) {
        Ok(cache) => super::credentials::effective_token_for_repository(remote, &cache),
        Err(_) => super::credentials::effective_token(remote, None),
    }
}
...
pub fn diff(
    ...
    if !print {
        for line in super::diff::render_worktree_stat_with_limit(
            &cache,
            &cfg.mount_point,
            &head_files,
            &changes,
...
pub fn agent_state_json(dest: &Path, refresh: bool, compact: bool) -> Result<()> {
    tokio::runtime::Runtime::new()
        .map_err(|error| OakError::Server(format!("could not start agent-state runtime: {error}")))?
        .block_on(agent_state_json_paged(dest, refresh, compact, None))
}

pub async fn agent_state_json_paged(
    dest: &Path,
    refresh: bool,
    compact: bool,
    publication_after: Option<&str>,
) -> Result<()> {
    let state = crate::work_state::mount_agent_state_json(dest, refresh, publication_after).await?;
    if compact {
...
fn update_description_local(cfg: &MountConfig, state_dir: &Path, description: &str) -> Result<()> {
    ...
    // just a row update.
    cache.edit_branch_description(&cfg.virtual_branch, description)?;
    output::success(&format!(
    ...
    if unpushed > 0 {
        if let Err(e) = push(&dest).await {
            ...
            } = &e
            {
                ...
                commands.push("oak mount status --json --compact".to_string());
                return Err(mount_finish_phase_failed_owned(
                    "push",
                    &completed_phases,
                    &["push", "mount_end"],
                    format!(
                        "mount finish could not confirm whether {unpushed} commit(s) on '{}' were published: {e}",
                        cfg.virtual_branch
                    ),
                    None,
                    commands,
        ...
        sync_description_to_server(&cfg, &state_dir)
            ...
            .map_err(|e| {
                ...
                } = &e
                {
                    let mut commands = reconciliation_commands.clone();
                    commands.push("oak mount status --json --compact".to_string());
                    mount_finish_phase_failed_owned(
                        "metadata_sync",
                        &completed_phases,
                        &["metadata_sync", "mount_end"],
                        format!(
                            "description saved locally, but mount finish could not confirm whether it synced to the server: {e}"
                        ),
                        None,
                        commands,
                    )
...
fn mount_finish_phase_failed_owned(
    phase: impl Into<String>,
    completed_phases: &[String],
    pending_phases: &[&str],
    message: impl Into<String>,
...
pub fn end(dest: &Path, force: bool) -> Result<()> {
    ...
    if !force && crate::publication_state::AttemptStore::mount(&state_dir).has_records()? {
        return Err(OakError::Server(format!(
            "refusing to end mount '{}': it has an unresolved or retained publication record. Run `oak agent state --refresh --json --compact` inside the mount, then explicitly acknowledge the exact operation before teardown",
            dest.display()
        )));
    }
