...
use oak_core::{path_in_any_prefix, FileMode, Manifest, ManifestEntry, OakError, Result};
use oak_core::{Repository, SqliteRepository};
use tokio::runtime::Handle;
use windows::core::{GUID, HRESULT, PCWSTR};
use windows::Win32::Foundation::{BOOLEAN, ERROR_FILE_NOT_FOUND, ERROR_INSUFFICIENT_BUFFER, S_OK};
...
struct ProjFsState {
    ...
    token: Option<String>,
    /// Optional project-scope filter — when non-empty, paths outside any
    /// prefix are not surfaced to the user during enumeration and produce
    /// ERROR_FILE_NOT_FOUND on direct lookup.
    #[allow(dead_code)] // checked via `inner.files`/`inner.dirs` populated under the filter
    prefixes: Vec<String>,
...
impl ProjFsMount {
    ...
    pub fn start(
        ...
        // Build the in-memory tree from the manifest, applying the scope.
        let mut inner = build_inner(manifest, sizes, &prefixes)?;
...
fn build_inner(
    ...
    for entry in &manifest.entries {
        // Scope filter: skip whole entries whose path is out of scope.
        if !prefixes.is_empty() && !path_in_any_prefix(prefixes, &entry.path) {
            continue;
        }
