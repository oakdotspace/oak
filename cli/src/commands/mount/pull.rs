...
use super::{build_committed_manifest, config_for_dest, split_overlay, token_for_mount};
use crate::output;
...
async fn fresh_pull(cfg: &MountConfig, state_dir: &Path) -> Result<()> {
    ...
    // Resolve the trunk's current head on the server and fetch commit history
    // into the cache until the branch's and the trunk's ancestries connect, so
    // the merge-base walk works.
    ...
    fetch_histories_until_connected(&cache, cfg, token.as_deref(), &branch_head, &parent_head)
        .await?;

    let branch_manifest = manifest_for(&cache, &branch_head)?;
    let parent_manifest = manifest_for(&cache, &parent_head)?;
    // Lazy-cache LCA: the mount holds blob metadata without content, and
    // `plan_merge` below fetches exactly the blobs the merge needs.
    let base_manifest = crate::commands::merge::find_lca_manifest_for_lazy_cache(
        &cache,
        &cfg.virtual_branch,
        &parent_name,
        Some(&parent_head),
...
/// all the LCA walk needs. If an ancestor can't be fetched we stop gracefully:
/// the merge base may then be missed (over-reporting conflicts) but the pull
/// still runs instead of hard-failing.
///
/// Two synchronized walks — one from the virtual branch's head, one from the
/// parent's — descend until they share a commit, which is exactly when the
/// LCA search in `resolve_merge_base` becomes locally decidable. Commits
/// already in the cache advance their walk for free; each layer fetches both
/// walks' missing commits in a single `/commits/info` round trip. A fresh
/// trunk-based mount costs the same walk as before (trunk head down to the
/// mount's fork point, where the virtual branch's all-local chain has already
/// been); an existing-branch mount (`oak mount --branch`) additionally pages
/// in the branch's own commit metadata down to the fork point.
async fn fetch_histories_until_connected(
    cache: &SqliteRepository,
    cfg: &MountConfig,
    token: Option<&str>,
    branch_head: &Hash,
    ...
    // Which walk(s) have reached a commit: bit 0 = branch, bit 1 = parent.
    // Both bits set means the walks connected.
    let mut seen: HashMap<String, u8> = HashMap::new();
    seen.insert(branch_head.0.clone(), 1);
    let joined = {
        let entry = seen.entry(parent_head.0.clone()).or_insert(0);
        *entry |= 2;
        *entry == 3
    };
    if joined {
    ...
    let mut frontiers = [Some(branch_head.clone()), Some(parent_head.clone())];
    ...
    for _ in 0..10_000 {
        let missing: Vec<Hash> = {
            let mut v = Vec::new();
            for h in frontiers.iter().flatten() {
        ...
        if !missing.is_empty() {
            ...
            .await
            {
                // Couldn't fetch this layer — stop walking and merge with
                // whatever history we already have rather than failing the
                // pull outright.
                output::info(&format!("  (stopped fetching history early: {e})"));
                return Ok(());
            }
        }

        let mut advanced = false;
        for (walk, slot) in frontiers.iter_mut().enumerate() {
            let Some(h) = slot.clone() else { continue };
            let Some(commit) = cache.get_commit(&h)? else {
                // The fetch above couldn't materialize it — this walk is done.
                *slot = None;
                continue;
            };
            let Some(parent) = commit.parent_hash else {
                // Chain root.
                *slot = None;
                continue;
            };
            let entry = seen.entry(parent.0.clone()).or_insert(0);
            *entry |= 1 << walk;
            if *entry == 3 {
                return Ok(());
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn history_walk_connects_local_chains_without_network() {
        // Branch chain b1 → m1, parent chain m2 → m1, all local. The walk must
        // connect at m1 without touching the (unreachable) remote, after which
        // the lazy LCA resolves to m1's manifest.
        ...
        let base_manifest = repo
            .put_manifest(vec![entry("f.txt", store(&repo, "base\n"))])
            .unwrap();
        let m1 = repo
            .put_commit(
                "main".to_string(),
            ...
            .unwrap();
        let m2_manifest = repo
            .put_manifest(vec![entry("f.txt", store(&repo, "trunk\n"))])
            .unwrap();
        let m2 = repo
            .put_commit(
                "main".to_string(),
            ...
            .unwrap();
        let b1_manifest = repo
            .put_manifest(vec![entry("f.txt", store(&repo, "branch\n"))])
            .unwrap();
        let b1 = repo
            .put_commit(
                "feat--abcd1234".to_string(),
            ...
            .unwrap();
        repo.set_branch_head("main", &m2).unwrap();
        repo.set_branch_head("feat--abcd1234", &b1).unwrap();

        fetch_histories_until_connected(&repo, &test_cfg(), None, &b1, &m2)
            .await
            .unwrap();

        let base = crate::commands::merge::find_lca_manifest_for_lazy_cache(
            &repo,
            "feat--abcd1234",
            "main",
            Some(&m2),
        ...
        .unwrap();
        assert_eq!(base.hash, base_manifest);
    }

    #[tokio::test]
    async fn history_walk_degrades_gracefully_when_fetch_fails() {
        // A frontier commit that isn't local and can't be fetched (remote
        // unreachable) must stop the walk without failing the pull.
        let tmp = TempDir::new().unwrap();
        let repo = open_cache(&tmp);
        repo.store_branch(&oak_core::Branch::new(
            "feat--abcd1234".to_string(),
        ...
        .unwrap();
        let manifest = repo
            .put_manifest(vec![entry("f.txt", store(&repo, "x\n"))])
            .unwrap();
        let head = repo
            .put_commit(
                "feat--abcd1234".to_string(),
                Some(Hash("ab".repeat(32))),
                None,
                manifest,
                "t".to_string(),
                None,
            ...
            .unwrap();
        repo.set_branch_head("feat--abcd1234", &head).unwrap();

        let far = Hash("cd".repeat(32));
        fetch_histories_until_connected(&repo, &test_cfg(), None, &head, &far)
            .await
            .unwrap();
    }
