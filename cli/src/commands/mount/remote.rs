...
/// Resolve `branch` on the remote to its full branch record (head, parent,
/// description, status). Errors when the branch doesn't exist.
pub async fn resolve_branch_info(
    remote_url: &str,
    owner: &str,
    repo: &str,
    branch_name: &str,
    ...
    let client = crate::http::api_client();
    let url = format!(
        "{remote_url}/api/{owner}/{repo}/branches/{}",
        crate::commands::branch_api_segment(branch_name)
    );
    ...
    if resp.status().as_u16() == 404 {
        return Err(OakError::Server(format!(
            "branch '{branch_name}' not found on {owner}/{repo} — list remote branches with \
             `oak branch list --remote`"
        )));
    }
    if !resp.status().is_success() {
    ...
    resp.json().await.map_err(|e| OakError::Http(e.to_string()))
}
...
pub async fn resolve_branch_head(
    ...
    if let Some(branch_name) = branch {
        let info = resolve_branch_info(remote_url, owner, repo, branch_name, token).await?;
    ...
    // The repo head and the branch list are independent reads — issue them
    // concurrently so this phase costs one round trip instead of two. The
    // branch list only refines which branch name owns the head.
    let repo_url = format!("{remote_url}/api/{owner}/{repo}");
    let list_url = format!("{remote_url}/api/{owner}/{repo}/branches");
    let (repo_resp, list_resp) = tokio::join!(
        auth(client.get(&repo_url), token).send(),
        auth(client.get(&list_url), token).send(),
    );

    let resp = repo_resp.map_err(|e| OakError::Http(e.to_string()))?;
    if !resp.status().is_success() {
        let body = resp.text().await.unwrap_or_default();
        return Err(OakError::Server(body));
    }
    ...
    let resp = list_resp.map_err(|e| OakError::Http(e.to_string()))?;
