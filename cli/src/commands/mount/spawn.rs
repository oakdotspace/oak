...
/// and block until it serves a live mountpoint at `dest`. `branch` mounts an
/// existing remote branch instead of a fresh trunk-based virtual branch.
///
...
pub fn spawn_detached(remote: &str, spec: &str, dest: &Path, branch: Option<&str>) -> Result<()> {
...
/// Windows has no signals; ask the kernel to terminate the process directly.
/// Best-effort — a vanished or inaccessible pid just no-ops.
#[cfg(windows)]
pub(crate) fn terminate_pid(pid: u32) {
    use windows::Win32::Foundation::{CloseHandle, FALSE};
    use windows::Win32::System::Threading::{OpenProcess, TerminateProcess, PROCESS_TERMINATE};
    // SAFETY: we close every handle we open and pass valid arguments.
    unsafe {
...
/// Detach the spawned mount on Windows: DETACHED_PROCESS drops the inherited
/// console so the daemon outlives the launching terminal, and
/// CREATE_NEW_PROCESS_GROUP keeps a parent Ctrl-C/Ctrl-Break from propagating
/// to it.
#[cfg(windows)]
fn detach(cmd: &mut std::process::Command) {
