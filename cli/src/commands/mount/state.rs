...
pub struct MountConfig {
    ...
    /// Set when the mount serves an *existing* remote branch
    /// (`oak mount --branch <name>`): the branch's name, which is then also
    /// `virtual_branch`. `base_branch`/`base_commit` hold the branch's parent
    /// (the trunk) and the branch head the mount started from. `None` for the
    /// normal fresh-task mount whose virtual branch is brand new.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mounted_branch: Option<String>,
}
...
/// Probe process existence on Windows through the shared conservative helper.
/// Access denied and unclassified failures remain alive-for-safety so teardown
/// and publication retirement cannot erase state owned by an unknown process.
#[cfg(windows)]
pub fn pid_alive(pid: u32) -> bool {
    !matches!(
...
mod tests {
    ...
    fn config_roundtrip() {
        ...
        assert_eq!(loaded.mounted_branch, None);

        // Existing-branch mounts round-trip the branch name.
        let cfg = MountConfig {
            virtual_branch: "zach-b391f3".to_string(),
            mounted_branch: Some("zach-b391f3".to_string()),
            ..cfg
        };
        save_config(dir.path(), &cfg).unwrap();
        let loaded = load_config(dir.path()).unwrap();
        assert_eq!(loaded.mounted_branch.as_deref(), Some("zach-b391f3"));
    }
