...
pub fn worktree_create(remote: &str, spec: &str) -> Result<()> {
    ...
    spawn_detached(remote, spec, &dest, None)?;
    output::print_line(&format!("{}", dest.display()));
    Ok(())
}
...
pub fn worktree_remove_at(dest: &Path) -> Result<()> {
    ...
    let unpushed_known = super::unpushed_commits_checked(&state_dir);
    let unpushed = unpushed_known.unwrap_or(0);
    // Unreadable cache/overlay state means we can't prove the mount is safe to
    // remove — leave it in place rather than risk discarding unpushed work.
    let state_unknown =
        unpushed_known.is_none() || super::state::overlay_state_unreadable(&state_dir);
    if dirty || unpushed > 0 || state_unknown {
        output::warning(&format!(
            "leaving mount {} in place: {} (run `oak push` inside it, then `oak mount end`; \
             or `oak mount end --force` to discard)",
            dest.display(),
            if state_unknown {
                "its local state is unreadable (can't rule out unpushed work)".to_string()
            } else if dirty {
