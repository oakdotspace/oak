...
use oak_core::{
    ...
    Manifest, MetadataKey, OakError, Result,
};
...
pub async fn fetch_and_store_blobs(
    ...
    if !all_missing_hashes.is_empty() {
        ...
        // Drain the per-chunk GETs (large / fallback chunks).
        ...
        for result in raw_results {
            ...
            let chunk_data = oak_core::chunk_decode(chunk_data, &chunk_hash);
        ...
        // Fetch the small chunks in bulk through the CDN Worker batch endpoint.
            ...
            if !batch_hashes.is_empty() {
                ...
                while let Some(res) = batch_set.join_next().await {
                    ...
                    for (hash, data) in decoded {
                        let chunk_hash = Hash(hash);
                        let data = oak_core::chunk_decode(data, &chunk_hash);
                        bytes_since_flush += data.len() as u64;
