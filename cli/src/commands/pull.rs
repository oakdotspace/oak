...
pub async fn pull_async(
    ...
    if !pull_resp.renames.is_empty() {
        ...
        for rn in &pull_resp.renames {
            ...
            if repo.get_branch(&rn.new_name)?.is_some()
                || repo.get_branch_head(&rn.new_name)?.is_some()
            {
                output::warning(&format!(
    ...
    // Store the explicitly requested branch before the empty-commits early
    // return: a freshly created branch seeded at a main commit has no
    // commits of its own, and `oak switch <remote-branch>` relies on this
    // row landing even when there's nothing else to pull.
    ...
    // `branch_heads` rows are learned from commit rows below, so a branch
    // with no commits of its own never gets one. Fall back to the head the
    // server reported for the requested branch — only when that commit is
    // already in local storage and there's no local head to overwrite.
    ...
    for br_data in sorted_branches {
        let status = BranchStatus::from_db_str(&br_data.status);
        // A branch that's already closed on the server and has no local row
        // is finished history (merged or abandoned elsewhere) — don't
        // materialize a tombstone for it.
    ...
    // `--force` keepsafe: when the forced sync is about to move this
    // branch's head to a server state that doesn't contain the local tip,
    // park the local state as `<branch>.orphaned-<timestamp>` first. The
    // commits were never deleted even before this; the orphan branch row
    // keeps them *reachable* (`oak switch <orphan>` recovers them).
    if force {
        if let (Some(branch), Some(old_tip)) = (branch_name, local_head) {
            if let Some(new_head) = branch_heads_seen.get(branch) {
                if new_head != old_tip
                    && !super::sync::local_history_contains(repo, new_head, old_tip)?
                {
                    let orphan = park_orphaned_branch(repo, branch, old_tip)?;
                    output::info(&format!(
    ...
    // deletions relative to the pre-pull head.
    if let Some(ref head) = new_head {
