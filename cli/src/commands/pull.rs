...
pub async fn fetch_and_store_blobs(
    ...
    for blob_data in blobs {
        if blob_data.chunks.is_empty() {
            ...
            if oak_core::ensure_empty_blob(repo, &Hash(blob_data.hash.clone()))? {
                continue;
            }
...
pub(crate) fn update_working_dir(
    ...
    let report = apply_manifest(
        ...
        ApplyOpts {
            ...
            missing_blobs: if allow_partial || !restricted.is_empty() {
                MissingBlobs::Skip
            } else {
                MissingBlobs::Error
            },
    ...
    // errors otherwise).
    let (restricted_skips, partial_skips): (Vec<&String>, Vec<&String>) = report
        .skipped
        .iter()
...
mod pull_ingest_publish_tests {
    use super::{
        publish_pull_refs, resolve_response_head, stage_pull_objects, BranchData, BranchRenameData,
        CommitData, PullResponse,
    };
    use oak_core::protocol::{BlobData, FileChangeData};
    use oak_core::{
        Branch, ChangeType, Commit, FileChange, Hash, MetadataKey, Repository, SqliteRepository,
        Tree,
    };
