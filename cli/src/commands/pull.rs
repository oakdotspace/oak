...
use oak_core::{
    reassemble_chunks, Blob, Branch, BranchStatus, ChunkInfo, CloseReason, Commit, Hash, Manifest,
    MetadataKey, OakError, Result, SparseCone,
};
...
/// Validate one untrusted blob-to-chunk descriptor before any of its objects
/// are persisted. Wire order is canonical order: offsets must begin at zero
/// and advance exactly by each preceding chunk's declared length.
pub(crate) fn validate_blob_mapping_descriptor<'a>(
    blob_hash: &str,
    blob_size: u64,
    chunks: impl IntoIterator<Item = (&'a str, u64, u64)>,
) -> Result<(Hash, Vec<ChunkInfo>)> {
    let blob_hash = Hash::from_hex(blob_hash)
        .map_err(|error| OakError::Server(format!("invalid blob hash in mapping: {error}")))?;
    if blob_hash.kind() != oak_core::HashKind::Blake3 {
        return Err(OakError::Server(format!(
            "blob mapping uses non-canonical content hash {blob_hash}"
        )));
    }
    let supplied: Vec<_> = chunks.into_iter().collect();
    if blob_hash == Blob::empty_hash() && blob_size == 0 {
        if supplied.is_empty() {
            return Ok((blob_hash, Vec::new()));
        }
        if supplied.len() == 1 {
            let (hash, offset, size) = supplied[0];
            let hash = Hash::from_hex(hash).map_err(|error| {
                OakError::Server(format!(
                    "empty blob has invalid sentinel chunk hash: {error}"
                ))
            })?;
            if hash == blob_hash && offset == 0 && size == 0 {
                return Ok((blob_hash, Vec::new()));
            }
        }
        return Err(OakError::Server(
            "empty blob has a non-canonical chunk mapping".to_string(),
        ));
    }
    let mut raw = Vec::new();
    let mut expected_offset = 0u64;
    let mut summed_size = 0u64;
    for (chunk_hash, offset, size) in supplied {
        let chunk_hash = Hash::from_hex(chunk_hash).map_err(|error| {
            OakError::Server(format!(
                "blob {blob_hash} has invalid chunk hash in mapping: {error}"
            ))
        })?;
        if chunk_hash.kind() != oak_core::HashKind::Blake3 {
            return Err(OakError::Server(format!(
                "blob {blob_hash} mapping uses non-canonical chunk hash {chunk_hash}"
            )));
        }
        if size == 0 {
            return Err(OakError::Server(format!(
                "blob {blob_hash} has a zero-length chunk in its mapping"
            )));
        }
        if offset != expected_offset {
            return Err(OakError::Server(format!(
                "blob {blob_hash} has non-contiguous chunk offset {offset}; expected {expected_offset}"
            )));
        }
        expected_offset = expected_offset
            .checked_add(size)
            .ok_or_else(|| OakError::Server(format!("blob {blob_hash} chunk offsets overflow")))?;
        summed_size = summed_size
            .checked_add(size)
            .ok_or_else(|| OakError::Server(format!("blob {blob_hash} chunk sizes overflow")))?;
        raw.push((chunk_hash, offset, size));
    }

    if raw.is_empty() {
        return Err(OakError::Server(format!(
            "blob {blob_hash} has no chunk refs"
        )));
    }
    if expected_offset != blob_size || summed_size != blob_size {
        return Err(OakError::Server(format!(
            "blob {blob_hash} chunk mapping covers {expected_offset} bytes (sum {summed_size}), expected {blob_size}"
        )));
    }
    ...
    let chunks = raw
        ...
        .map(|(hash, offset, size)| {
            Ok(ChunkInfo {
                hash,
                offset,
                length: u32::try_from(size).map_err(|_| {
                    OakError::Server(format!(
                        "blob {blob_hash} chunk length {size} exceeds the wire range"
                    ))
                })?,
            })
        })
...
pub(crate) fn validate_fetched_chunk(
    blob_hash: &Hash,
    chunk: &ChunkInfo,
    content: &[u8],
) -> Result<()> {
    if content.len() as u64 != chunk.length as u64 {
        return Err(OakError::Server(format!(
            "blob {blob_hash} chunk {} has fetched size {}, expected {}",
            chunk.hash,
            content.len(),
            chunk.length
        )));
    }
    let actual = oak_core::hash_bytes(content);
    if actual != chunk.hash {
        return Err(OakError::Server(format!(
            "blob {blob_hash} chunk {} fetched bytes hash to {actual}",
            chunk.hash
        )));
    }
    Ok(())
...
pub(crate) fn wire_to_core_tree(td: &TreeData) -> oak_core::Result<oak_core::Tree> {
    oak_core::protocol::tree_data_to_core(td)
...
pub(crate) fn commit_from_pull_data(commit_data: &CommitData) -> Result<Commit> {
    oak_core::protocol::commit_data_to_core(commit_data)
        .map_err(|e| OakError::Server(format!("invalid commit object from server: {e}")))
}

pub(crate) fn response_head_for_branch(
    pull_resp: &PullResponse,
    requested_branch: Option<&str>,
) -> Result<Option<(String, Hash)>> {
    let Some(head_str) = pull_resp.head.as_deref() else {
        return Ok(None);
    };
    let head = Hash::from_hex(head_str).map_err(|e| {
        OakError::Server(format!(
            "invalid response head from server {head_str:?}: {e}"
        ))
    })?;
    let branch_name = requested_branch
        .map(str::to_string)
        .or_else(|| pull_resp.branch.as_ref().map(|b| b.name.clone()))
        .or_else(|| {
            pull_resp
                .branches
                .iter()
                .find(|b| b.name == oak_core::DEFAULT_BRANCH)
                .map(|b| b.name.clone())
        });
    Ok(branch_name.map(|name| (name, head)))
}

/// Defense-in-depth for the historical-only known-loss contract. The hosted
/// server proves that no published branch head contains a declared lost blob;
/// the client independently proves the selected response head before it
/// records the disposition or publishes any local refs.
pub(crate) fn validate_known_loss_absent_from_selected_head(
    repo: &SqliteRepository,
    pull_resp: &PullResponse,
    selected_head: Option<&Hash>,
    known_lost: &std::collections::HashSet<String>,
) -> Result<()> {
    if known_lost.is_empty() {
        return Ok(());
    }
    let head = selected_head.ok_or_else(|| {
        OakError::Server(
            "report_v1 returned missing content without a selected response head".to_string(),
        )
    })?;
    let commit = if let Some(wire) = pull_resp
        .commits
        .iter()
        .find(|commit| commit.hash == head.0)
    {
        commit_from_pull_data(wire)?
    } else {
        repo.get_commit(head)?.ok_or_else(|| {
            OakError::Server(format!(
                "report_v1 could not prove selected head {head} before publishing refs"
            ))
        })?
    };

    let mut response_trees = std::collections::HashMap::with_capacity(pull_resp.trees.len());
    for wire in &pull_resp.trees {
        let tree = wire_to_core_tree(wire)?;
        response_trees.insert(tree.hash.clone(), tree);
    }
    let mut pending = vec![commit.manifest_hash];
    let mut visited = std::collections::HashSet::new();
    while let Some(hash) = pending.pop() {
        if !visited.insert(hash.clone()) {
            continue;
        }
        let tree = match response_trees.get(&hash) {
            Some(tree) => tree.clone(),
            None => repo.get_tree(&hash)?.ok_or_else(|| {
                OakError::Server(format!(
                    "report_v1 selected head references missing tree {hash}"
                ))
            })?,
        };
        for entry in tree.entries {
            match entry.kind {
                oak_core::TreeEntryKind::Blob if known_lost.contains(entry.hash.as_str()) => {
                    return Err(OakError::Server(format!(
                        "report_v1 rejected missing blob {} because it is reachable from selected head {head}",
                        entry.hash
                    )));
                }
                oak_core::TreeEntryKind::Tree => pending.push(entry.hash),
...
pub(crate) struct BlobFetchStats {
    pub(crate) required: std::collections::HashSet<Hash>,
    pub(crate) downloaded: std::collections::HashSet<Hash>,
    pub(crate) valid_before_fetch: std::collections::HashSet<Hash>,
    pub(crate) downloaded_logical_bytes: u64,
}
...
pub async fn fetch_and_store_blobs(
    ...
    fetch_and_store_blobs_with_stats(
        repo,
        blobs,
        BlobFetchContext {
            client,
...
pub(crate) struct BlobFetchContext<'a> {
    pub(crate) client: &'a reqwest::Client,
    pub(crate) remote: &'a str,
    pub(crate) owner: &'a str,
    pub(crate) name: &'a str,
    pub(crate) api_key: Option<&'a str>,
    pub(crate) emit_human_output: bool,
}

pub(crate) async fn fetch_and_store_blobs_with_stats(
    repo: &SqliteRepository,
    blobs: &[BlobData],
    context: BlobFetchContext<'_>,
) -> Result<BlobFetchStats> {
    let BlobFetchContext {
        client,
        remote,
        owner,
        name,
    ...
    let validated_mappings: Vec<(Hash, Vec<ChunkInfo>)> = blobs
        .iter()
        .map(|blob| {
            validate_blob_mapping_descriptor(
                &blob.hash,
                blob.size,
                blob.chunks
                    .iter()
    ...
    // Open the rollback guard only after every descriptor has passed. From
    // this point, corrupt-cache removal, empty-blob synthesis, downloaded
    // chunks, blobs, and replacement mappings are one ingest transaction.
    ...
    let mut stats = BlobFetchStats::default();

    for (blob_data, (blob_hash, chunks)) in blobs.iter().zip(&validated_mappings) {
        if chunks.is_empty() {
            // The empty blob legitimately has no chunks — push never sends it
            // through the chunk pipeline (`chunk_content(&[])` is empty), so
            // "no chunk refs" is its normal shape, not unreachable bytes. Its
            // content is implied by its hash, so store it and move on.
        ...
        for chunk in chunks {
            if stats.required.insert(chunk.hash.clone()) {
                if chunk_is_present_and_valid(repo, &chunk.hash)? {
                    stats.valid_before_fetch.insert(chunk.hash.clone());
                } else {
                    all_missing_hashes.push(chunk.hash.to_string());
                    all_missing_sizes.insert(chunk.hash.to_string(), chunk.length);
                }
            }
        }
    }
    ...
    if !all_missing_hashes.is_empty() {
        let mut dl_result = ChunkDownloadResponse {
            chunks: Vec::new(),
            batch_url: None,
            restricted: Vec::new(),
        };
        for page in all_missing_hashes.chunks(10_000) {
            ...
            if dl_result.batch_url.is_none() {
                dl_result.batch_url = page.batch_url;
            }
            dl_result.chunks.extend(page.chunks);
            dl_result.restricted.extend(page.restricted);
        }
        ...
        stats.downloaded = all_missing_hashes
            .iter()
            .map(|hash| Hash(hash.clone()))
            .collect();
        stats.downloaded_logical_bytes = all_missing_hashes
            .iter()
            .map(|hash| u64::from(*all_missing_sizes.get(hash).unwrap_or(&0)))
            .sum();
    ...
    for (_blob_data, (expected, mapping)) in blobs.iter().zip(&validated_mappings) {
        if mapping.is_empty() {
            continue;
        }
        ...
        for chunk in mapping {
            let data = repo.get_chunk(&chunk.hash)?.ok_or_else(|| {
                OakError::Server(format!("Missing chunk after download: {}", chunk.hash))
            })?;
            validate_fetched_chunk(expected, chunk, &data)?;
        ...
        store_verified_blob_chunks(repo, expected, mapping.clone(), verified.repaired)?;
    ...
    super::known_loss::clear_recovered_known_lost_blobs(
        repo,
        blobs.iter().map(|blob| blob.hash.clone()),
    )?;
    Ok(stats)
}
...
pub(crate) fn store_verified_blob_chunks(
    ...
    if !repaired {
        return repo.replace_blob_chunks(blob_hash, &advertised);
    }
    let blob = repo
        .get_blob(blob_hash)?
...
pub async fn run(
    ...
    if continue_after_conflict || abort {
        return run_once(path, "", false, force, continue_after_conflict, abort).await;
    }

    let remote = super::push::resolve_push_remote(path, remote_url)?;
    match run_once(
        path,
        &remote.url,
        remote.persist,
        force,
        continue_after_conflict,
        abort,
    )
    .await
    {
...
async fn run_once(
    ...
    fetch_current_branch(path, remote_url, persist_remote, force).await?;
    ...
    match super::sync::sync_from_parent_for_pull(
        path,
        branch_open_at_pull_start.as_deref(),
        Some(remote_url),
    )
    .await
    {
...
pub async fn fetch_current_branch(
    path: &Path,
    remote_url: &str,
    persist_remote: bool,
    force: bool,
...
fn stage_pull_objects(
    ...
    stage_pull_objects_with_output(repo, pull_resp, true)
}

fn stage_pull_objects_with_output(
    repo: &SqliteRepository,
    pull_resp: &PullResponse,
...
/// Resolve the server-advertised response head to one whose commit is
/// actually present locally after staging. Servers predating the
/// branch-scoped head fix report the repo-global head (main's tip) even for
/// branch-scoped pulls; a branch pull never ships that commit (typical
/// trigger: the branch was squash-merged elsewhere, so main's tip is the
/// squash commit), and adopting it would publish a dangling ref. Fall back
/// to the newest commit the response actually shipped for the branch, or to
/// no advance at all — the parent-sync phase of `oak pull` converges the
/// branch with main afterwards.
fn resolve_response_head(
    repo: &SqliteRepository,
    advertised: Option<(String, Hash)>,
    branch_heads_seen: &std::collections::HashMap<String, Hash>,
) -> Result<Option<(String, Hash)>> {
    let Some((branch, head)) = advertised else {
        return Ok(None);
    };
    if repo.get_commit(&head)?.is_some() {
        return Ok(Some((branch, head)));
    }
    let fallback = branch_heads_seen.get(&branch).cloned();
    if let Some(ref fb) = fallback {
        // Commits were shipped but the advertised head wasn't among them —
        // possibly a truncated response. Publishing the newest shipped
        // commit is consistent; flag the oddity instead of dead-ending.
        output::warning(&format!(
            "server advertised head {} for branch '{}' without shipping it; staying at {}",
            head.short(),
            branch,
            fb.short()
        ));
    }
    Ok(fallback.map(|h| (branch, h)))
}

fn reconcile_pull_branch(
    repo: &SqliteRepository,
    branch: &Branch,
    observed_generation: i64,
) -> Result<()> {
    let fresh = repo.reconcile_pulled_branch_metadata(branch, observed_generation)?;
    if !fresh {
        output::warning(&format!("Retained newer local description for branch {:?}: the remote response predates a local metadata update. Run `oak pull` again to refresh.", branch.name));
    } else if repo.branch_description_pending(&branch.name)? {
        output::warning(&format!(
            "Retained local description for branch {:?}: it differs from the remote and has no confirmed synchronization baseline. Inspect `oak branch list --remote --json`, then use `oak desc` on that branch with your chosen description to publish it.",
            branch.name
        ));
    }
    Ok(())
...
fn publish_pull_refs_at_generation(
    repo: &SqliteRepository,
    pull_resp: &PullResponse,
    ...
    force: bool,
    branch_heads_seen: &std::collections::HashMap<String, Hash>,
    response_head: Option<&(String, Hash)>,
    new_head: Option<&Hash>,
    known_lost_hashes: &[String],
    last_rename_id: i64,
    observed_generation: i64,
    ...
    if let Some(br_data) = &pull_resp.branch {
        ...
        reconcile_pull_branch(repo, &br, observed_generation)?;
    ...
    if force {
        if let (Some(branch), Some(old_tip)) = (branch_name, local_head) {
            let new_branch_head = response_head
                .filter(|(name, _)| name == branch)
                .map(|(_, head)| head)
                .or_else(|| branch_heads_seen.get(branch));
            if let Some(new_branch_head) = new_branch_head {
    ...
    if let Some((branch, head)) = response_head {
        ensure_commit_present(repo, head, &format!("response head for branch '{branch}'"))?;
    ...
    super::known_loss::record_known_lost_blobs(repo, known_lost_hashes)?;
...
pub(crate) enum PullWorktreeMode {
    /// Complete the ordinary pull lifecycle before publishing local refs.
    Apply,
    /// A guarded clone owns the empty destination and will materialize the
    /// selected head once after this fetch publishes its local object graph.
    DeferToClone,
}

#[derive(Debug, Default)]
pub(crate) struct PullAcquisitionStats {
    pub(crate) response_body_bytes: Option<u64>,
    pub(crate) commits_received: u64,
    pub(crate) trees_received: u64,
    pub(crate) blob_descriptors_received: u64,
    pub(crate) blob_fetch: BlobFetchStats,
    pub(crate) materialization_passes: u64,
    pub(crate) counters_complete: bool,
}

#[allow(clippy::too_many_arguments)]
pub(crate) async fn pull_async_with_stats(
    lock: &WorkdirLock,
    repo: &SqliteRepository,
    remote: &str,
    endpoint_pull_path: &str,
    ...
    api_key: Option<&str>,
    emit_human_output: bool,
    worktree_mode: PullWorktreeMode,
) -> Result<PullAcquisitionStats> {
    ...
    // Sparse checkout: ask the server to ship only the blobs reachable from the
    // active cone. Out-of-cone files still come back in the tree structure (so
    // the manifest hash verifies) but with their content withheld.
    if let Some(cone) =
        SparseCone::from_metadata(repo.get_metadata(MetadataKey::SparsePaths)?.as_deref())
    {
        params.push(format!(
            "paths={}",
            urlencoding::encode(&cone.prefixes().join(","))
        ));
        // This exact opt-in tells corrected servers that the client preserves
        // full manifest identities while intentionally storing bytes for only
        // the active cone. Older servers ignore the additive query parameter
        // and may safely overfetch, preserving compatibility.
        params.push(format!(
    ...
    let observed_generation = repo.description_generation()?;
    ...
    if resp.status().as_u16() == 409 {
        ...
        .await?;
        return Ok(PullAcquisitionStats {
            counters_complete: false,
            ..PullAcquisitionStats::default()
        });
    }
    ...
    let pull_resp: PullResponse =
        serde_json::from_slice(&response_body).map_err(|e| OakError::Http(e.to_string()))?;
    let mut acquisition = PullAcquisitionStats {
        response_body_bytes: Some(response_body.len() as u64),
        commits_received: pull_resp.commits.len() as u64,
        trees_received: pull_resp.trees.len() as u64,
        blob_descriptors_received: pull_resp.blobs.len() as u64,
        counters_complete: true,
        ..PullAcquisitionStats::default()
    };

    let known_lost = super::repo::parse_pull_missing_content(&pull_resp.missing_content)?;
    let response_head = response_head_for_branch(&pull_resp, branch_name)?;
    validate_known_loss_absent_from_selected_head(
        repo,
        &pull_resp,
        response_head.as_ref().map(|(_, head)| head),
        &known_lost,
    )?;
    let mut known_lost_hashes: Vec<String> = known_lost.iter().cloned().collect();
    known_lost_hashes.sort();

    // Path permissions: remember which blobs the server withheld as
    // restricted BEFORE materializing, so the working-dir update (and later
    // status/commit) can skip those files with an access message instead of
    // failing the pull as incomplete.
    super::restricted::record_restricted_blobs(repo, &pull_resp.restricted_blobs)?;

    if pull_resp.commits.is_empty() {
        let response_head =
            resolve_response_head(repo, response_head, &std::collections::HashMap::new())?;
        let new_head = response_head
            .as_ref()
            .map(|(_, head)| head.clone())
            .or_else(|| local_head.cloned());
        let published = publish_pull_refs_at_generation(
            repo,
            &pull_resp,
            branch_name,
            local_head,
        ...
        if emit_human_output && published.renames_applied > 0 {
            output::info(&format!(
                "Applied {} branch rename(s)",
                published.renames_applied
            ));
        }
        if !known_lost_hashes.is_empty() {
            output::warning(&super::known_loss::warning(known_lost_hashes.len()));
        }
        if emit_human_output {
    ...
    acquisition.blob_fetch = fetch_and_store_blobs_with_stats(
        repo,
        &pull_resp.blobs,
        BlobFetchContext {
            client: &client,
    ...
    let branch_heads_seen = stage_pull_objects_with_output(repo, &pull_resp, emit_human_output)?;
    let response_head = resolve_response_head(repo, response_head, &branch_heads_seen)?;

    // Determine the head to advance the current branch / global head to.
    // Prefer the server's explicit response head for the requested branch; if
    // an older server omits it, fall back to the last commit seen for the
    // branch, then to the local head (no change).
    let new_head: Option<Hash> = response_head
        .as_ref()
        .map(|(_, head)| head.clone())
        .or_else(|| {
            branch_name
    ...
    if new_head.is_some() && !force && worktree_mode == PullWorktreeMode::Apply {
    ...
    // A legacy selected-branch clone has an empty, guarded destination. Its
    // caller performs the one final materialization after selecting the exact
    // local branch head, so applying here would rewrite the entire tree twice.
    if worktree_mode == PullWorktreeMode::Apply {
        if let Some(ref head) = new_head {
            update_working_dir(lock, work_tree, repo, local_head, head, &known_lost)?;
            acquisition.materialization_passes = 1;
        }
    }
...
async fn reparent_after_divergence(
    ...
    if let Err(e) = super::sync::fetch_parent_from_server_with_remote_quiet(
        repo,
...
pub(crate) fn update_working_dir(
    ...
    head: &Hash,
    declared_known_lost: &std::collections::HashSet<String>,
) -> Result<()> {
    let commit = repo
        .get_commit(head)?
    ...
    // In a sparse checkout, out-of-cone files are intentionally withheld by the
    // server, so their absence here is expected — not a broken pull. Only
    // in-cone entries must have their blobs present.
    let cone = oak_core::SparseCone::from_metadata(
        repo.get_metadata(oak_core::MetadataKey::SparsePaths)?
            .as_deref(),
    );

    // Files the server withheld under path-based permissions can't
    // materialize: skip them with an access message rather than failing the
    // pull as incomplete. (Absence + the recorded restricted marker is what
    // status/commit later use to avoid reporting them as deletions.)
    let restricted: std::collections::HashSet<String> =
        super::restricted::restricted_paths_in_manifest(repo, &manifest)
            .into_iter()
            .collect();
    let mut known_lost: std::collections::HashSet<String> =
    ...
    known_lost.extend(
        manifest
            .entries
            .iter()
            .filter(|entry| declared_known_lost.contains(entry.blob_hash.as_str()))
            .map(|entry| entry.path.clone()),
    );

    // The empty blob is derivable from its hash alone, so reconstruct it up
    // front rather than reporting it as a blob the server failed to ship.
    // (`apply_manifest` does the same for the callers that reach it directly;
    // this pull path has its own preflight below.)
    ...
    if !allow_partial {
        for entry in &manifest.entries {
            if cone.as_ref().is_some_and(|c| !c.covers(&entry.path)) {
                continue;
            }
            if restricted.contains(&entry.path) {
                continue;
            }
            if known_lost.contains(&entry.path) {
                continue;
            }
    ...
    let report = apply_manifest(
        ...
        ApplyOpts {
            ...
            missing_blobs: if allow_partial || !restricted.is_empty() || !known_lost.is_empty() {
                MissingBlobs::Skip
            } else {
                MissingBlobs::Error
            },
    ...
    // Split the skips: restricted paths get the access message; anything else
    // only ever skips under OAK_ALLOW_PARTIAL_CLONE (the preflight above hard
    // errors otherwise).
    let (restricted_skips, remaining_skips): (Vec<&String>, Vec<&String>) = report
        .skipped
        .iter()
        .partition(|p| restricted.contains(p.as_str()));
    let (known_lost_skips, partial_skips): (Vec<&String>, Vec<&String>) = remaining_skips
        .into_iter()
        .partition(|p| known_lost.contains(p.as_str()));
    if !restricted_skips.is_empty() {
        output::warning(&super::restricted::withheld_summary(restricted_skips.len()));
        for p in &restricted_skips {
            output::warning(&format!("  - {p}"));
        }
    }
    if !known_lost_skips.is_empty() {
        output::warning(&super::known_loss::warning(known_lost_skips.len()));
        for path in &known_lost_skips {
...
mod verify_blob_content_tests {
    use super::{
        store_verified_blob_chunks, validate_blob_mapping_descriptor, verify_blob_content,
    };
    use oak_core::{hash_bytes, Blob, ChunkInfo, Hash, Repository, SqliteRepository};
    ...
    #[test]
    fn mapping_extent_checked_add_rejects_overflow() {
        let blob = "aa".repeat(32);
        let first = "bb".repeat(32);
        let second = "cc".repeat(32);
        let error = validate_blob_mapping_descriptor(
            &blob,
            u64::MAX,
            [
                (first.as_str(), 0, u64::MAX),
                (second.as_str(), u64::MAX, 1),
            ],
        )
        .unwrap_err();
        assert!(error.to_string().contains("offsets overflow"));
    }

    #[test]
    fn mapping_rejects_non_blake3_chunk_hash_shape() {
        let blob = "aa".repeat(32);
        let sha1_shaped = "bb".repeat(20);
        let error =
            validate_blob_mapping_descriptor(&blob, 1, [(sha1_shaped.as_str(), 0, 1)]).unwrap_err();
        assert!(error.to_string().contains("non-canonical chunk hash"));
    }

    #[test]
    fn verified_mapping_replaces_stale_rows_instead_of_appending() {
        let temp = tempfile::tempdir().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let content = b"replacement mapping".to_vec();
        let blob = Blob::new(content.clone());
        repo.store_blob(&blob).unwrap();
        let stale = hash_bytes(b"stale chunk");
        repo.store_chunk(&stale, b"stale chunk").unwrap();
        ...
        .unwrap();
        repo.store_chunk(&blob.hash, &content).unwrap();

        store_verified_blob_chunks(
            &repo,
            &blob.hash,
            vec![ChunkInfo {
        ...
        let mapping = repo.get_blob_chunks(&blob.hash).unwrap().unwrap();
        assert_eq!(mapping.len(), 1);
        assert_eq!(mapping[0].hash, blob.hash);
        assert_eq!(mapping[0].length, content.len() as u32);
    }
}
...
mod pull_ingest_publish_tests {
    use super::{
        resolve_response_head, stage_pull_objects, validate_known_loss_absent_from_selected_head,
        BranchData, BranchRenameData, CommitData, PullResponse,
    };
    use oak_core::protocol::{tree_to_wire, BlobData, FileChangeData};
    use oak_core::{
        Branch, ChangeType, Commit, FileChange, FileMode, Hash, MetadataKey, Repository,
    ...
    fn publish_pull_refs(
        ...
        local_head: Option<&Hash>,
        force: bool,
        heads: &std::collections::HashMap<String, Hash>,
        response_head: Option<&(String, Hash)>,
        new_head: Option<&Hash>,
        known_lost: &[String],
        rename_id: i64,
    ) -> oak_core::Result<super::PullRefPublication> {
        super::publish_pull_refs_at_generation(
            repo,
            response,
            branch,
            local_head,
    ...
    #[test]
    fn pull_refreshes_remote_description_without_moving_heads() {
        let (_temp, repo) = repo();
        let mut remote = branch("feature");
        repo.store_branch(&super::branch_from_pull_data(&remote).unwrap())
            .unwrap();
        remote.description = Some("updated remotely".to_string());
        let (mut response, _) = response_for("feature");
        response.branches = vec![remote];
        response.commits.clear();
        response.head = None;
        for _ in 0..2 {
            publish_pull_refs(
                &repo,
            ...
            assert_eq!(
                repo.get_branch("feature")
                    ...
                    .description
                    .as_deref(),
                Some("updated remotely")
            );
            assert_eq!(repo.get_branch_head("feature").unwrap(), None);
        }
    }

    #[test]
    fn pull_preserves_pending_description_until_matching_acknowledgement() {
        let (_temp, repo) = repo();
        let remote = branch("feature");
        repo.store_branch(&super::branch_from_pull_data(&remote).unwrap())
            .unwrap();
        repo.edit_branch_description("feature", "my unsynced text")
            .unwrap();
        repo.update_branch_status("feature", oak_core::BranchStatus::Closed)
            .unwrap();
        let (mut response, _) = response_for("feature");
        response.branch = Some(remote);
        response.branches.clear();
        response.commits.clear();
        response.head = None;
        let publish = |response: &PullResponse| {
            publish_pull_refs(
                &repo,
                response,
                Some("feature"),
        ...
        publish(&response);
        let warning = crate::output::end_capture();
        assert!(warning.contains("Retained local description"));
        assert!(warning.contains("oak branch list --remote --json"));
        assert_eq!(
            repo.get_branch("feature")
                .unwrap()
                .unwrap()
        ...
        // An older publication completing after a newer local edit cannot clear it.
        repo.acknowledge_branch_description(
            "feature",
            Some("older text"),
            repo.description_publication_generation().unwrap(),
        )
        .unwrap();
        assert!(repo.branch_description_pending("feature").unwrap());
        response.branch.as_mut().unwrap().description = Some("my unsynced text".into());
        publish(&response);
        assert!(!repo.branch_description_pending("feature").unwrap());
        response.branch.as_mut().unwrap().description = Some("next remote edit".into());
        publish(&response);
        assert_eq!(
            repo.get_branch("feature")
                .unwrap()
    ...
    fn wire_commit(
        ...
        let commit = Commit::with_timestamp(
            ...
            vec![FileChange {
                ...
                old_blob_hash: None,
                new_blob_hash: Some(Hash::from_hex(&"b".repeat(64)).unwrap()),
                old_path: None,
                old_mode: None,
                new_mode: None,
            }],
        ...
        (commit_to_wire(&commit), hash)
    }

    fn response_for(branch_name: &str) -> (PullResponse, Hash) {
        let (commit, commit_hash) = wire_commit(branch_name, None, "2026-06-16T00:00:00Z");
        (
            PullResponse {
    ...
    #[test]
    fn report_v1_loss_must_be_absent_from_selected_head_manifest() {
        let (_temp, repo) = repo();
        let current_blob = Hash::from_hex(&"a1".repeat(32)).unwrap();
        let historical_blob = Hash::from_hex(&"b2".repeat(32)).unwrap();
        let tree = Tree::new(vec![TreeEntry {
            name: "current.txt".to_string(),
            kind: TreeEntryKind::Blob,
            hash: current_blob.clone(),
        ...
        let response = PullResponse {
            head: Some(commit.hash.to_string()),
            branch: None,
        ...
        validate_known_loss_absent_from_selected_head(
            ...
            &response,
            Some(&commit.hash),
            &std::collections::HashSet::from([historical_blob.to_string()]),
        )
        .expect("historical-only loss is safe for the selected checkout");
        let error = validate_known_loss_absent_from_selected_head(
            &repo,
            &response,
            Some(&commit.hash),
            &std::collections::HashSet::from([current_blob.to_string()]),
        )
        .expect_err("a missing current file must remain fail closed");
        assert!(error.to_string().contains("selected head"), "{error}");
    }
    ...
    fn staging_pull_objects_does_not_publish_branch_refs() {
        ...
        let (pull, commit_hash) = response_for("feature");

        let heads = stage_pull_objects(&repo, &pull).unwrap();

        assert_eq!(heads.get("feature"), Some(&commit_hash));
        assert!(repo.get_commit(&commit_hash).unwrap().is_some());
    ...
    fn publish_refs_rolls_back_branch_rows_on_error() {
        ...
        let known_lost = vec!["ab".repeat(32)];
        ...
        assert!(
            super::super::known_loss::load_known_lost_blobs(&repo).is_empty(),
            "known-loss marker must roll back with rejected ref publication"
        );
    }
    ...
    fn publish_refs_rejects_missing_commit_head_and_rolls_back() {
        let (_temp, repo) = repo();
        let missing_hash = Hash::from_hex(&"d".repeat(64)).unwrap();
        ...
        let response_head = ("feature".to_string(), missing_hash.clone());
    ...
    fn publish_refs_lands_rename_heads_current_head_and_watermark_together() {
        ...
        let (mut pull, commit_hash) = response_for("new");
        ...
        let heads = stage_pull_objects(&repo, &pull).unwrap();
        let response_head = ("new".to_string(), commit_hash.clone());
        ...
        assert_eq!(repo.get_head().unwrap(), Some(commit_hash));
        assert_eq!(
    ...
    #[test]
    fn staging_pull_objects_rejects_mismatched_commit_hash() {
        let (_temp, repo) = repo();
        let (mut pull, original_hash) = response_for("feature");
        pull.commits[0].hash = "d".repeat(64);

        let err = stage_pull_objects(&repo, &pull).unwrap_err();
        assert!(
            err.to_string().contains("commit hash mismatch"),
            "unexpected error: {err}"
        );
        assert!(repo.get_commit(&original_hash).unwrap().is_none());
    }

    #[test]
    fn publish_refs_prefers_response_head_over_payload_order() {
        let (_temp, repo) = repo();
        let (base, base_hash) = wire_commit("feature", None, "2026-06-16T00:00:00Z");
        let (tip, tip_hash) =
            wire_commit("feature", Some(base_hash.clone()), "2026-06-16T00:01:00Z");
        let pull = PullResponse {
            head: Some(tip_hash.to_string()),
            branch: Some(branch("feature")),
            branches: vec![branch("feature")],
            commits: vec![tip, base],
            blobs: Vec::<BlobData>::new(),
            trees: Vec::new(),
            renames: Vec::new(),
            restricted_blobs: Vec::new(),
        ...
        let heads = stage_pull_objects(&repo, &pull).unwrap();
        assert_eq!(heads.get("feature"), Some(&base_hash));

        let response_head = ("feature".to_string(), tip_hash.clone());
        publish_pull_refs(
            &repo,
            &pull,
            Some("feature"),
        ...
        assert_eq!(repo.get_branch_head("feature").unwrap(), Some(tip_hash));
    }

    #[test]
    fn resolve_response_head_demotes_unshipped_head() {
        let (_temp, repo) = repo();
        let missing = Hash::from_hex(&"d".repeat(64)).unwrap();

        // Nothing shipped for the branch and the advertised head is unknown
        // locally: this is an old server leaking the repo-global head (main's
        // tip) into a branch-scoped pull — e.g. after the branch was
        // squash-merged elsewhere. Drop the head instead of dead-ending the
        // pull; the parent-sync phase converges with main.
        let resolved = resolve_response_head(
            &repo,
            Some(("feature".to_string(), missing.clone())),
            &std::collections::HashMap::new(),
        )
        .unwrap();
        ...
        // Commits were shipped but the advertised head wasn't among them:
        // publish the newest shipped commit rather than a dangling ref.
        let (pull, shipped) = response_for("feature");
        stage_pull_objects(&repo, &pull).unwrap();
        let heads = std::collections::HashMap::from([("feature".to_string(), shipped.clone())]);
        let resolved =
            resolve_response_head(&repo, Some(("feature".to_string(), missing)), &heads).unwrap();
        assert_eq!(resolved, Some(("feature".to_string(), shipped.clone())));

        // A head whose commit is present resolves to itself.
        let resolved = resolve_response_head(
            &repo,
            Some(("feature".to_string(), shipped.clone())),
            &std::collections::HashMap::new(),
        )
        .unwrap();
        assert_eq!(resolved, Some(("feature".to_string(), shipped)));
    }
}
