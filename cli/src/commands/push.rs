...
async fn push_async_with_endpoint(
    ...
    if total_inline > MAX_INLINE_BYTES {
        for blob in &mut blobs {
            if !blob.chunks.is_empty() {
                continue; // already chunked above (content moved out then)
            }
            if blob.content.is_empty() {
                // The empty blob. It is *not* already chunked — it is inline
                // with zero bytes — so testing `content.is_empty()` alone
                // would assert a false invariant here. It contributes nothing
                // to the inline payload this loop exists to shrink, and
                // rewriting it into a zero-length chunk ref would upload an
                // empty chunk whose v1 hash collides with the empty-tree
                // sentinel. Leave it inline; the receiver reconstructs it from
                // its hash regardless (see `oak_core::ensure_empty_blob`).
                continue;
            }
    ...
    let branch_data = if let Some(name) = branch_name {
        repo.get_branch(name)?.map(|br| BranchData {
            ...
            close_reason: br.close_reason.as_ref().map(|r| r.as_str().to_string()),
        })
    } else {
        None
    };
