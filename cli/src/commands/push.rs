...
async fn push_async_with_endpoint(
    ...
    if push_resp.success {
        if let (Some(repo_path), Some(branch)) = (repo_path_for_web, branch_name) {
...
fn link_remote_identity(
    ...
    let (owner, name) = spec.split_once('/').ok_or_else(|| {
        OakError::Server(format!(
            "--repo expects ORG/REPO (got '{spec}'). Example: --repo my-org/my-repo"
        ))
    })?;
    ...
    {
        return Err(OakError::Server(format!(
            "Invalid --repo value '{spec}'. Use ORG/REPO with no spaces; REPO may not contain '/' or '\\'."
        )));
    }
