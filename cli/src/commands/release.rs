...
fn auth_token(remote: &str, cwd: &Path) -> Option<String> {
    super::credentials::effective_token_for_worktree(remote, cwd)
}
...
fn validate_release_tag(tag: &str) -> Result<()> {
    if tag.is_empty() || tag.trim() != tag {
        return Err(OakError::InvalidArgument(
            "release tag cannot be empty or have leading/trailing whitespace".to_string(),
        ));
    }
    if tag.chars().any(|c| c.is_control()) {
        return Err(OakError::InvalidArgument(
            "release tag cannot contain control characters".to_string(),
        ));
    }
    Ok(())
}

fn validate_asset_filename(filename: &str) -> Result<()> {
    if filename.is_empty() || filename.trim() != filename {
        return Err(OakError::InvalidArgument(
            "release asset filename cannot be empty or have leading/trailing whitespace"
                .to_string(),
        ));
    }
    if filename == "." || filename == ".." {
        return Err(OakError::InvalidArgument(
            "release asset filename cannot be '.' or '..'".to_string(),
        ));
    }
    if filename
        .chars()
    ...
    {
        return Err(OakError::InvalidArgument(
            "release asset filename cannot contain control characters or path separators"
                .to_string(),
        ));
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn validate_release_tag_accepts_safe_tags() {
        for tag in ["v1.0.0", "release-2026-06-16", "beta/rc1"] {
            validate_release_tag(tag).unwrap();
        }
    }

    #[test]
    fn validate_release_tag_rejects_empty_trimmed_and_control_tags() {
        for tag in ["", " v1.0.0", "v1.0.0 ", "v1\n0", "v1\t0", "v1\0"] {
            let err = validate_release_tag(tag).unwrap_err();
            assert!(
                matches!(err, OakError::InvalidArgument(_)),
                "{tag:?} should be InvalidArgument, got {err:?}"
            );
        }
    }

    #[test]
    fn validate_asset_filename_accepts_safe_filenames() {
        for filename in [
            "oak-aarch64-apple-darwin.tar.gz",
            "checksums.sha256",
            "app.dmg",
        ] {
            validate_asset_filename(filename).unwrap();
        }
    }

    #[test]
    fn validate_asset_filename_rejects_empty_path_and_control_names() {
        for filename in [
            "",
            " asset.zip",
            "asset.zip ",
            ...
            let err = validate_asset_filename(filename).unwrap_err();
            assert!(
                matches!(err, OakError::InvalidArgument(_)),
                "{filename:?} should be InvalidArgument, got {err:?}"
            );
        }
    }
}
