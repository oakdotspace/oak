...
pub async fn clone_repo(remote: &str, spec: &str, dest: &Path, shallow: bool) -> Result<()> {
        ...
        match super::credentials::get_username_for_server(remote) {
            ...
            None => {
                return Err(OakError::Server(format!(
                    "No login for {remote}; run `oak login` first, or specify the org explicitly (e.g. `oak clone <org>/{spec}`)"
                )));
            }
        }
    };

    // Split into owner / name. A third `/path` segment used to scope the clone
    // to a subtree; path-scoping is gone, so reject it with a clear error.
    ...
    if owner.is_empty() || name.is_empty() {
        return Err(OakError::Server(format!(
            "Invalid spec '{spec}': expected org/repo"
        )));
    }
    if subpath.is_some() {
        return Err(OakError::Server(format!(
            "Invalid spec '{spec}': path-scoped clones are no longer supported — clone the whole repo with org/repo"
        )));
    }
    ...
    match get_single_repo(remote, owner, name, dest, shallow).await {
        Err(OakError::RemoteMoved { origin }) if crate::http::is_trusted_origin(&origin) => {
            note_clone_remote_move(remote, &origin)?;
            get_single_repo(&origin, owner, name, dest, shallow).await
        }
        result => result,
    }
}
...
fn write_working_directory(repo: &SqliteRepository, head: &Hash, repo_dir: &Path) -> Result<()> {
    ...
    let entries_to_write: Vec<&ManifestEntry> = manifest.entries.iter().collect();
