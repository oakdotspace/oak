...
use oak_core::{
    Branch, BranchStatus, ChangeType, Commit, FileChange, FileMode, Hash, ManifestEntry,
...
struct PullResponse {
    head: Option<String>,
    branch: Option<BranchPullData>,
    #[serde(default)]
    branches: Vec<BranchPullData>,
    commits: Vec<CommitData>,
    blobs: Vec<BlobData>,
    #[serde(default)]
    trees: Vec<TreeData>,
    /// When the server filtered the response by team/project, the
    /// resolved path prefixes are echoed here so the client can
    /// materialize and persist the scope locally. Empty / absent =
    /// whole-repo response.
    #[serde(default)]
...
struct CommitData {
    ...
    author: String,
    // Nullable on the wire: only present for main-branch squash-merges,
    // where it carries the merged branch's description.
    #[serde(default)]
...
// Wire-format blob entry. Same struct used by `oak pull` — keeps both
// flows on a single chunked-fetch path (see `pull::fetch_and_store_blobs`).
use super::pull::BlobData;
...
pub async fn clone_interactive(remote: &str, cwd: &Path, full: bool) -> Result<()> {
    let choices = fetch_clone_choices(remote).await?;
    if choices.is_empty() {
        output::info("No repositories or projects found.");
        return Ok(());
    }
    ...
    let idx = FuzzySelect::new()
        .with_prompt("Search repos and projects")
        .items(&labels)
        .default(0)
        .interact()
    ...
    let default_leaf = match choice.project.as_deref() {
        Some(project) if choice.path_prefix != "/" => format!("{}-{project}", choice.repo),
        _ => choice.repo.clone(),
    };
    let default_dest = cwd.join(&default_leaf);
    let dest_actions = [
    ...
    clone_repo(remote, &spec, &dest, None, choice.project.as_deref(), full).await
}
...
async fn fetch_clone_choices(remote: &str) -> Result<Vec<CloneChoice>> {
    ...
    for repo in list.repos {
        ...
        for project in repo.projects {
            let whole_repo = project.path_prefix == "/";
            let label = if whole_repo {
                format!("{}/{}  (whole repo)", owner, repo.name)
            } else {
                format!("{}/{} / {}", owner, repo.name, project.slug)
            };
            let mut details = Vec::new();
            if !whole_repo {
                details.push(project.path_prefix.clone());
            }
            if project.name != project.slug && !project.name.is_empty() {
                details.push(project.name.clone());
            }
            if let Some(desc) = project.description.as_deref().filter(|d| !d.is_empty()) {
                details.push(desc.to_string());
            } else if let Some(desc) = repo.description.as_deref().filter(|d| !d.is_empty()) {
                details.push(desc.to_string());
            }
            details.push(project.visibility.clone());
            choices.push(CloneChoice {
                owner: owner.clone(),
                repo: repo.name.clone(),
                project: if whole_repo {
                    None
                } else {
...
pub async fn clone_repo(
    ...
    let resolved = if spec.contains('/') {
        ...
        return Err(OakError::Server(
            "Repository spec is empty; expected '<repo>' or '<owner>/<repo>'".to_string(),
        ));
    } else {
        match super::credentials::get_username_for_server(remote) {
            Some(username) => format!("{username}/{spec}"),
            None => {
                return Err(OakError::Server(format!(
                    "No login for {remote}; run `oak login` first, or specify the owner explicitly (e.g. `oak clone <owner>/{spec}`)"
                )));
            }
        }
    };

    // Split into owner / name / optional-subpath (e.g. "acme/mono/packages/api").
    let mut parts = resolved.splitn(3, '/');
    let owner = parts.next().unwrap_or("").trim();
    let name = parts.next().unwrap_or("").trim();
    let subpath = parts.next().map(|p| p.trim()).filter(|p| !p.is_empty());

    if owner.is_empty() || name.is_empty() {
        return Err(OakError::Server(format!(
            "Invalid spec '{spec}': expected owner/repo or owner/repo/path"
        )));
    }
    if (team.is_some() || project.is_some()) && subpath.is_some() {
        return Err(OakError::Server(
            "path prefix in spec is mutually exclusive with --team / --project".to_string(),
        ));
    }

    get_single_repo(remote, owner, name, dest, team, project, subpath, full).await
}

/// Clone a single repo from the server.
///
/// `full` selects between a shallow clone (the default — only the most recent
/// commit on the default branch, like `git clone --depth=1`) and a full clone
/// that downloads the entire history. Shallow is the fast path: the head
...
async fn get_single_repo(
    ...
    // When cloning with --team / --project / a path prefix, ask the server
    // to filter the response so we don't transfer blob content for paths
    // outside the scope. The server still emits every tree object so future
    // commit-under-scope operations can preserve out-of-scope entries.
    let pull_url = {
        ...
        if let Some(pfx) = path_prefix {
            params.push(format!("prefix={}", urlencoding::encode(pfx)));
        }
        // Shallow clone by default: ask the server for just the most recent
        // commit on the default branch. `--full` omits the param to fetch the
        // entire history.
        if !full {
            params.push("depth=1".to_string());
        }
        if params.is_empty() {
            format!("{remote}/api/{owner}/{name}/pull")
        } else {
            format!("{remote}/api/{owner}/{name}/pull?{}", params.join("&"))
        }
    };
    ...
    // Persist the active scope metadata so subsequent commands respect
    // the filter. Saving on the live repo handle (not yet committed via
    // `store_pull_response`) is fine — the metadata table doesn't have
    // FKs that conflict with the bulk import.
    let scope_prefixes = pull_resp.path_prefixes.clone();
    scope::set_active_scope(&repo, team, project, &scope_prefixes)?;
    ...
    let commit_count = store_pull_response(&repo, &pull_resp).await?;
    ...
    if let Some(head) = repo.get_branch_head(&chosen_branch)? {
        repo.set_head(&head)?;
        write_working_directory(&repo, &head, &repo_dir, &scope_prefixes).inspect_err(|_| {
            cleanup();
        })?;
    }

    let scope_label = match (team, project) {
        (Some(t), _) => format!(" (team '{t}')"),
    ...
    if !full {
        output::item("Shallow clone (recent history only) — re-run with `oak clone --full` for the complete history");
    }
...
pub fn prepare_personal_branch(repo: &SqliteRepository, proposed_name: &str) -> Result<String> {
    ...
    while repo.get_branch(&name)?.is_some() {
        name = format!(
            "{}-{}",
            super::init::author_slug(),
            super::init::random_branch_suffix()
        );
    }
...
async fn store_pull_response(repo: &SqliteRepository, pull_resp: &PullResponse) -> Result<usize> {
    ...
    for tree_data in &pull_resp.trees {
        ...
        let tree = oak_core::Tree {
            hash: Hash(tree_data.hash.clone()),
            entries,
        };
...
///
/// `prefixes` (when non-empty) restricts which manifest entries are
/// written. Storage (blobs, trees, manifests, commits) is unfiltered —
/// the local repo always holds the full graph the server shipped, so
/// future scope changes don't require re-cloning.
fn write_working_directory(
    ...
    let entries_to_write: Vec<&ManifestEntry> = if prefixes.is_empty() {
        manifest.entries.iter().collect()
    } else {
        oak_core::filter_entries_by_prefixes(manifest.entries.iter(), prefixes)
    };
    ...
    for entry in &entries_to_write {
        ...
        // mismatch that the user needs to see.
        ...
        fs::write(&write_path, &blob.content)?;
        crate::file_permissions::apply_file_permissions(&write_path, entry.mode)?;
        write_pb.inc(1);
    }
...
pub async fn transfer(remote: &str, spec: &str, target: &str) -> Result<()> {
    ...
    if !resp.status().is_success() {
        ...
        let err_text = resp.text().await.unwrap_or_default();
        return Err(OakError::Server(format!("{status}: {err_text}")));
    }
