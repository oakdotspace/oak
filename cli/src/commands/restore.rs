...
pub fn run(
    ...
    // Literal HEAD has all the default-source semantics, including current
    // branch selection, unborn repositories, output, and ignored-path warnings.
    let source = source.filter(|src| *src != "HEAD");
    ...
    let target_relatives: Vec<String> = paths
        .iter()
    ...
    let matches_target = |file_path: &str| -> bool {
        ...
        for target in &target_relatives {
            if super::diff::path_matches(file_path, std::slice::from_ref(target)) {
                return true;
            }
        }
        false
    ...
    if restore_all {
        ...
        for target in &target_relatives {
            ...
            } else if full_target.is_file() && !head_files.contains(target.as_str()) {
                files_to_delete.push(target.clone());
            }
        }
    }

    // Tracked paths that ignore rules now cover are in a state restore cannot
    // clear: they're on disk, but the status scan skips them, so `oak status`
    // reports them as pending deletions no matter what restore writes. Detect
    // them through the same predicate status's warning uses, and explain —
    // never claim "already at source state" while status disagrees (fb-25).
    // Only meaningful against HEAD: with an explicit `--source`, status isn't
    // diffing against that commit, so there's no report to reconcile.
    let now_ignored: Vec<String> = if source.is_none() {
        manifest
            .entries
            .iter()
    ...
    if files_to_restore.is_empty() && files_to_delete.is_empty() {
        if !now_ignored.is_empty() {
            super::commit::explain_now_ignored_unclearable(&now_ignored, "restore");
        } else if restore_all {
    ...
    // Both streams, not just stdin: the confirmation below prints the question
    // on stdout and reads the answer from stdin, so a TTY stdin with a
    // redirected or dead stdout would show nothing and still accept a "y" —
    // a blind confirmation of a destructive restore. `can_prompt` refuses that
    // shape, same as it refuses a scripted stdin.
    if !force && !output::can_prompt() {
    ...
    output::print_line("The following changes will be discarded:\n");

    if !files_to_restore.is_empty() {
        output::print_line("Tracked files:");
        for file in &files_to_restore {
            output::print_line(file);
        }
        output::print_line("");
    }

    if !files_to_delete.is_empty() {
        output::print_line("Untracked files to delete:");
        for file in &files_to_delete {
            output::print_line(&format!("  delete:  {file}"));
        }
        output::print_line("");
    }

    if !force {
        output::print_raw("Are you sure you want to discard these changes? [y/N] ");
    ...
    // Even after a successful restore, now-ignored tracked paths keep showing
    // as pending deletions in `oak status` — say why, so the lingering report
    // is never a surprise. Re-evaluate with fresh ignore rules: the restore
    // may have deleted an untracked .oakignore/.gitignore, un-ignoring the
    // paths.
    if !now_ignored.is_empty() {
        ...
        let still_ignored: Vec<String> = now_ignored
            ...
            .collect();
        super::commit::explain_now_ignored_unclearable(&still_ignored, "restore");
    }
