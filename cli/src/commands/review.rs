...
use crate::commands::triage::{
    derive_branch_triage, guard_dirty_current_worktree, BranchTriageInput, BranchTriageJson,
    ChecksJson, ContributionState, Mergeability, RecommendedAction, TargetRisk,
    UniqueContributionJson,
};
use crate::output;
...
mod tests {
    ...
    #[test]
    fn review_verdict_cannot_be_overridden_by_a_clean_certified_preview() {
        let preview = merge_preview(Some(true));
        for action in [RecommendedAction::Review, RecommendedAction::Unknown] {
            for remote in [false, true] {
                let command = "oak branch review feature --merge-preview --json";
                assert_eq!(
                    review_recommendations("feature", remote, Some(&preview), action, command),
    ...
    fn file_summary_without_hydrated_blobs_omits_content_category() {
        ...
        assert_eq!(summary.content_unavailable_reason, Some("missing_blob"));
    ...
    #[test]
    fn remote_comparison_uses_pinned_heads_not_matching_local_refs() {
        let temp = tempfile::TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let make_commit = |branch: &str, content: &str| {
            let (entry, _) = manifest_entry(&repo, "tracked.txt", content);
            let manifest = repo.put_manifest(vec![entry]).unwrap();
            repo.put_commit(
                branch.to_string(),
                None,
                None,
        ...
        let remote_source = make_commit("feature", "remote source\n");
        let remote_target = make_commit("main", "remote target\n");
        let local_source = make_commit("feature", "local unpushed\n");
        let local_target = make_commit("main", "local target\n");
        repo.store_branch(&oak_core::Branch::new(
            "feature".to_string(),
            None,
            Some("main".to_string()),
        ...
        .unwrap();
        repo.set_branch_head("feature", &local_source).unwrap();
        repo.store_branch(&oak_core::Branch::new("main".to_string(), None, None))
            .unwrap();
        repo.set_branch_head("main", &local_target).unwrap();

        let comparison = remote_branch_comparison(
            &repo,
            &RemoteAnalysisBranch {
                name: "feature".to_string(),
                head: Some(remote_source.clone()),
                parent: Some("main".to_string()),
            },
            &RemoteAnalysisBranch {
                name: "main".to_string(),
                head: Some(remote_target.clone()),
                parent: None,
            },
        )
        .unwrap();

        assert_eq!(comparison.branch_head, Some(remote_source));
        assert_eq!(comparison.against_head, Some(remote_target));
        assert_ne!(comparison.branch_head, Some(local_source));
        assert_ne!(comparison.against_head, Some(local_target));
    }

    #[test]
    fn remote_commit_count_storage_failure_cannot_certify_empty() {
        let walk = ReviewAncestorWalk {
            ancestors: vec![Hash("11".repeat(32))],
            ..Default::default()
        };
        let result = count_remote_branch_commits(&walk, "feature", |_| {
            Err(OakError::Database("count read failed".into()))
        });
        assert!(
            matches!(result, Err(OakError::Database(message)) if message == "count read failed")
        );
    }

    #[test]
    fn remote_commit_count_disappearing_commit_cannot_certify_empty() {
        let hash = Hash("22".repeat(32));
        let walk = ReviewAncestorWalk {
            ancestors: vec![hash.clone()],
            ..Default::default()
        };
        let result = count_remote_branch_commits(&walk, "feature", |_| Ok(None));
        assert!(
            matches!(result, Err(OakError::IncompleteCommitData { missing, .. }) if missing == hash.to_string())
        );
    }

    #[test]
    fn remote_commit_count_missing_history_cannot_certify_empty() {
        let walk = ReviewAncestorWalk {
            missing_edges: vec![ReviewMissingEdge {
                hash: Hash("33".repeat(32)),
                depth: 1,
            }],
            ..Default::default()
        };
        let result = count_remote_branch_commits(&walk, "feature", |_| unreachable!());
        assert!(matches!(result, Err(OakError::IncompleteAncestry { .. })));
    }

    #[test]
    fn remote_positive_commit_count_with_missing_history_is_an_explicit_lower_bound() {
        let temp = tempfile::TempDir::new().unwrap();
        let repo = SqliteRepository::open(&temp.path().join("oak.db")).unwrap();
        let manifest = repo.put_manifest(vec![]).unwrap();
        let head = repo
            .put_commit(
                "feature".to_string(),
                Some(Hash("44".repeat(32))),
                None,
                manifest,
                "tester".to_string(),
                None,
        ...
        .unwrap();
        assert_eq!(comparison.branch_commit_count, 1);
        assert!(comparison
            .caveats
            .iter()
            .any(|caveat| caveat.contains("branch_commit_count is a lower bound")));
    }
}
...
struct FileSummaryJson {
    ...
    /// Content could not be read; distinct from observed binary/large bytes.
    #[serde(skip_serializing_if = "Option::is_none")]
    content_unavailable_reason: Option<&'static str>,
...
pub(crate) struct BranchComparison {
    ...
    /// Only a complete local history against unborn main has an empty base.
    /// Missing ancestry and disconnected existing histories are unavailable.
    empty_fork_available: bool,
    branch_commit_count: usize,
...
pub async fn remote_branch_diff_json(
    ...
    validate_changed_files_window(changed_files_limit)?;
    let preparation =
        prepare_remote_branch_review(path, branch, against, diff_mode != DiffMode::Tree).await?;
    ...
    let comparison = remote_branch_comparison(repo, &preparation.source, &preparation.target)?;
    ...
    let hunks_truncated = attach_remote_branch_patches_from_snapshot(
        repo,
        &comparison,
        branch,
        against,
    ...
    caveats.extend(computed.extra_caveats);
    caveats.push(
        "Remote branch evidence was pinned from one fetch; verified objects may be cached, but local refs and branch metadata were not changed."
            .to_string(),
    );
...
pub fn branch_review_json(
    ...
    let mut triage = branch_triage_json(
        repo.as_ref(),
        branch,
        false,
        &comparison,
    ...
    let dirty_file_count = if repo.get_current_branch_name()?.as_deref() == Some(branch) {
        Some(
    ...
    if dirty_file_count.is_some_and(|count| count > 0) {
        guard_dirty_current_worktree(repo.as_ref(), branch, &mut triage)?;
    }
    ...
    output::print_json(&review_json(
        ...
        against,
        &comparison,
        dirty_file_count.map(|count| count > 0),
        dirty_file_count,
        changed_file_count,
        changed_files,
        changed_files_page,
...
pub async fn remote_branch_review_json(
    ...
    validate_changed_files_window(changed_files_limit)?;
    let mut preparation = prepare_remote_branch_review(path, branch, against, true).await?;
    let workspace = &preparation.workspace;
    let repo = &workspace.repo;
    let comparison = remote_branch_comparison(repo, &preparation.source, &preparation.target)?;
    let content = remote_content::prepare(
        repo,
        &workspace.remote,
        &preparation.source,
        &preparation.target,
    ...
    .await?;
    let tree_evidence =
        if content.is_some() || remote_content::snapshot_content_verified(repo, &comparison)? {
            tree_diff_evidence(repo, &comparison, branch, against)?
        } else {
            remote_content::unverified_tree_evidence(&comparison, against)
        };
    let changed_file_count = tree_evidence.changed_file_count;
    ...
    let merge_preview_data = match content.as_ref() {
        Some(content) => merge_preview_for_remote_branch_with_content(
            repo, &preparation.source, &preparation.target, Some(content),
        )?,
        None => unavailable_merge_preview(
            branch, preparation.source.parent.clone(), preparation.source.head.clone(),
            preparation.target.head.clone(), &comparison,
            "required remote review content unavailable",
            vec!["Required review content could not be verified within the bounded raw-file preparation; prediction is unavailable. No broad fetch or fallback was attempted.".into()],
        ),
    };
...
struct RemoteReviewPreparation {
    ...
    budget: crate::commands::blob_fetch::ReviewPreparationBudget,
}

/// Immutable remote branch evidence captured by one branch-list response.
/// It is deliberately not stored in the repository: remote review, diff, and
/// triage may populate the content-addressed object cache but must never move
/// local refs or replace local branch metadata.
#[derive(Clone)]
pub(crate) struct RemoteAnalysisBranch {
    pub(crate) name: String,
    pub(crate) head: Option<Hash>,
...
async fn prepare_remote_branch_review(
    ...
    let source = RemoteAnalysisBranch {
        ...
        head: source_data
            ...
            .map(Hash::from_hex)
            .transpose()?,
        parent: source_data.parent_branch.clone(),
    };
    let target_data = branches
        .iter()
        .find(|candidate| candidate.name == against)
        .ok_or_else(|| OakError::BranchNotFound(against.to_string()))?;
    let target = RemoteAnalysisBranch {
        ...
        head: target_data
            ...
            .map(Hash::from_hex)
            .transpose()?,
        parent: target_data.parent_branch.clone(),
    };
    let branch_head = source
        .head
        .as_ref()
        .ok_or_else(|| OakError::Server(format!("remote branch '{branch}' has no head")))?;

    let mut heads = vec![branch_head.clone()];
    ...
    .await?;
    if prepare_ancestry && target.head.is_some() {
        prepare_remote_review_ancestry(repo, remote, &source, &target, &mut preparation_budget)
            .await?;
    } else {
        prepare_remote_review_bases_from_snapshots(
            repo,
            remote,
...
/// Bounded frontier hydration for one pinned review pair.
/// Batch triage/train retain their existing failure-isolation behavior.
async fn prepare_remote_review_ancestry(
    repo: &oak_core::SqliteRepository,
    remote: &crate::commands::branch::RemoteIdentity,
    source: &RemoteAnalysisBranch,
    target: &RemoteAnalysisBranch,
    budget: &mut crate::commands::blob_fetch::ReviewPreparationBudget,
) -> Result<()> {
    const MAX_EXACT_COMMITS: usize = 256;
    let mut requested: std::collections::BTreeSet<String> = source
        .head
        .iter()
        .chain(target.head.iter())
        .map(ToString::to_string)
        .collect();
    loop {
        budget.check_deadline()?;
        let resolution = crate::commands::merge::resolve_merge_base_identity_from_heads(
            repo,
            &source.name,
            source.head.as_ref(),
        ...
        budget.check_deadline()?;
        let missing = match resolution {
            crate::commands::merge::MergeBaseIdentity::Unavailable(
                crate::commands::merge::MergeBaseUnavailable::IncompleteAncestry { missing },
            ) => missing,
            crate::commands::merge::MergeBaseIdentity::Commit(base) => {
                requested.insert(base.to_string());
                if requested.len() > MAX_EXACT_COMMITS {
                    return Err(OakError::Server(
                        "review preparation incomplete: cumulative exact-commit budget exceeded"
                            .into(),
                    ));
                }
                return crate::commands::blob_fetch::ensure_review_commits_with_budget(
                    repo,
                    &remote.remote_url,
                    &remote.owner,
                    &remote.repo_name,
        ...
        let missing: std::collections::BTreeSet<_> =
            missing.into_iter().map(|hash| hash.to_string()).collect();
        if missing.is_empty() || missing.iter().any(|hash| requested.contains(hash)) {
            return Err(OakError::Server(
                "review preparation incomplete: ancestry hydration made no progress".into(),
            ));
        }
        if requested.len().saturating_add(missing.len()) > MAX_EXACT_COMMITS {
            return Err(OakError::Server(
                "review preparation incomplete: cumulative exact-commit budget exceeded".into(),
            ));
        }
        requested.extend(missing.iter().cloned());
        crate::commands::blob_fetch::ensure_review_commits_with_budget(
            repo,
            &remote.remote_url,
            &remote.owner,
            &remote.repo_name,
            remote.token.as_deref(),
            &missing.into_iter().map(Hash).collect::<Vec<_>>(),
            budget,
        )
        .await?;
    }
...
pub(crate) async fn prepare_remote_review_bases_from_snapshots(
    repo: &oak_core::SqliteRepository,
    remote: &crate::commands::branch::RemoteIdentity,
    branches: &[RemoteAnalysisBranch],
    against: &RemoteAnalysisBranch,
    budget: &mut crate::commands::blob_fetch::ReviewPreparationBudget,
) -> Result<()> {
    let mut bases = std::collections::BTreeSet::new();
    for branch in branches {
        budget.check_deadline()?;
        if let crate::commands::merge::MergeBaseIdentity::Commit(base) =
            crate::commands::merge::resolve_merge_base_identity_from_heads(
                repo,
                &branch.name,
                branch.head.as_ref(),
    ...
    crate::commands::blob_fetch::ensure_review_commits_with_budget(
        ...
        &remote.repo_name,
        remote.token.as_deref(),
        &bases.into_iter().map(Hash).collect::<Vec<_>>(),
        budget,
    )
    .await
}

#[allow(dead_code)] // legacy local-ref preparation; remote paths use snapshots
pub(crate) async fn prepare_remote_review_bases(
    repo: &oak_core::SqliteRepository,
    remote: &crate::commands::branch::RemoteIdentity,
    branches: &[String],
    against: &str,
    ...
    for branch in branches {
        budget.check_deadline()?;
        if let crate::commands::merge::MergeBaseIdentity::Commit(base) =
            crate::commands::merge::resolve_merge_base_identity(
                repo,
                branch,
                against,
                against_head,
...
pub(crate) fn endpoint_diff_json(
    ...
    let hunks_truncated = if hunks && !worktree_side {
...
fn attach_patches(
    ...
    changes: &[FileChange],
    snapshot: Option<&HashMap<Hash, Vec<u8>>>,
    ...
    for summary in summaries.iter_mut() {
        ...
        let lines = if let Some(blobs) = snapshot {
            match crate::commands::diff::render_change_lines_from_snapshot(
                repo,
                &base_files,
                change,
                blobs,
                ...
                Ok(lines) => lines,
                Err(OakError::IncompleteBlobData { .. }) => {
                    // No live-file fallback for unavailable captured content.
                    summary.patch_omitted = true;
                    truncated = true;
                    continue;
                }
            ...
            crate::commands::diff::render_change_lines_with_context_and_text(
                repo,
                work_tree,
                &base_files,
                change,
...
/// Patch rendering for remote diff is derived from the pinned comparison,
/// never by resolving the same branch names through local refs. Net-merge
/// hunks retain the pinned predicted manifest and its in-memory clean-merge
/// blob overlay, so a complete remote prediction remains renderable.
#[allow(clippy::too_many_arguments)]
fn attach_remote_branch_patches_from_snapshot(
    repo: &dyn Repository,
    ...
    let base = match diff_mode {
        DiffMode::Tree => comparison.comparison_manifest.clone(),
        DiffMode::Contribution => match comparison.fork_point.as_ref() {
            Some(fork) => manifest_for_head(repo, Some(fork))?,
            None => {
                for summary in &mut *summaries {
                    summary.patch_omitted = true;
                }
                return Ok(!summaries.is_empty());
            }
        },
        DiffMode::NetMerge => {
            return attach_remote_net_merge_patches(
                repo, comparison, branch, against, max_bytes, context, force_text, summaries,
            )
        }
    };
    let changes = diff_manifests_with_content_renames(repo, &base, &comparison.branch_manifest)?;
    attach_patches_from_store(
        repo,
        &Some(base),
        &changes,
...
#[allow(clippy::too_many_arguments)]
fn attach_remote_net_merge_patches(
    repo: &dyn Repository,
    comparison: &BranchComparison,
    branch: &str,
    against: &str,
    ...
    let prediction = predict_merge_against(
        repo,
        branch,
        comparison.branch_head.as_ref(),
        comparison.parent.as_deref(),
        against,
        &comparison.branch_manifest,
        against_head,
    )?;
    let Some(base) = prediction.against_manifest else {
        for summary in &mut *summaries {
            summary.patch_omitted = true;
        }
        return Ok(!summaries.is_empty());
    ...
    attach_patches_from_store_with_overlay(
        repo,
        &Some(base),
        &changes,
        &prediction.merged_blobs,
...
#[allow(clippy::too_many_arguments)]
fn attach_patches_from_store_with_overlay(
    repo: &dyn Repository,
    base_manifest: &Option<Manifest>,
    changes: &[FileChange],
    ...
    for summary in summaries.iter_mut() {
        ...
        let Some(lines) = render_change_lines_from_store_with_overlay(
            repo,
            &base_files,
            change,
            merged_blobs,
...
fn render_change_lines_from_store_with_overlay(
    repo: &dyn Repository,
    ...
    let Some((old_bytes, new_bytes)) =
        change_bytes_from_store_with_overlay(repo, base_files, change, merged_blobs)?
    else {
        return Ok(None);
    };
    ...
    // Shared hunk-body renderer: header, prefixed lines and the
    // `\ No newline at end of file` marker, identical to `oak diff --print`.
...
fn change_bytes_from_store_with_overlay(
    repo: &dyn Repository,
    base_files: &HashMap<&str, &ManifestEntry>,
    change: &FileChange,
    merged_blobs: &HashMap<Hash, Vec<u8>>,
    ...
    let new_bytes = match change.change_type {
        ...
        _ => match &change.new_blob_hash {
            Some(hash) => match merged_blobs.get(hash) {
                ...
                None => repo.get_blob(hash)?.map(|blob| blob.content),
            },
            None => None,
        },
    };
...
fn omitted_patch_commands(stub: &str, changed_files: &[FileSummaryJson]) -> Vec<String> {
    ...
    omitted_patch_commands_with_flags(stub, changed_files, output_flags)
}

fn omitted_patch_commands_with_flags(
    stub: &str,
...
pub fn worktree_diff_json(
    ...
    worktree_diff_json_with_scope(path, paths, options, false)
}

pub(crate) fn current_branch_diff_json(
    path: &Path,
    paths: &[std::path::PathBuf],
    options: DiffJsonOptions,
) -> Result<bool> {
    worktree_diff_json_with_scope(path, paths, options, true)
}

fn worktree_diff_json_with_scope(
    path: &Path,
    paths: &[std::path::PathBuf],
    options: DiffJsonOptions,
    whole_branch: bool,
    ...
    let repo = ctx.open()?;
    let filters = crate::commands::diff::resolve_filters(path, &ctx.work_tree, paths)?;
    let branch_source = if whole_branch {
        Some(crate::commands::diff::branch_worktree_source(
            repo.as_ref(),
            &ctx.work_tree,
        )?)
    } else {
    ...
    let (changes, head, branch) = if let Some(source) = &branch_source {
        (
            source.changes.clone(),
            crate::commands::commit::resolve_effective_head(repo.as_ref(), &source.branch)?,
            Some(source.branch.clone()),
        )
    } else {
        crate::commands::commit::compute_changes_for_status(repo.as_ref(), &ctx.work_tree)?
    };
    let changes = crate::commands::diff::filter_changes(changes, &filters);
    let changed_files = if let Some(source) = &branch_source {
        summarize_manifest_changes_with_merged_blobs(
            repo.as_ref(),
            &changes,
    ...
    let changed_file_count = changed_files.len();
    let stub = if whole_branch {
        "oak diff --branch"
    } else {
        "oak diff"
    };
    let path_suffix = if whole_branch && !paths.is_empty() {
        format!(
            " -- {}",
            paths
                .iter()
    ...
    let hunks_truncated = if hunks {
        let base_manifest = match branch_source.as_ref() {
            Some(source) => Some(source.base_manifest.clone()),
            None => match head.as_ref() {
        ...
        attach_patches(
            ...
            &base_manifest,
            &changes,
            branch_source.as_ref().map(|source| &source.worktree_blobs),
            &mut changed_files,
            max_bytes,
            context,
            force_text,
    ...
    {
        recommended.push(format!("{stub} --json{path_suffix}"));
    }
    if hunks_truncated {
        if whole_branch {
            let mut flags = "--json".to_string();
            // A single-file retrieval removes the exhausted byte cap, but
            // preserves rendering choices and the exact contribution scope.
            append_hunk_flags(&mut flags, true, None, context, force_text);
            let mut command_files = changed_files.clone();
            if path.canonicalize()? != ctx.work_tree {
                for file in &mut command_files {
                    file.path = ctx
                        .work_tree
                        .join(&file.path)
                        .to_string_lossy()
            ...
            recommended.extend(omitted_patch_commands_with_flags(
                stub,
                &command_files,
                &flags,
            ));
        } else {
            recommended.extend(omitted_patch_commands(stub, &changed_files));
        }
    }
    if dirty {
        if !hunks {
            let mut command = format!("{stub} --json --hunks");
            if whole_branch {
                command = format!("{stub} --json");
                append_hunk_flags(&mut command, true, None, context, force_text);
            }
            command.push_str(&path_suffix);
            recommended.push(command);
        }
        recommended.push(format!("{stub} --print{path_suffix}"));
        if !whole_branch {
    ...
    let lineage = if let Some(source) = &branch_source {
        let own_head = repo.get_branch_head(&source.branch)?;
        LineageJson {
            branch_parent: Some(source.parent.clone()),
            branch_own_head: own_head.as_ref().map(ToString::to_string),
            branch_effective_head: head.as_ref().map(ToString::to_string),
            against_head: crate::commands::commit::resolve_effective_head(
                repo.as_ref(),
                &source.parent,
            )?
            .as_ref()
            .map(ToString::to_string),
            comparison_head: source.fork.as_ref().map(ToString::to_string),
            comparison_source: "working_tree_vs_fork_point".to_string(),
            fork_point: source.fork.as_ref().map(ToString::to_string),
            branch_commit_count: repo.count_commits_for_branch(&source.branch)?,
    ...
    output::print_json(&DiffJson {
        schema_version: SCHEMA_VERSION,
        kind: if whole_branch {
            "current_branch_diff"
        } else {
            "working_tree_diff"
        },
        diff_mode: if whole_branch {
            "contribution"
        } else {
            "working_tree"
        },
        ...
        against: Some(
            branch_source
                .as_ref()
                .map_or("HEAD", |s| s.parent.as_str())
                .to_string(),
        ),
        branch_head: head.as_ref().map(ToString::to_string),
        parent: branch_source.as_ref().map(|s| s.parent.clone()),
        changed_file_count,
        changed_files,
        changed_files_page,
        files_identical_to_against: None,
        ...
        conflict_files: Vec::new(),
        merge_lineage_evidence: Some(lineage),
        caveats: if whole_branch {
            vec![
                "Contribution uses the locally available fork and a non-persisting worktree scan; no remote fetch or merge prediction was performed.".to_string(),
                "Sparse checkout and path permissions limit the visible scope; excluded paths are not reported as deletions.".to_string(),
                "Branch heads and branch_commit_count describe local committed state, not the dirty result or a pinned atomic filesystem snapshot.".to_string(),
            ]
        } else {
            vec!["Working-tree diff was computed from local filesystem state only.".to_string()]
        },
...
pub(crate) fn merge_preview_for_remote_branch(
    repo: &dyn Repository,
    source: &RemoteAnalysisBranch,
    target: &RemoteAnalysisBranch,
) -> Result<MergePreviewJson> {
    merge_preview_for_remote_branch_with_content(repo, source, target, None)
}

fn merge_preview_for_remote_branch_with_content(
    repo: &dyn Repository,
    source: &RemoteAnalysisBranch,
    target: &RemoteAnalysisBranch,
    content: Option<&remote_content::VerifiedContent>,
) -> Result<MergePreviewJson> {
    let comparison = remote_branch_comparison(repo, source, target)?;
    if source.parent.as_deref() != Some(target.name.as_str()) {
        return Ok(unavailable_merge_preview(
            ...
            target.head.clone(),
            &comparison,
            "remote target does not match the source branch's declared parent",
            vec![
                "Remote preview refused to substitute a local or differently named target head."
                    .to_string(),
            ],
        ));
    }
    merge_preview_from_evidence(
        ...
        target.head.clone(),
        comparison.clone(),
        comparison.branch_commit_count,
        &merge_safety::TargetDataOrigin::LiveFetched {
            target: &target.name,
            head: target.head.as_ref(),
        },
        content,
    )
}

/// Conflict preview over explicit evidence. Remote callers supply the heads
/// and parent captured from the remote instead of consulting local refs.
#[allow(clippy::too_many_arguments)] // explicit immutable evidence prevents remote ref fallback
fn merge_preview_from_evidence(
    repo: &dyn Repository,
    branch: &str,
    parent: Option<String>,
    ...
    let mut caveats = comparison.caveats.clone();
    caveats.push(match target_origin {
        merge_safety::TargetDataOrigin::LocalOnly => "No remote fetch was performed; conflict prediction uses only local commits, manifests, and blobs.",
        merge_safety::TargetDataOrigin::LiveFetched { .. } => "Conflict prediction uses the pinned immutable cache from the preceding remote fetch; it performs no additional fetch.",
    }.to_string());
    ...
    let base_manifest = if let Some(content) = content {
        content.base().clone()
    } else {
        match crate::commands::merge::resolve_merge_base_from_heads(
            repo,
            branch,
            branch_head.as_ref(),
            parent.as_deref(),
...
fn review_recommendations(
    ...
    // Structural merge safety cannot override an incomplete contribution
    // assessment (for example, missing file bytes). Keep suggested actions
    // consistent with the verdict even when the predicted tree merge is clean.
    if matches!(
        triage_action,
        RecommendedAction::Review | RecommendedAction::Unknown
    ) {
...
struct PredictedMergeAgainst {
    ...
    /// Clean content-merge bytes are not stored; hunks must consume this
    /// pinned in-memory overlay rather than resolving a local branch ref.
    merged_blobs: HashMap<Hash, Vec<u8>>,
...
fn compute_branch_diff(
    ...
    match diff_mode {
        ...
        DiffMode::Contribution => {
            ...
            } else if comparison.empty_fork_available {
                (
                    Manifest::empty(),
                    None,
                    "unborn_main_empty_tree".to_string(),
                )
            } else {
                extra_caveats.push(contribution_unavailable_message(branch, against));
                return Ok(unavailable_branch_diff(
                    None,
                    against,
                    "contribution_unavailable",
        ...
        DiffMode::NetMerge => {
            ...
            let changed_files = summarize_manifest_changes_with_merged_blobs(
                repo,
                &changes,
                &prediction.merged_blobs,
            )?;
    ...
    match diff_mode {
        ...
        DiffMode::Contribution => {
            let (base, source) = match comparison.fork_point.as_ref() {
                ...
                None if comparison.empty_fork_available => {
                    (Manifest::empty(), "unborn_main_empty_tree")
                }
                None => return Err(contribution_unavailable_error(branch, against)),
            };
            Ok(EndpointManifests {
                base,
                target: comparison.branch_manifest,
...
fn predict_merge_against(
    ...
    let resolution = crate::commands::merge::resolve_merge_base_from_heads(
        repo,
        branch,
        branch_head,
        branch_parent,
    ...
    Ok(PredictedMergeAgainst {
        ...
        merged_manifest: Manifest::new(merged_entries),
        merged_blobs: prediction.merged_blobs,
        conflict_paths,
        caveats: Vec::new(),
    })
}
...
pub(crate) fn branch_comparison(
    ...
    if repo.get_branch(against)?.is_none() && !is_rowless_branch_name(repo, against)? {
        return Err(OakError::BranchNotFound(against.to_string()));
    }
    ...
    let empty_fork_available = against == DEFAULT_BRANCH
        && comparison_head.is_none()
        && match branch_head.as_ref() {
            Some(head) => collect_ancestors(repo, head)?.missing_edges.is_empty(),
            None => true,
        };
...
/// Compare two heads captured from the same remote branch-list response.
/// This deliberately has no local-ref fallbacks: doing so would turn an
/// unpushed local commit into remote review evidence.
pub(crate) fn remote_branch_comparison(
    repo: &dyn Repository,
    branch: &RemoteAnalysisBranch,
    against: &RemoteAnalysisBranch,
) -> Result<BranchComparison> {
    let mut caveats = Vec::new();
    let branch_head = branch.head.clone();
    let against_head = against.head.clone();
    if branch_head.is_none() {
        caveats.push(format!(
            "Remote branch '{}' has no head; its side of the comparison is unavailable.",
            branch.name
        ));
    }
    if against_head.is_none() {
        caveats.push(format!(
            "Remote against branch '{}' has no head; no local-head fallback was used.",
            against.name
        ));
    }
    let (branch_manifest, branch_snapshot_available) =
        match manifest_for_head(repo, branch_head.as_ref()) {
            Ok(manifest) if branch_head.is_some() => (manifest, true),
            Ok(_) => (Manifest::empty(), false),
            Err(err) if is_local_merge_data_unavailable(&err) => {
                caveats.push(format!(
                    "Remote branch '{}' snapshot is incomplete in the immutable cache.",
                    branch.name
                ));
                (Manifest::empty(), false)
            }
    ...
    let (comparison_manifest, comparison_snapshot_available) =
        match manifest_for_head(repo, against_head.as_ref()) {
            Ok(manifest) if against_head.is_some() => (manifest, true),
            Ok(_) => (Manifest::empty(), false),
            Err(err) if is_local_merge_data_unavailable(&err) => {
                caveats.push(format!(
                    "Remote against branch '{}' snapshot is incomplete in the immutable cache.",
                    against.name
                ));
                (Manifest::empty(), false)
            }
    ...
    let fork_point = match (branch_head.as_ref(), against_head.as_ref()) {
        (Some(left), Some(right)) => match first_common_ancestor(repo, left, right)? {
            ForkPointResolution::Found(hash) => Some(hash),
            ForkPointResolution::Unavailable { missing } => {
                caveats.push(format!("Remote lineage is incomplete; fork point is unavailable because commit(s) {} are missing.", format_missing_hashes(&missing)));
                None
            }
            ForkPointResolution::None => None,
        },
    ...
    let branch_commit_count = match branch_head.as_ref() {
        Some(head) => {
            let walk = collect_ancestors(repo, head)?;
            let count =
                count_remote_branch_commits(&walk, &branch.name, |hash| repo.get_commit(hash))?;
            if !walk.missing_edges.is_empty() {
                caveats.push("branch_commit_count is a lower bound from available remote ancestry; older commits are missing.".to_string());
            }
            count
        }
        None => 0,
    ...
    Ok(BranchComparison {
        ...
        branch_own_head: branch_head.clone(),
        branch_head,
        against_head,
        comparison_head: against.head.clone(),
        comparison_source: "remote_branch_list_head".to_string(),
        fork_point,
        empty_fork_available: false,
        branch_commit_count,
        branch_manifest,
        branch_snapshot_available,
        comparison_manifest,
...
fn count_remote_branch_commits(
    walk: &ReviewAncestorWalk,
    branch: &str,
    mut load: impl FnMut(&Hash) -> Result<Option<oak_core::Commit>>,
) -> Result<usize> {
    let mut count = 0;
    for hash in &walk.ancestors {
        let commit = load(hash)?.ok_or_else(|| OakError::IncompleteCommitData {
            context: format!("remote branch '{branch}' contribution count"),
    ...
    // A positive observed count is sufficient to rule out an empty branch,
    // even in a shallow cache. Zero is not proof when history is incomplete.
    if count == 0 && !walk.missing_edges.is_empty() {
        return Err(OakError::IncompleteAncestry {
            left: branch.to_string(),
            right: "remote contribution history".to_string(),
            missing: format_missing_hashes(
                &walk
                    .missing_edges
                    .iter()
...
/// without picking up unrelated movement on `against`. An empty base is valid
/// only for complete local history against unborn main; unavailable forks error.
/// Exact fork-point identity plus its manifest. Status JSON and diff use this to
/// label committed branch changes with the same base as contribution diff,
/// rather than inventing a second fork-point algorithm.
pub(crate) fn branch_fork_base_snapshot(
    repo: &dyn Repository,
    branch: &str,
    against: &str,
) -> Result<(Option<Hash>, Manifest, Vec<String>)> {
    let comparison = branch_comparison(repo, branch, against)?;
    match comparison.fork_point.as_ref() {
        ...
        None if comparison.empty_fork_available => {
            Ok((None, Manifest::empty(), comparison.caveats))
        }
        None => Err(contribution_unavailable_error(branch, against)),
    }
}

fn contribution_unavailable_message(branch: &str, against: &str) -> String {
    format!("Fork point between branch '{branch}' and '{against}' is unavailable; contribution diff cannot be computed. Fetch the required history with `oak fetch` or use explicit tree mode to compare snapshots without claiming branch contribution.")
}

fn contribution_unavailable_error(branch: &str, against: &str) -> OakError {
    OakError::IncompleteCommitData {
        context: format!("contribution diff of '{branch}' against '{against}'"),
        missing: contribution_unavailable_message(branch, against),
    }
}
...
fn file_summary(
    ...
    if missing_blob {
        return FileSummaryJson {
            ...
            content_unavailable_reason: Some("missing_blob"),
...
fn predict_conflicts(
    ...
    predict_conflicts_with(base_manifest, conflicts, branch_name, parent_name, |hash| {
        bytes_for_hash(repo, Some(hash))
    })
}

fn predict_conflicts_with(
    base_manifest: &Manifest,
    conflicts: Vec<MergeConflict>,
    branch_name: &str,
    parent_name: &str,
    load: impl Fn(&Hash) -> Result<Option<Vec<u8>>>,
) -> Result<MergePrediction> {
    ...
    } in conflicts
    {
        match (branch_entry, parent_entry) {
            (Some(be), Some(pe)) => {
                let branch_bytes = load(&be.blob_hash)?.unwrap_or_default();
                let parent_bytes = load(&pe.blob_hash)?.unwrap_or_default();
                let base_entry = base_manifest.get(&path);
                let base_text = base_entry
                    .and_then(|entry| load(&entry.blob_hash).ok().flatten())
...
pub(crate) fn remote_branch_triage_evidence(
    repo: &dyn Repository,
    source: &RemoteAnalysisBranch,
    target: &RemoteAnalysisBranch,
    include_merge_preview: bool,
    ...
    let comparison = remote_branch_comparison(repo, source, target)?;
    let tree_evidence = tree_diff_evidence(repo, &comparison, &source.name, &target.name)?;
    let merge_preview = if include_merge_preview {
        merge_preview_for_remote_branch(repo, source, target)?
    } else {
        summary_merge_preview(&source.name, &comparison)
    };
    let triage = branch_triage_json(
        repo,
        &source.name,
