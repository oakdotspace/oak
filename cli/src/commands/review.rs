...
use oak_core::{
    three_way_merge_manifests, ChangeType, DiffLine, FileChange, FileDiff, Hash, Manifest,
    ManifestEntry, MergeConflict, OakError, Repository, Result, SqliteRepository, DEFAULT_BRANCH,
};
use serde::Serialize;
...
pub fn branch_diff_json(
    ...
    output::print_json(&DiffJson {
        ...
        caveats,
        recommended_next_commands: branch_diff_recommendations(branch, against, diff_mode),
    })
}
...
fn review_page_command(
    ...
    format!(
        ...
        if remote { " --remote" } else { "" },
        if merge_preview { " --merge-preview" } else { "" }
    )
}
...
fn review_recommendations(
    ...
    let mut commands = vec![format!("oak switch {branch}")];
    match preview.clean {
...
fn branch_diff_recommendations(branch: &str, against: &str, diff_mode: DiffMode) -> Vec<String> {
    let mut commands = vec![
        format!("oak branch review {branch} --merge-preview --json"),
        format!("oak switch {branch}"),
    ];
    if diff_mode != DiffMode::Tree {
        commands.insert(
            0,
            format!("oak branch diff {branch} --against {against} --diff-mode tree --json"),
        );
    }
    if diff_mode != DiffMode::NetMerge {
        commands.insert(
            0,
            format!("oak branch diff {branch} --against {against} --diff-mode net-merge --json"),
        );
    }
    commands
}
