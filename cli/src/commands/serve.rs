...
use oak_core::{
    ...
    CloseReason, Commit, FileChange, Hash, OakError, Repository, Result, SqliteRepository, Tree,
};
...
fn branch_to_pull(b: &Branch) -> BranchPullData {
    BranchPullData {
        ...
        close_reason: b.close_reason.map(|r| r.as_str().to_string()),
    }
}
...
async fn push(
    ...
    blocking(move || {
        ...
        // row), so for a row that already exists the pushed description and
        // status must be applied explicitly — otherwise a re-push or a
        // metadata-only `oak desc` sync is silently ignored.
        if let Some(bd) = &req.branch {
            ...
            if let Some(reason) = branch.close_reason {
        ...
        for b in &req.blobs {
            ...
            if b.chunks.is_empty() {
                ...
                // The chunks individually hash-verify on upload, but nothing
                // yet proves they reassemble into the blob the client claims
                // — a client that chunked the wrong bytes (e.g. a compressed
                // frame) would otherwise poison the store under a plaintext
                // hash that every future pull trusts.
                if oak_core::hash_bytes(&content) != blob_hash {
                    return Err(ServeError::bad_request(format!(
                        "blob {} reassembled from its chunks does not hash to its claimed hash",
                        b.hash
                    )));
                }
                repo.store_blob(&Blob {
        ...
        // Hash-position strings in pushed commits are untrusted wire data:
        // they get embedded verbatim in descendants' hash preimages, so
        // anything that isn't real hex is rejected, not stored.
        let wire_hash =
            |s: &String| Hash::from_hex(s).map_err(|e| ServeError::bad_request(e.to_string()));
        for c in &req.commits {
            let mut files: Vec<FileChange> = Vec::with_capacity(c.files.len());
            for f in &c.files {
                files.push(FileChange {
                    ...
                    old_blob_hash: f.old_blob_hash.as_ref().map(wire_hash).transpose()?,
                    new_blob_hash: f.new_blob_hash.as_ref().map(wire_hash).transpose()?,
                    old_path: f.old_path.clone(),
                    old_mode: None,
                    new_mode: None,
                });
            ...
            let commit = Commit {
                hash: wire_hash(&c.hash)?,
                branch_name: c.branch_name.clone(),
                parent_hash: c.parent_hash.as_ref().map(wire_hash).transpose()?,
                merge_parent_hash: c.merge_parent_hash.as_ref().map(wire_hash).transpose()?,
                manifest_hash: wire_hash(&c.manifest_hash)?,
                author: c.author.clone(),
                message: c.message.clone(),
...
mod tests {
    use super::*;
    use chrono::Utc;
    use oak_core::protocol::BranchPushData;
