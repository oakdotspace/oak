...
use super::credentials::{effective_token, effective_token_for_worktree};
...
pub async fn enable(
    ...
    let token = effective_token_for_worktree(&remote, work_path);
    ...
    output::print_line(&format!("  branch:     {}", site.branch));
    output::print_line(&format!("  source dir: {}", site.source_dir));
    Ok(())
}
...
pub async fn show(
    ...
    output::print_line(&format!("  url:        {}", site.url));
    output::print_line(&format!("  repo:       {}", site.repo_name));
    output::print_line(&format!("  branch:     {}", site.branch));
    output::print_line(&format!("  source dir: {}", site.source_dir));
    output::print_line(&format!("  updated:    {}", site.updated_at));
    Ok(())
}
...
pub async fn list(work_path: &Path, remote_arg: Option<&str>) -> Result<()> {
    let remote = resolve_remote(work_path, remote_arg)?;
    // Listing spans every account-visible organization, so it intentionally
    // uses the account credential rather than a current repository identity.
    let token = effective_token(&remote, None);
    if token.is_none() {
    ...
    output::print_line("");
    for s in list.sites {
        output::print_line(&format!("  {}  →  {}", s.organization_slug, s.url));
        output::print_line(&format!(
            "    repo: {}, branch: {}, source: {}",
