...
branch → end the mount after publish succeeds. This is a **multi-leg saga,
not a rollback-atomic transaction**: if a leg fails, the mount stays intact
and JSON names the completed phase. Explicit rejections may name a manual retry;
an unconfirmed publication has a null retry command and supplies read-only
reconciliation commands instead. Its bounded local operation record survives a
restart and blocks non-force teardown; inspect it with `oak agent state
--refresh --json --compact`. Do not start over blindly.
...
oak space inventory [DIR] --json  # bounded local checkout/mount observations
...
`oak space inventory` observes an explicit directory tree (default cwd) and
registered mounts beneath it. It does not require a `.oak-space` marker or
infer ownership from a directory name. `--max-entries` bounds examined
filesystem entries and mount registrations (default 1000, maximum 10000);
`--max-depth` bounds directory descent (default 3, maximum 16). Symlinks,
repository internals, registered mount contents, and detected filesystem
boundaries are not traversed. Git-backed checkouts are identified but their
state is not inspected in this initial slice.

JSON v1 reports `local_observation` consistency. `complete` covers directory
discovery only: coverage gaps identify skipped/error/budget-limited areas.
Checkout metadata is read in a SQLite read transaction without migrations,
chmod, database creation, or journal-mode changes. Normal WAL coordination
can use shared-memory sidecars; this is not a raw-filesystem-purity promise.
An unsupported or unreadable schema remains unavailable. Mount configuration
is local evidence, and a live recorded PID does not prove process ownership.

Working-tree changes, unpublished commits, ignored/untracked artifacts,
process ownership, and remote publication remain explicitly unverified.
The command does not hydrate content, contact servers, delete anything, or
certify cleanup safety. Full publication checks, artifact/process accounting,
and recoverable retirement are later work; inventory alone does not complete
the broader fb-168 cleanup workflow or the whole fb-371 task-safety request.
