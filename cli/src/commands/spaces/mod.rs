...
pub async fn new(spec: &str, dest: Option<&Path>, cwd: &Path, remote_url: &str) -> Result<()> {
    ...
    match fetch_org_repos(remote_url, &org).await {
        Ok(repos) if !repos.is_empty() => {
            ...
            for repo in &repos {
                output::print_line(&format!("  {org}/{repo}"));
            }
        }
    ...
    output::print_line("  oak space repos");
...
pub async fn repos(org: Option<&str>, cwd: &Path, remote_url: &str) -> Result<()> {
    ...
    output::print_line(&format!("  oak mount {org}/<repo> ./<task-slug>/<repo>"));
    Ok(())
}
...
fn mount_in_flight(dest: &Path) -> Option<String> {
    ...
    match crate::publication_state::AttemptStore::mount(&state_dir).has_records() {
        Ok(true) => return Some("unresolved publication outcome".to_string()),
        Ok(false) => {}
        Err(_) => return Some("unverifiable publication state".to_string()),
    }
