...
fn ensure_manifest_blobs_available(
    ...
    for entry in &manifest.entries {
        if repo.has_blob(&entry.blob_hash)? || restricted.contains(entry.blob_hash.as_str()) {
            continue;
        }
...
pub fn update_working_dir(
    ...
    let report = crate::materialize::apply_manifest(
        ...
        crate::materialize::ApplyOpts {
            missing_blobs: if restricted.is_empty() {
    ...
    let (restricted_skips, other_skips): (Vec<&String>, Vec<&String>) = report
        .skipped
        .iter()
