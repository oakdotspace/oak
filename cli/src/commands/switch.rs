...
impl WorktreePolicy {
    pub fn from_discard_flag(discard: bool) -> Self {
        if discard {
            Self::Discard
        } else {
            Self::Carry
...
pub fn run(path: &Path, name: Option<&str>, detach: bool) -> Result<()> {
    ...
    if name.is_none() && !is_interactive() {
        return Err(OakError::Io(std::io::Error::other(
            "oak switch needs a branch name in non-interactive mode. Use `oak switch -c` to create a generated branch from latest available main, `oak switch -c --discard` to start clean, or `oak switch NAME` to switch to an existing branch.",
        )));
    }
...
fn create_from_main(path: &Path, name: Option<&str>, policy: WorktreePolicy) -> Result<()> {
    ...
    let repo = ctx.open()?;
    let manifest = manifest_for_head(repo.as_ref(), main_head.as_ref())?;
    ...
    let materialize_main = discard || (!dirty && main_head.is_some());
    if materialize_main {
        let lock = WorkdirLock::acquire(&ctx.oak_dir)?;
        update_working_dir(&lock, &root, repo.as_ref(), &manifest)?;
    }
    ...
    if let Some(ref head) = main_head {
