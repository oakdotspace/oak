...
pub(crate) async fn sync_from_parent_for_pull(
    ...
    // Sparse repositories retain complete manifest identities while storing
    // content only for the active cone. Merge planning may therefore compare
    // out-of-cone hashes, but must load bytes only when resolving a real
    // content conflict.
    ...
    let parent_head = if parent_name == "main" {
        match fetch_parent_from_server_with_remote(&repo, &parent_name, remote_override).await {
            Ok(head) => head,
            Err(err) => {
                record_parent_integrity_failure_if_any(&repo, &parent_name, &err);
                return Err(err);
            }
        }
    } else {
    ...
    let parent_manifest = match &parent_commit {
        Some(c) => super::merge::manifest_for_commit_or_unavailable_in_cone(
            &repo,
            c,
            &branch_name,
            &parent_name,
        ...
        && parent_head.as_ref() != branch_head.as_ref()
    {
        ...
        let branch_seed_manifest = if let Some(branch_head) = &branch_head {
            ...
            Some(super::merge::manifest_for_commit_or_unavailable_in_cone(
                &repo,
                &branch_commit,
                &branch_name,
                &parent_name,
    ...
    let branch_manifest = super::merge::manifest_for_commit_or_unavailable_in_cone(
        &repo,
        &branch_commit,
        &branch_name,
        &parent_name,
    ...
    let base_manifest = if cone.is_some() {
        // The manifest-only resolver proves the same ancestry and tree
        // identity without requiring every excluded blob up front. Conflict
        // resolution below still requires every byte it actually reads.
        super::merge::find_lca_manifest_for_lazy_cache(
            &repo,
            &branch_name,
            &parent_name,
            parent_head.as_ref(),
        ...
        super::merge::find_lca_manifest_with_parent(
            &repo,
            &branch_name,
            &parent_name,
            parent_head.as_ref(),
    ...
    if let Some(cone) = cone.as_ref() {
        let mut hidden_conflicts: Vec<&str> = conflict_paths
            .iter()
            .chain(binary_conflict_paths.iter())
            .map(String::as_str)
            .filter(|path| !cone.covers(path))
            .collect();
        hidden_conflicts.sort_unstable();
        if !hidden_conflicts.is_empty() {
            return Err(OakError::MergeFailed(format!(
                "merge conflict outside the active sparse cone: {}. Widen the cone with `oak sparse add <path>` and retry so the conflict can be materialized and resolved",
                hidden_conflicts.join(", ")
            )));
        }
    }
...
pub(crate) async fn fetch_parent_from_server(
    ...
    match fetch_parent_from_server_with_remote(repo, parent_name, None).await {
        Ok(head) => Ok(head),
        Err(err) => {
            record_parent_integrity_failure_if_any(repo, parent_name, &err);
            Err(err)
...
pub(crate) async fn fetch_parent_from_server_with_remote(
    ...
    fetch_parent_from_server_with_remote_quiet(repo, parent_name, remote_override, false).await
}

pub(crate) async fn fetch_parent_from_server_with_remote_quiet(
    repo: &dyn Repository,
    parent_name: &str,
    remote_override: Option<&str>,
    quiet: bool,
    ...
    if quiet {
        output::vlog(&format!("Updating '{parent_name}' from {remote}…"));
    } else {
    ...
    // The head of the parent branch was committed *on* the parent branch, so
    // its own branch name and `parent_name` agree; still prefer the server's
    // field when present since branch_name is part of the hash preimage.
    let head_branch_name = meta.branch_name.unwrap_or_else(|| parent_name.to_string());
...
async fn backfill_parent_ancestor_chain(
    ...
    while let Some(ancestor_hash) = queue.pop_front() {
        if seen.contains(ancestor_hash.as_str()) {
            continue;
        }
        // Bound all unique visited rows, including locally cached history.
        // Duplicate edges must not exhaust the budget at the boundary.
        if seen.len() >= MAX_CHAIN_BACKFILL {
            report.incomplete = true;
            break;
        }
        seen.insert(ancestor_hash.to_string());
        // A cached commit may itself be the boundary of a shallow clone.
        // Its presence proves this row, not the closure of either parent edge.
        if let Some(commit) = repo.get_commit(&ancestor_hash)? {
            queue.extend(
        ...
        // The ancestor's own branch name is required: branch_name is part of
        // the commit hash preimage, and merge-parent edges reach tips of
        // *other* branches. Stamping `parent_name` here would make every
        // merged feature-branch tip fail verification, so when the server
        // omits the field the gap is left instead of guessed at.
        let Some(branch_name) = ameta.branch_name else {
            report.incomplete = true;
            continue;
        };
...
fn record_parent_checked(repo: &dyn Repository, parent_name: &str) {
    ...
    let _ = repo.set_metadata(MetadataKey::MainLastFetchIntegrityError, "");
}

pub(crate) fn record_parent_integrity_failure_if_any(
    repo: &dyn Repository,
    parent_name: &str,
    err: &OakError,
) {
    if parent_name != oak_core::DEFAULT_BRANCH || !is_parent_fetch_integrity_failure(err) {
        return;
    }
    let _ = repo.set_metadata(
        MetadataKey::MainLastFetchIntegrityError,
        &format!("{err}; local main may be stale or unsafe until a verified fetch succeeds"),
    );
}

fn is_parent_fetch_integrity_failure(err: &OakError) -> bool {
    if matches!(err, OakError::InvalidHash(_)) {
        return true;
    }
    let message = err.to_string().to_ascii_lowercase();
    message.contains("hash mismatch")
        || message.contains("does not reproduce its hash")
        || message.contains("canonical tree objects")
}

pub(crate) fn parent_fetch_integrity_error(
    repo: &dyn Repository,
    parent_name: &str,
...
struct CommitInfoEntry {
    hash: String,
    /// The branch the commit was created on. Part of the commit hash
    /// preimage, so a backfilled ancestor can only verify under this exact
    /// name — a merge-parent ancestor from a feature branch never verifies
    /// under the parent branch's name.
    #[serde(default)]
...
struct RemoteCommitMeta {
    /// The branch the commit was created on — part of the commit hash
    /// preimage (see `Commit::rehydrate_verified`), so synthesized rows must
    /// carry this exact name for verification to succeed. `None` if the
    /// server didn't return it or the fetch failed.
    branch_name: Option<String>,
    parent_hash: Option<Hash>,
...
/// plus `branch_name` / `author` / `message` / `timestamp` — all of which are
/// inputs to the commit content hash (see `Commit::with_timestamp`), so the caller
...
mod canonical_main_tests {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn fetch_repairs_history_beyond_a_cached_shallow_boundary() {
        check_fetch_repairs_cached_boundary(false, false, true).await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn fetch_repairs_merge_parent_history_beyond_a_cached_shallow_boundary() {
        check_fetch_repairs_cached_boundary(true, false, true).await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn fetch_repairs_cached_boundary_when_main_head_is_already_hydrated() {
        check_fetch_repairs_cached_boundary(false, true, true).await;
        check_fetch_repairs_cached_boundary(true, true, true).await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn fetch_keeps_missing_remote_ancestry_beyond_cached_boundary_incomplete() {
        check_fetch_repairs_cached_boundary(false, true, false).await;
    }

    async fn check_fetch_repairs_cached_boundary(
        merge_parent: bool,
        head_cached: bool,
        remote_has_ancestor: bool,
    ) {
        let temp = TempDir::new().unwrap();
        let server = MockServer::start().await;
        let repo = open_linked_worktree_repo(&temp, &server.uri());
        let (manifest, trees) = tree_fixture(
            &repo,
            vec![ManifestEntry {
                path: "tracked.txt".into(),
                blob_hash: repo.put_blob(b"new main\n".to_vec()).unwrap(),
                mode: FileMode::Regular,
            }],
        );
        let boundary_manifest = repo
            .put_manifest(vec![ManifestEntry {
                path: "tracked.txt".into(),
                blob_hash: repo.put_blob(b"shallow boundary\n".to_vec()).unwrap(),
                mode: FileMode::Regular,
            }])
            .unwrap();
        let ancestor = main_commit_with_manifest(Manifest::empty().hash, None, "ancestor");
        let boundary = Commit::with_timestamp(
            "main".into(),
            (!merge_parent).then(|| ancestor.hash.clone()),
            merge_parent.then(|| ancestor.hash.clone()),
            boundary_manifest,
            "<remote>".into(),
            Some("shallow boundary".into()),
            Vec::new(),
            chrono::DateTime::from_timestamp(1_700_000_300, 0).unwrap(),
        )
        .unwrap();
        let head = main_commit_with_manifest(manifest, Some(boundary.hash.clone()), "new main");
        repo.store_commit(&boundary).unwrap();
        repo.store_branch(&oak_core::Branch::new("main".into(), None, None))
            .unwrap();
        if head_cached {
            repo.store_commit(&head).unwrap();
        }
        repo.set_branch_head(
            "main",
            if head_cached {
                &head.hash
            } else {
        ...
        .unwrap();
        repo.set_branch_head("topic", &boundary.hash).unwrap();
        repo.set_current_branch("topic").unwrap();
        repo.set_head(&boundary.hash).unwrap();
        std::fs::write(temp.path().join("tracked.txt"), "shallow boundary\n").unwrap();
        std::fs::write(temp.path().join("scratch.txt"), "keep local work\n").unwrap();
        let remote_commits = if remote_has_ancestor {
            vec![&head, &ancestor]
        } else {
            vec![&head]
        };
        mount_main_head_with_commits(&server, &head, remote_commits, trees).await;

        output::begin_capture();
        super::super::fetch::run(temp.path(), Some(&server.uri()))
            .await
            .unwrap();
        let captured = output::end_capture();

        assert_eq!(
            repo.get_commit(&ancestor.hash).unwrap().is_some(),
            remote_has_ancestor,
            "fetch must repair a cached boundary only with verified remote evidence"
        );
        assert_eq!(repo.get_branch_head("main").unwrap(), Some(head.hash));
        assert_eq!(repo.get_head().unwrap(), Some(boundary.hash.clone()));
        assert_eq!(repo.get_branch_head("topic").unwrap(), Some(boundary.hash));
        assert_eq!(
        ...
        if !remote_has_ancestor {
            assert!(
                captured.contains("still incomplete"),
                "a remote omission must remain explicit: {captured}"
            );
            return;
        }
        ...
        assert_eq!(
            server
                ...
                .count(),
            metadata_requests,
            "a fully repaired cached graph must need no additional commit metadata request"
        );
    }
    ...
    /// Regression: backfilled merge-parent ancestors live on *other*
    /// branches, and branch_name is part of the commit hash preimage.
    /// Stamping the parent branch's name on them (the old behavior) made
    /// every merged feature-branch tip fail verification with "commit
    /// metadata does not reproduce its hash", aborting the whole fetch.
    #[tokio::test(flavor = "current_thread")]
    async fn fetch_parent_backfills_merge_parent_ancestor_under_its_own_branch_name() {
        let temp = TempDir::new().unwrap();
        let server = MockServer::start().await;
        let repo = open_linked_repo(&temp, &server.uri());

        // Local history: the old main head both sides fork from.
        let old_blob = repo.put_blob(b"old\n".to_vec()).unwrap();
        let old_manifest = repo
            .put_manifest(vec![ManifestEntry {
                path: "tracked.txt".to_string(),
        ...
        // Server-only: a feature-branch tip whose hash was sealed under its
        // own branch name, reachable from main's head via merge_parent.
        let feature_blob = repo.put_blob(b"feature\n".to_vec()).unwrap();
        let feature_manifest = repo
            .put_manifest(vec![ManifestEntry {
                path: "tracked.txt".to_string(),
                blob_hash: feature_blob,
                mode: FileMode::Regular,
            }])
            .unwrap();
        let feature_tip = Commit::with_timestamp(
            "feature-x".to_string(),
            Some(old_head.clone()),
            None,
            feature_manifest,
        ...
        // The squash-merge head on main carrying the merge_parent edge.
        let head_blob = repo.put_blob(b"new\n".to_vec()).unwrap();
        let (head_manifest, wire_trees) = tree_fixture(
            &repo,
            vec![ManifestEntry {
        ...
        let head_commit = Commit::with_timestamp(
            "main".to_string(),
            Some(old_head),
            Some(feature_tip.hash.clone()),
            head_manifest,
            "<remote>".to_string(),
            Some("merge feature-x".to_string()),
            Vec::new(),
            chrono::DateTime::from_timestamp(1_700_000_300, 0).unwrap(),
        )
        .unwrap();
        ...
        repo.set_branch_head("main", &head_commit.hash).unwrap();
        assert!(
            repo.get_commit(&feature_tip.hash).unwrap().is_none(),
            "test setup should leave the merge-parent edge dangling"
        );

        mount_main_head_with_commits(
            &server,
            &head_commit,
            vec![&head_commit, &feature_tip],
            wire_trees,
        )
        .await;
        ...
        let fetched = fetch_parent_from_server_with_remote(&repo, "main", Some(&server.uri()))
            .await
            .expect("merged feature tip must verify under its own branch name");
        let captured = output::end_capture();

        assert_eq!(fetched, Some(head_commit.hash));
        let stored = repo
            .get_commit(&feature_tip.hash)
            .unwrap()
            .expect("fetch should backfill the merged feature-branch tip");
        assert_eq!(
            stored.branch_name, "feature-x",
            "backfilled ancestor must keep the branch name its hash was sealed under"
        );
        assert_eq!(stored.hash, feature_tip.hash);
        assert!(
            captured.contains("linked 1 ancestor commit(s) of 'main'"),
            "expected backfill message, got {captured:?}"
        );
    ...
    /// Local history plus a squash-merge head whose `merge_parent_hash` edge
    /// dangles at a server-only feature-branch tip — the exact shape every
    /// remote squash-merge leaves an existing clone in. The tip's timestamp
    /// is parameterized so tests can seal its hash on a nanosecond clock.
    struct MergeParentFixture {
        feature_tip: Commit,
        head_commit: Commit,
    }

    fn seed_dangling_merge_parent(
        repo: &SqliteRepository,
        feature_tip_timestamp: chrono::DateTime<chrono::Utc>,
    ) -> MergeParentFixture {
        // Local history: the old main head both sides fork from.
        let old_blob = repo.put_blob(b"old\n".to_vec()).unwrap();
        let old_manifest = repo
    ...
    async fn mount_main_head_with_commit_entries(
        server: &MockServer,
        head_commit: &Commit,
        commit_entries: Vec<serde_json::Value>,
    ) {
        Mock::given(method("GET"))
            .and(urlpath("/api/oak/oak"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
    ...
    /// Regression for the fb-84 / fb-103 family, second acceptance rule: a
    /// merge-parent ancestor whose hash was sealed on a nanosecond clock but
    /// whose served timestamp round-tripped through Postgres `TIMESTAMPTZ`
    /// (microseconds) must verify via nanosecond-tail recovery — under its
    /// own branch name — and the stored row must carry the *recovered*
    /// nanosecond timestamp so it is exactly self-verifying when re-read.
    #[tokio::test(flavor = "current_thread")]
    async fn fetch_parent_backfills_ns_truncated_merge_parent_ancestor() {
        let temp = TempDir::new().unwrap();
        let server = MockServer::start().await;
        let repo = open_linked_repo(&temp, &server.uri());

        // Sealed with a nanosecond tail (…228) that a microsecond store drops.
        let nano_ts = chrono::DateTime::from_timestamp(1_700_000_200, 778_307_228).unwrap();
        let fixture = seed_dangling_merge_parent(&repo, nano_ts);

        // Serve the tip the way Postgres stores it: microsecond-truncated.
        let truncated = oak_core::truncate_to_micros(nano_ts);
        assert_ne!(truncated, nano_ts, "fixture must actually lose precision");
        let mut tip_entry = commit_info_json(&fixture.feature_tip);
        tip_entry["timestamp"] = serde_json::json!(truncated.to_rfc3339());
        mount_main_head_with_commit_entries(
            &server,
            &fixture.head_commit,
            vec![commit_info_json(&fixture.head_commit), tip_entry],
        )
        .await;
        ...
        let fetched = fetch_parent_from_server_with_remote(&repo, "main", Some(&server.uri()))
            .await
            .expect("µs-truncated merged tip must verify via ns-tail recovery");
        let captured = output::end_capture();

        assert_eq!(fetched, Some(fixture.head_commit.hash));
        let stored = repo
            .get_commit(&fixture.feature_tip.hash)
            .unwrap()
            .expect("fetch should backfill the merged feature-branch tip");
        assert_eq!(stored.hash, fixture.feature_tip.hash);
        assert_eq!(stored.branch_name, "feature-x");
        assert_eq!(
            stored.timestamp, nano_ts,
            "stored row must carry the recovered nanosecond timestamp, not the truncated one"
        );
        assert!(
            captured.contains("linked 1 ancestor commit(s) of 'main'"),
            "expected backfill message, got {captured:?}"
    ...
    /// Truncation tolerance must not swallow genuine corruption: a served
    /// ancestor whose non-timestamp fields were tampered with still hard-fails
    /// the fetch with the "does not reproduce its hash" integrity error.
    #[tokio::test(flavor = "current_thread")]
    async fn fetch_parent_rejects_tampered_merge_parent_ancestor() {
        let temp = TempDir::new().unwrap();
        let server = MockServer::start().await;
        let repo = open_linked_repo(&temp, &server.uri());
        ...
        let mut tip_entry = commit_info_json(&fixture.feature_tip);
        tip_entry["timestamp"] = serde_json::json!(truncated.to_rfc3339());
        tip_entry["author"] = serde_json::json!("someone-else");
        mount_main_head_with_commit_entries(
            &server,
            &fixture.head_commit,
            vec![commit_info_json(&fixture.head_commit), tip_entry],
        ...
        output::begin_capture();
        let err = fetch_parent_from_server_with_remote(&repo, "main", Some(&server.uri()))
            .await
            .expect_err("tampered ancestor fields must abort the fetch");
        let _ = output::end_capture();

        assert!(
            err.to_string()
                .contains("remote ancestor commit metadata does not reproduce its hash"),
            "expected the ancestor integrity error, got: {err}"
        );
        assert!(
            repo.get_commit(&fixture.feature_tip.hash)
                .unwrap()
                .is_none(),
            "a tampered ancestor must never be stored"
        );
    }

    /// A server that omits `branch_name` can't prove which name the ancestor
    /// hash was sealed under; the backfill must leave the gap (reported as
    /// incomplete) rather than guess a name and hard-fail verification.
    #[tokio::test(flavor = "current_thread")]
    async fn fetch_parent_leaves_gap_when_ancestor_branch_name_is_missing() {
        let temp = TempDir::new().unwrap();
        let server = MockServer::start().await;
        let repo = open_linked_repo(&temp, &server.uri());
        ...
        let ancestor_commit =
            main_commit_with_manifest(ancestor_manifest, None, "ancestor without branch_name");
        ...
        let fetched = fetch_parent_from_server_with_remote(&repo, "main", Some(&server.uri()))
            .await
            .expect("missing branch_name should leave the gap, not abort fetch");
        let captured = output::end_capture();

        assert_eq!(fetched, Some(head_commit.hash));
        assert!(
            repo.get_commit(&ancestor_commit.hash).unwrap().is_none(),
            "ancestor without a provable branch name must not be stored"
        );
        assert!(
    ...
    async fn fetch_parent_without_commit_info_trees_fails_closed() {
        ...
        let marker = repo
            ...
            .unwrap();
        assert!(
            marker.contains("canonical tree objects"),
            "integrity marker should preserve the fetch failure, got {marker:?}"
        );
