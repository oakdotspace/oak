...
pub(crate) fn write_sync_conflict_state(
    ...
    fs::write(&sync_head_path, sync_head)?;
    fs::write(
        &sync_msg_path,
...
pub(crate) fn complete_reparent(
    ...
    repo.set_branch_head(branch_name, &commit.hash)?;
...
pub(crate) async fn fetch_parent_from_server(
    ...
    // already points at it, nothing to fetch. Other branch heads may still
    // sit on a synthetic duplicate of this head (minted by an older binary
    // or another ingestion path), so run the reconcile swap before the
    // early return.
