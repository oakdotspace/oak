...
async fn download_and_verify(
    ...
    let repo = release_repo();
    // Windows release assets carry a `.exe` suffix (`oak-windows-x86_64.exe`);
    // every other platform's asset is extensionless. The `.minisig` sidecar is
    // appended to whatever asset name this produces.
    let asset = format!("oak-{platform}{}", env::consts::EXE_SUFFIX);
