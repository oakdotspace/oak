...
enum Commands {
    ...
    Merge {
        ...
        /// Branch to preview merge for (defaults to current branch)
    ...
    /// Commit dirty work if needed, push, and sync the branch description.
    Finish {
    ...
    let result = match cli.command {
        ...
        Commands::Diff {
            ...
            if let Some(dest) = mount_dest_for_cwd(&cwd) {
                if json {
                    Err(oak_core::OakError::InvalidArgument(
                        "`oak diff --json` is not supported inside mounts yet".to_string(),
                    ))
                } else {
        ...
        Commands::Status {
            ...
            if let Some(dest) = mount_dest_for_cwd(&cwd) {
                if json && compact {
                    Err(oak_core::OakError::InvalidArgument(
                        "`oak status --json --compact` is not supported inside mounts yet; use `oak agent state --json --compact` or `oak status --porcelain`".to_string(),
                    ))
                } else if json {
...
fn print_help() {
    ...
    print_cmd(
        "finish",
        "--desc-file FILE [--json]",
        "Commit, push, and sync description",
        YELLOW,
    );
