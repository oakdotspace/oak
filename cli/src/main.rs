    ...
    let result = match cli.command {
                    ...
                    Some(BranchCommands::Diff {
                        ...
                        Ok(diff_mode) => {
                            if remote {
                                ...
                                commands::diff::branch_diff_human(&cwd, &name, &against, diff_mode)
                                    .map(|_| ())
                            }
                        }
                    },
        ...
        Commands::Diff {
            ...
            if let Some(dest) = mount_dest_for_cwd(&cwd) {
                ...
                {
                    Err(oak_core::OakError::InvalidArgument(
                        "oak diff --exit-code/--against/--mode/--hunks/--unified/--word-diff are not supported inside a mount yet".to_string(),
                    ))
                } else if json {
                ...
                match commands::diff::dispatch(&cwd, request) {
                    Ok(true) if exit_code => std::process::exit(1),
                    Ok(_) => Ok(()),
                    Err(e) => Err(e),
                }
            }
            ...
            match name {
                ...
                Some(name) => {
                    ...
                    if commands::git_clone::looks_like_git_url(&name) {
                        ...
                        match rt.block_on(commands::repo::clone_repo_sparse(
                            &remote,
                            &name,
                            &full_dest,
                            shallow,
                ...
                None => {
                    ...
                    match spec {
                        None => {
                            ...
                            // mount target must be named explicitly.
                            use clap::CommandFactory;
                            let mut app = Cli::command();
                            let _ = match app.find_subcommand_mut("mount") {
                                Some(sub) => sub.clone().print_help(),
                                None => app.print_help(),
                            };
                            Ok(())
                        }
