...
fn command_outputs_structured(command: &Commands) -> bool {
    match command {
        ...
        Commands::Space {
            action: SpaceCommands::Inventory { json, .. },
        } => *json,
        Commands::Status {
            json,
            compact,
...
fn command_outputs_json_error_envelope(command: &Commands) -> bool {
    match command {
        ...
        Commands::Space {
            action: SpaceCommands::Inventory { json: true, .. },
        } => true,
...
fn main() {
    ...
    let result = match cli.command {
            ...
            match name {
                None => {
                    if branch.is_some() {
                        ...
                        if json {
                            Err(oak_core::OakError::InvalidArgument(
                                "clone --json requires ORG/REPO; the interactive picker is not a machine-readable acquisition target"
                                    .to_string(),
                            ))
                        } else {
                ...
                Some(name) => {
                    ...
                    if commands::git_clone::looks_like_git_url(&name) {
                        ...
                        match rt.block_on(
                            commands::repo::clone_repo_sparse_on_branch_with_policy_and_output(
                                &remote,
                                &name,
                                &full_dest,
                                shallow,
        ...
        } => match commands::file::inspect(&cwd, &at, &path, max_bytes, json) {
            Ok(true) => Ok(()),
        ...
        Commands::Ci { action } => match action {
            ...
            CiCommands::Wait {
                ...
                json,
                summary,
            } => match rt.block_on(commands::ci::wait_with_summary(
                &cwd,
                &run_ids,
                commit.as_deref(),
...
fn print_help() {
    ...
    print_cmd(
        "clone",
        "[ORG/REPO] [--branch NAME [--expected-head FULL]] [--path PREFIX] [--shallow] [--allow-unverified-integrity] [--allow-legacy-scope]",
    ...
    print_cmd(
        "file",
        "inspect --at HEAD|HASH --json PATH",
    ...
    #[cfg(any(target_os = "macos", target_os = "linux", target_os = "windows"))]
    {
        ...
        print_sub(
            "inventory [ROOT] [--max-entries N --max-depth N --json]",
