    ...
    pub fn from_error_with_path(err: &oak_core::OakError, cwd: Option<&std::path::Path>) -> Self {
        let (code, recommended_next_commands): (&str, Vec<String>) = match err {
            ...
            oak_core::OakError::UncommittedChanges | oak_core::OakError::DirtyWorkingTree(_) => {
                ("dirty_worktree", vec!["oak status --json".to_string()])
            }
            oak_core::OakError::InvalidArgument(_) | oak_core::OakError::InvalidPath(_) => {
                ("invalid_argument", Vec::new())
            }
            oak_core::OakError::Config(_) | oak_core::OakError::RemoteNotConfigured => {
                ("configuration_error", vec!["oak info --json".to_string()])
            }
            ...
            | oak_core::OakError::RemoteRepoAlreadyExists(_) => {
                ("remote_error", vec!["oak status --json".to_string()])
            }
            _ => ("error", Vec::new()),
        };
        JsonErrorEnvelope {
...
pub fn format_log_more_hint(shown: usize) -> String {
    format_log_more_hint_with_mode(shown, false)
}

pub fn format_log_more_hint_with_mode(shown: usize, oneline: bool) -> String {
    let mode = if oneline { " --oneline" } else { "" };
    format!(
        "... more commits: oak log{} -n {}",
        mode,
...
mod tests {
    ...
    fn log_more_hint_doubles_the_window() {
        ...
        assert_eq!(
            format_log_more_hint_with_mode(20, true),
