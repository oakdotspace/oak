...
/// Informational progress is routed to stderr for these commands as well.
...
impl JsonErrorEnvelope {
    ...
    pub fn from_error_with_path(err: &oak_core::OakError, cwd: Option<&std::path::Path>) -> Self {
        let (code, recommended_next_commands, finish, commit): (
            ...
            oak_core::OakError::WorktreeCaptureRaced { .. } => (
                "worktree_capture_raced",
                vec!["oak change capture --json".to_string()],
                None,
                None,
            ),
            oak_core::OakError::UnsupportedChangeCaptureFormat(_) => {
                ("unsupported", Vec::new(), None, None)
            }
            oak_core::OakError::CapturedBlobIntegrity { .. } => {
                ("captured_blob_integrity", Vec::new(), None, None)
            }
            oak_core::OakError::CapturedTreeIntegrity { .. } => {
                ("captured_tree_integrity", Vec::new(), None, None)
            }
            oak_core::OakError::ChangeCaptureNotFound { .. } => (
                "change_capture_not_found",
                vec!["oak change capture --json".to_string()],
                None,
                None,
            ),
            oak_core::OakError::ChangeCaptureIntegrity { .. } => {
                ("change_capture_integrity", Vec::new(), None, None)
            }
            oak_core::OakError::ChangeCaptureScopeExcluded { .. } => {
                ("capture_scope_excluded", Vec::new(), None, None)
            }
            ...
            oak_core::OakError::PublicationUnconfirmed {
...
/// Print informational progress, keeping structured stdout free of narration.
pub fn info(msg: &str) {
    if STRUCTURED_OUTPUT.load(Ordering::Relaxed) {
        emit_stderr_line(msg);
    } else {
        emit_line(msg);
    }
}
