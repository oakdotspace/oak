...
use oak_core::{BranchStatus, MetadataKey, Repository, Result, SqliteRepository};
...
pub struct AgentWorkState {
    ...
    pub pending_publications: Vec<crate::publication_state::PendingPublicationJson>,
    pub pending_publication_diagnostics: Vec<crate::publication_state::PublicationRecordDiagnostic>,
...
impl From<AgentWorkState> for output::AgentStateJson {
    fn from(state: AgentWorkState) -> Self {
        output::AgentStateJson {
            ...
            pending_publication_count: state.pending_publication_count,
            pending_publications: state.pending_publications,
            pending_publication_diagnostics: state.pending_publication_diagnostics,
            pending_publications_has_more: state.pending_publications_has_more,
            pending_publications_next_after: state.pending_publications_next_after,
            publication_capacity: state.publication_capacity,
            publication_blocked: state.publication_blocked,
            mount: state.mount,
        }
    }
}
...
pub async fn checkout_agent_state_json(
    path: &Path,
    refresh: bool,
    publication_after: Option<&str>,
) -> Result<output::AgentStateJson> {
    Ok(checkout_agent_state(path, refresh, publication_after)
        .await?
        .into())
}

pub async fn mount_agent_state_json(
    dest: &Path,
    refresh: bool,
    publication_after: Option<&str>,
) -> Result<output::AgentStateJson> {
    Ok(mount_agent_state(dest, refresh, publication_after)
        .await?
        .into())
}

pub async fn checkout_agent_state(
    path: &Path,
    refresh: bool,
    publication_after: Option<&str>,
) -> Result<AgentWorkState> {
    let ctx = crate::resolve::resolve(path)?;
    let publication_page = crate::publication_state::AttemptStore::checkout(&ctx.oak_dir).page(
        publication_after,
        crate::publication_state::DEFAULT_PAGE_LIMIT,
    )?;
    let pending_publication_count = publication_page.pending_publication_count;
    let pending_publications_has_more = publication_page.pending_publications_has_more;
    let pending_publications_next_after = publication_page.pending_publications_next_after.clone();
    let pending_publication_diagnostics = publication_page.pending_publication_diagnostics.clone();
    let publication_capacity = publication_page.publication_capacity;
    let has_pending_publication = pending_publication_count > 0;
    let repo = ctx.open()?;
    ...
    let current_publication_identity = receipt_remote
        .as_ref()
        .zip(repo_owner.as_ref())
        .zip(repo_name.as_ref())
        .map(
            |((remote, owner), repo_name)| crate::publication_state::CurrentIdentity {
                remote: remote.clone(),
                owner: owner.clone(),
                repo: repo_name.clone(),
                repository_token: repo.get_metadata(MetadataKey::ApiKey).ok().flatten(),
            },
        );
    let pending_publications = publication_page
        .into_json_rows(refresh, current_publication_identity.as_ref())
        .await;
    ...
    let mut finish_eligible = can_finish;
    let mut blocking_reason = blocking_reason(
        &progress.json,
        branch_name.as_deref(),
        branch_closed,
        needs_pull,
    ...
    if has_pending_publication {
        ...
        blocking_reason = Some("publication_outcome_unconfirmed".to_string());
        recommended_next_commands.retain(|command| !is_publication_mutation_command(command));
        recommended_next_commands
            .insert(0, "oak agent state --refresh --json --compact".to_string());
        if let Some(after) = pending_publications_next_after.as_deref() {
            recommended_next_commands.push(format!(
                "oak agent state --json --compact --publication-after {after}"
            ));
        }
    }
    ...
    Ok(AgentWorkState {
        ...
        pending_publications_has_more,
        pending_publications_next_after,
        publication_capacity,
        publication_blocked: has_pending_publication,
        mount: None,
    })
}

pub async fn mount_agent_state(
    dest: &Path,
    refresh: bool,
    publication_after: Option<&str>,
) -> Result<AgentWorkState> {
    let (cfg, state_dir) = commands::mount::config_for_dest(dest)?;
    let publication_page = crate::publication_state::AttemptStore::mount(&state_dir).page(
        publication_after,
        crate::publication_state::DEFAULT_PAGE_LIMIT,
    )?;
    let pending_publication_count = publication_page.pending_publication_count;
    ...
    let current_publication_identity = crate::publication_state::CurrentIdentity {
        remote: cfg.remote_url.clone(),
        owner: cfg.owner.clone(),
        repo: cfg.repo.clone(),
        repository_token: cache.get_metadata(MetadataKey::ApiKey).ok().flatten(),
    };
    let pending_publications = publication_page
        .into_json_rows(refresh, Some(&current_publication_identity))
        .await;
    ...
    let refresh_errors: Vec<String> = refresh
        ...
        .into_iter()
        .collect();
    let mut recommended_next_commands = mount_next_commands(
        finish_eligible,
        branch_closed,
...
fn is_publication_mutation_command(command: &str) -> bool {
    command == "oak push"
        || command.starts_with("oak push ")
        || command == "oak finish"
        || command.starts_with("oak finish ")
        || command.starts_with("oak commit --push")
}
