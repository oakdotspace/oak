                ...
                .open(&lock_path)
            {
                ...
                Err(e) if e.kind() == ErrorKind::AlreadyExists => {
                    if let Some(snapshot) = stale_lock_snapshot(&lock_path)? {
                        if remove_if_unchanged(&lock_path, &snapshot)? {
                            continue;
                        }
                        // Another process changed the lock while we were
                        // reaping a stale one. Loop so create_new gets the
                        // first chance to prove whether it still exists.
                        continue;
                    }
...
fn is_process_alive(pid: u32) -> bool {
    ...
    {
        let _ = pid;
        // Fail safe where we do not have a real liveness probe: keep the lock
        // rather than allowing concurrent working-tree writers.
        true
    }
}
