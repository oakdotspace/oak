...
#[test]
fn agent_state_preserves_malformed_publication_evidence_with_bounded_guidance() {
    let temp = fixture_repo();
    let operation_id = uuid::Uuid::new_v4().to_string();
    let records = temp.path().join(".oak/publication-attempts");
    std::fs::create_dir(&records).unwrap();
    std::fs::write(records.join(format!("{operation_id}.json")), b"{not-json").unwrap();

    let out = oak(temp.path(), &["agent", "state", "--json", "--compact"]);
    assert!(out.status.success(), "{}", stderr(&out));
    assert!(
        out.stdout.len() < 16 * 1024,
        "diagnostic output must stay bounded"
    );
    let value: Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(value["pending_publication_count"], 1);
    assert_eq!(value["publication_blocked"], true);
    assert_eq!(
        value["pending_publication_diagnostics"][0]["operation_id"],
        operation_id
    );
    assert_eq!(
        value["pending_publication_diagnostics"][0]["problem"],
        "malformed"
    );
    assert_eq!(
        value["pending_publication_diagnostics"][0]["owner_liveness"],
        "unknown"
    );
    assert!(
        value["pending_publication_diagnostics"][0]["operator_action"]
            .as_str()
            .unwrap()
            .contains("will not remove")
    );
    assert!(!value["recommended_next_commands"]
        .as_array()
        .unwrap()
        .iter()
        .any(|command| command
            .as_str()
            .is_some_and(|command| command.contains("forget"))));
}
...
#[test]
fn review_rejects_unknown_explicit_against_branch() {
    let temp = fixture_repo();
    ...
    for args in [
        ...
        vec!["diff", &branch, "--against", "typo-main", "--json"],
    ] {
        let out = oak(temp.path(), &args);
        assert!(
            !out.status.success(),
            "unknown target must not produce review evidence: {}",
            stdout(&out)
        );
        assert!(
            stdout(&out).contains("typo-main") || stderr(&out).contains("typo-main"),
            "{}",
            stderr(&out)
        );
    }
...
#[test]
fn missing_fork_never_invents_an_empty_tree_contribution() {
    for main_available in [false, true] {
        let temp = fixture_repo();
        let dir = temp.path();
        let branch = current_branch(dir);
        let repo = SqliteRepository::open(&dir.join(".oak/oak.db")).unwrap();
        let head = repo.get_branch_head(&branch).unwrap().unwrap();
        let manifest = repo.get_commit(&head).unwrap().unwrap().manifest_hash;
        let missing = oak_core::Hash("d2".repeat(32));
        let main = repo
            .put_commit(
                "main".to_string(),
                Some(missing.clone()),
        ...
        let feature = repo
            ...
            .unwrap();
        repo.set_branch_head(&branch, &feature).unwrap();
        drop(repo);
        ...
        let diff = json_stdout(&out);
        assert_eq!(
            diff["merge_lineage_evidence"]["comparison_source"],
            "contribution_unavailable"
        );
        assert_eq!(
            diff["changed_files"].as_array().unwrap().len(),
            0,
            "missing history must not invent additions"
        );
        assert_eq!(diff["files_identical_to_against"]["available"], false);
        ...
        for args in [
            ...
            assert!(
                !out.status.success(),
                "must refuse unavailable contribution: {}",
                stdout(&out)
            );
            assert!(!stdout(&out).contains("+base"));
        }
    }
}

#[test]
fn contribution_from_complete_initial_history_uses_empty_main() {
    let temp = fixture_repo();
    let branch = current_branch(temp.path());
    ...
    assert_eq!(diff["changed_files"][0]["status"], "added");
    assert_eq!(
        diff["merge_lineage_evidence"]["comparison_source"],
        "unborn_main_empty_tree"
    );
}
...
#[tokio::test(flavor = "current_thread")]
async fn commit_push_missing_credentials_cannot_publish_to_protected_server() {
    let temp = fixture_repo();
    let dir = temp.path();
    let server = MockServer::start().await;
    let branch_name = current_branch(dir);
    ...
    assert!(json["error"]["message"].as_str().unwrap().contains("401"));
    let repo = SqliteRepository::open(&dir.join(".oak/oak.db")).unwrap();
    assert_ne!(
    ...
    assert_eq!(
        ...
        "server rejection must retain the local checkpoint for retry"
    );
    assert!(
        server
            .received_requests()
        ...
        "authentication rejection must prevent every remote mutation"
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn commit_push_without_credentials_uses_open_loopback_server() {
    let _ = rustls::crypto::ring::default_provider().install_default();
    let temp = fixture_repo();
    let served = tempfile::TempDir::new().unwrap();
    let remote = oak_cli::commands::serve::spawn_loopback(served.path().to_path_buf())
        .await
        .unwrap();
    let response = reqwest::Client::new()
        .post(format!("{remote}/api/repos"))
        .json(&serde_json::json!({"name":"oak", "organization_slug":"oak"}))
        .send()
        .await
        .unwrap();
    ...
    repo.set_metadata(MetadataKey::RepoName, "oak").unwrap();
    let branch = current_branch(temp.path());
    repo.edit_branch_description(&branch, "published with commit")
        .unwrap();
    std::fs::write(
    ...
    .unwrap();
    let out = oak(temp.path(), &["commit", "--push", "--json", "--quiet"]);
    assert!(
        out.status.success(),
        "stdout: {} stderr: {}",
        stdout(&out),
    ...
    let published = reqwest::Client::new()
        .get(format!("{remote}/api/oak/oak/branches/{branch}"))
        .send()
        .await
        .unwrap()
        .json::<Value>()
    ...
    assert!(published
        .to_string()
        .contains(json["head_after"].as_str().unwrap()));
}
...
#[tokio::test(flavor = "current_thread")]
async fn finish_json_unconfirmed_push_preserves_state_without_blind_retry() {
    let temp = fixture_repo();
    let dir = temp.path();
    let branch = current_branch(dir);
    ...
    assert_eq!(json["error"]["finish"]["retry_command"], Value::Null);
    assert!(json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("durable local operation id"));
    assert_eq!(
        json["error"]["finish"]["manual_recovery_commands"],
        serde_json::json!([
    ...
    let diagnostics = format!("{}{}", stdout(&out), stderr(&out));
...
#[tokio::test(flavor = "current_thread")]
async fn finish_json_unconfirmed_metadata_sync_preserves_description_without_blind_retry() {
    let temp = tempfile::TempDir::new().unwrap();
    let dir = temp.path();
    assert!(oak(dir, &["init", "."]).status.success());
    let branch = current_branch(dir);
...
#[tokio::test(flavor = "current_thread")]
async fn commit_push_json_quiet_unconfirmed_publication_preserves_checkpoint() {
    let temp = fixture_repo();
    let dir = temp.path();
    let branch_name = current_branch(dir);
    let repo = SqliteRepository::open(&dir.join(".oak/oak.db")).unwrap();
    ...
    assert_eq!(json["error"]["commit"]["retry_command"], Value::Null);
    assert!(json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("durable local operation id"));
    assert_eq!(
        json["error"]["commit"]["manual_recovery_commands"],
        serde_json::json!([
            "oak agent state --refresh --json --compact",
            format!("oak branch show {branch_name} --remote --json"),
            "oak status --json"
        ])
    );
    assert!(
    ...
    assert_eq!(requests.len(), 3, "commit --push should publish once");
    assert_eq!(
        requests
            .iter()
...
async fn remote_branch_diff_hunks_do_not_use_local_checkout_bytes_when_blob_missing() {
    ...
    assert_ne!(diff["changed_files"][0]["binary_or_large"], true);
    assert_eq!(
        diff["changed_files"][0]["content_unavailable_reason"],
        "missing_blob"
    );
    assert_eq!(diff["changed_files"][0]["stats_available"], false);
...
#[test]
fn local_branch_diff_omits_unavailable_hunks_instead_of_using_worktree() {
    let temp = fixture_repo();
    let dir = temp.path();
    let main = seed_local_main_from_current_head(dir);
    let feature = seed_remote_feature_commit_with_missing_blob(dir, main.clone());
    let repo = SqliteRepository::open(&dir.join(".oak/oak.db")).unwrap();
    ...
    .unwrap();
    repo.set_branch_head("remote-feature", &feature).unwrap();
    drop(repo);
    std::fs::write(dir.join("tracked.txt"), "misleading local checkout\n").unwrap();
    for args in [
        vec!["branch", "diff", "remote-feature", "--json", "--hunks"],
        vec!["diff", "remote-feature", "--json", "--hunks"],
        vec!["diff", main.as_str(), feature.as_str(), "--json", "--hunks"],
    ] {
        let diff = json_stdout(&oak(dir, &args));
        let file = &diff["changed_files"][0];
        assert_eq!(file["content_unavailable_reason"], "missing_blob");
        assert!(
            file.get("patch").is_none(),
            "missing bytes must not be substituted: {file}"
        );
        assert_eq!(file["patch_omitted"], true);
    }
}
...
fn branch_review_merge_preview_json_predicts_local_conflicts() {
    ...
    // A predicted conflict is not itself a complete contribution assessment.
    // Keep the command aligned with this fixture's unresolved review verdict.
    assert_eq!(review["recommended_action"], "review");
    assert_eq!(review["merge_allowed"], false);
    assert_eq!(
        review["recommended_next_commands"],
        serde_json::json!(["oak branch review feature-conflict --merge-preview --json"])
    );
}
...
#[tokio::test]
async fn merge_json_ambiguous_server_error_is_redacted_unconfirmed_and_not_retried() {
    let temp = fixture_repo();
    let dir = temp.path();
    let branch_name = current_branch(dir);
    let branch_head = seed_local_main_from_current_head(dir);
    ...
    let worktree = dir.to_path_buf();
    let out = tokio::task::spawn_blocking(move || oak(&worktree, &["merge", "--json"]))
        .await
        .unwrap();
    ...
    assert!(json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("merge publication outcome is unconfirmed"));
    assert_eq!(
        json["error"]["recommended_next_commands"],
        serde_json::json!([format!("oak branch show {branch_name} --remote --json")])
    );
    let diagnostics = format!("{}{}", stdout(&out), stderr(&out));
    assert!(!diagnostics.contains("SYNTHETIC_PRIVATE_ERROR_MARKER"));
    assert!(!diagnostics.contains("do-not-print"));
    let repo = SqliteRepository::open(&dir.join(".oak/oak.db")).unwrap();
    assert_eq!(
        repo.get_branch(&branch_name).unwrap().unwrap().status,
        oak_core::BranchStatus::Open
    );
}
...
fn branch_review_triage_empty_branch_recommends_close() {
    ...
    assert_eq!(review["worktree_dirty"], false);
    assert_eq!(review["dirty_file_count"], 0);
...
#[test]
fn branch_review_dirty_current_branch_never_recommends_close() {
    let temp = fixture_repo();
    let dir = temp.path();
    seed_local_main_from_current_head(dir);
    assert!(oak(dir, &["switch", "-c", "dirty-empty-branch"])
        .status
        .success());
    std::fs::write(dir.join("unfinished.rs"), "fn unfinished() {}\n").unwrap();
    ...
    assert_eq!(review["worktree_dirty"], true);
    assert_eq!(review["dirty_file_count"], 1);
    assert_eq!(review["close_allowed"], false);
    assert_ne!(review["recommended_action"], "close");
    assert_ne!(review["recommended_action_detail"]["kind"], "close_branch");
    assert!(review["recommended_next_commands"]
        .as_array()
        .unwrap()
        .iter()
        .all(|command| !command.as_str().unwrap().starts_with("oak close ")));
}
...
async fn branch_review_triage_remote_does_not_switch_checkout() {
    ...
    assert_eq!(review["vcs_merge_safe"], true);
    assert_eq!(review["merge_allowed"], false);
    assert_eq!(review["worktree_dirty"], Value::Null);
    assert_eq!(review["dirty_file_count"], Value::Null);
    assert_eq!(current_branch(dir), original_branch);
}
