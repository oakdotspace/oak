...
fn agent_state_json_uses_repo_placeholder_for_unlinked_publish_action() {
    ...
    assert_eq!(json["needs_push"], true);
    assert_eq!(json["blocking_reason"], "no_remote_configured");
    // Keep legacy command strings compatible, but make the structured action
    // point at the non-interactive publish form that can actually configure a remote.
    assert_eq!(json["recommended_next_commands"][0], "oak push");
...
fn commit_json_quiet_reports_local_checkpoint_without_human_output() {
    ...
    assert_eq!(json["next_commands"][0], "oak push");
}
