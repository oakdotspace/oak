...
fn inventory_item() -> serde_json::Value {
    serde_json::json!({"id":ITEM_ID,"number":165,"title":"Inventory","body":"A report",
        "status":"planned","source":"cli","surface":null,"cli_version":null,
        "created_at":"2026-09-07T00:00:00Z","updated_at":"2026-09-07T00:00:00Z",
        "email":"PRIVATE-CONTACT", "admin_notes":"PRIVATE-NOTES"})
}

fn inventory_page() -> serde_json::Value {
    serde_json::json!({"schema_version":1,"scope":{"kind":"platform_admin","email":"PRIVATE-SCOPE"},
        "consistency":"live_keyset","redaction":"reporter_metadata_omitted",
        "items":[inventory_item()],"next_cursor":"abcd","email":"PRIVATE-TOP",
        "recommended_next_commands":["echo PRIVATE-COMMAND"]})
}

#[tokio::test]
async fn transition_preserves_supplied_revision_and_prints_only_confirmed_receipt() {
    let server = MockServer::start().await;
    let (work, home) = scene(&server);
    Mock::given(method("GET"))
        .and(path_matcher("/api/feedback/capabilities"))
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(serde_json::json!({"schema_version":1,"feedback_transition_v1":1})),
        )
        .expect(1)
        .mount(&server)
        .await;
    let mut item = inventory_item();
    item["revision"] = serde_json::json!("newer-observation");
    Mock::given(method("GET")).and(path_matcher("/api/feedback/fb-165"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"schema_version":1,"scope":{"kind":"platform_admin"},"consistency":"live_keyset","redaction":"reporter_metadata_omitted","item":item}))).expect(1).mount(&server).await;
    item["revision"] = serde_json::json!("result-token");
    item["status"] = serde_json::json!("in_progress");
    Mock::given(method("POST")).and(path_matcher(format!("/api/feedback/{ITEM_ID}/transition")))
        .and(body_json(serde_json::json!({"protocol":"feedback_transition_v1","expected_revision":"user-token","status":"in_progress","admin_notes":"PRIVATE-NOTE"})))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"schema_version":1,"protocol":"feedback_transition_v1","changed":true,"previous_revision":"user-token","revision":"result-token","item":item,"admin_notes":"PRIVATE-NOTE","reporter":"PRIVATE-REPORTER"}))).expect(1).mount(&server).await;
    let out = oak(
        work.path(),
        home.path(),
        &[
    ...
    assert!(!stderr(&out).contains("PRIVATE"));
    let receipt: serde_json::Value = serde_json::from_str(&text).unwrap();
    assert_eq!(receipt["confirmed"], true);
    assert_eq!(receipt["changed"], true);
    assert_eq!(receipt["feedback_id"], ITEM_ID);
    assert_eq!(receipt["previous_revision"], "user-token");
    assert_eq!(receipt["revision"], "result-token");
    assert!(server
        .received_requests()
        .await
        .unwrap()
        .iter()
        .all(|r| r.url.query().is_none()));
}

async fn transition_reads(server: &MockServer) {
    Mock::given(method("GET"))
        .and(path_matcher("/api/feedback/capabilities"))
        .respond_with(
            ResponseTemplate::new(200)
        ...
        .await;
    let mut item = inventory_item();
    item["revision"] = serde_json::json!("observed-token");
    Mock::given(method("GET")).and(path_matcher("/api/feedback/fb-165"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"schema_version":1,"scope":{"kind":"platform_admin"},"consistency":"live_keyset","redaction":"reporter_metadata_omitted","item":item}))).mount(server).await;
}

fn transition_receipt(changed: bool) -> serde_json::Value {
    ...
    serde_json::json!({"schema_version":1,"protocol":"feedback_transition_v1","changed":changed,"previous_revision":"user-token","revision":revision,"item":item,"admin_notes":null})
}

#[tokio::test]
async fn transition_unsupported_capability_never_resolves_or_posts() {
    for (status, body) in [
        (404, serde_json::json!({"error":"PRIVATE"})),
        (403, serde_json::json!({"error":"PRIVATE"})),
        (200, serde_json::json!({"schema_version":1})),
        (
            200,
            serde_json::json!({"schema_version":1,"feedback_transition_v1":0}),
        ),
        (
            200,
            serde_json::json!({"schema_version":1,"feedback_transition_v1":"1"}),
        ),
        (
            200,
            serde_json::json!({"schema_version":2,"feedback_transition_v1":1}),
        ),
    ] {
        let server = MockServer::start().await;
        let (work, home) = scene(&server);
        ...
        assert!(!stdout(&out).contains("PRIVATE"));
        assert!(!stderr(&out).contains("PRIVATE"));
        assert_eq!(
            server.received_requests().await.unwrap().len(),
            1,
            "unsupported must not fall back or resolve"
        );
    }
}

#[tokio::test]
async fn transition_noop_and_clear_notes_keep_distinct_payloads_and_parent_flags() {
    for clear in [false, true] {
        let server = MockServer::start().await;
        let (work, home) = scene(&server);
        transition_reads(&server).await;
        let mut expected = serde_json::json!({"protocol":"feedback_transition_v1","expected_revision":"user-token"});
        if clear {
            expected["admin_notes"] = serde_json::Value::Null;
        } else {
            expected["status"] = serde_json::json!("planned");
        }
        Mock::given(method("POST"))
            .and(path_matcher(format!("/api/feedback/{ITEM_ID}/transition")))
            .and(body_json(expected))
            .respond_with(ResponseTemplate::new(200).set_body_json(transition_receipt(false)))
            .expect(1)
            .mount(&server)
            .await;
        ...
        assert_eq!(receipt["revision"], "user-token");
        assert!(!stdout(&out).contains("PRIVATE"));
    }
}

#[tokio::test]
async fn transition_mismatched_or_ambiguous_receipts_are_unconfirmed_without_retry() {
    for fault in [
        "identity",
        "number",
        "previous",
        ...
        let (work, home) = scene(&server);
        transition_reads(&server).await;
        let mut receipt = transition_receipt(true);
        match fault {
            "identity" => receipt["item"]["id"] = serde_json::json!("wrong-id"),
            "number" => receipt["item"]["number"] = serde_json::json!(166),
            "previous" => receipt["previous_revision"] = serde_json::json!("newer-observation"),
            "revision" => receipt["item"]["revision"] = serde_json::json!("different"),
            "changed" => receipt["changed"] = serde_json::json!(false),
            "status" => receipt["item"]["status"] = serde_json::json!("done"),
            "protocol" => receipt["protocol"] = serde_json::json!("other"),
            "schema" => receipt["schema_version"] = serde_json::json!(2),
            "notes" => receipt["admin_notes"] = serde_json::json!("PRIVATE-NOT-CLEARED"),
            _ => {}
        }
        let response = match fault {
            "malformed" => ResponseTemplate::new(200).set_body_string("PRIVATE-MALFORMED"),
            "oversized" => ResponseTemplate::new(200).set_body_string("x".repeat(1024 * 1024 + 1)),
            "server_error" => ResponseTemplate::new(503).set_body_string("PRIVATE-SERVER"),
            _ => ResponseTemplate::new(200).set_body_json(receipt),
        };
        Mock::given(method("POST"))
            .and(path_matcher(format!("/api/feedback/{ITEM_ID}/transition")))
        ...
        assert_eq!(code(&out), 1, "fault={fault}: {}", stderr(&out));
        assert!(
            stderr(&out).contains("UNCONFIRMED"),
            "fault={fault}: {}",
            stderr(&out)
        );
        assert!(!stderr(&out).contains("PRIVATE"));
...
#[tokio::test]
async fn competing_transitions_keep_the_original_token_and_do_not_retry_conflict() {
    let server = MockServer::start().await;
    let (work, home) = scene(&server);
    transition_reads(&server).await;
    ...
    Mock::given(method("POST")).and(path_matcher(format!("/api/feedback/{ITEM_ID}/transition")))
        .and(body_json(serde_json::json!({"protocol":"feedback_transition_v1","expected_revision":"user-token","status":"planned"})))
        .respond_with(move |_: &wiremock::Request| {
            if observed.fetch_add(1,std::sync::atomic::Ordering::SeqCst)==0 {
                ResponseTemplate::new(200).set_body_json(transition_receipt(true))
            } else {ResponseTemplate::new(412).set_body_json(serde_json::json!({"error":"revision_conflict","revision":"result-token","admin_notes":"PRIVATE-CONFLICT"}))}
    ...
    let mut codes = outputs.iter().map(code).collect::<Vec<_>>();
    codes.sort();
    assert_eq!(codes, vec![0, 1]);
    let failed = outputs.iter().find(|output| code(output) == 1).unwrap();
    assert!(stderr(failed).contains("revision_conflict"));
    assert!(stderr(failed).contains("reconcile"));
    assert!(!stderr(failed).contains("PRIVATE"));
    assert_eq!(calls.load(std::sync::atomic::Ordering::SeqCst), 2);
}

#[tokio::test]
async fn transition_invalid_arguments_are_rejected_before_any_request() {
    let server = MockServer::start().await;
    let (work, home) = scene(&server);
    let long_notes = "x".repeat(20_001);
    let long_revision = "r".repeat(129);
    for args in [
        vec![
            "feedback",
            "transition",
...
#[tokio::test]
async fn inventory_revision_is_optional_but_validated_for_show_and_list() {
    for revision in [
        None,
        Some(serde_json::json!("opaque-token")),
        ...
        let mut page = inventory_page();
        page["items"] = serde_json::json!([item]);
        page["next_cursor"] = serde_json::Value::Null;
        Mock::given(method("GET"))
            .and(path_matcher("/api/feedback/inventory"))
        ...
        Mock::given(method("GET")).and(path_matcher("/api/feedback/fb-165"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"schema_version":1,"scope":{"kind":"platform_admin"},"consistency":"live_keyset","redaction":"reporter_metadata_omitted","item":item}))).mount(&server).await;
        for args in [
            vec!["feedback", "list", "--json"],
            vec!["feedback", "show", "fb-165", "--json"],
        ] {
            let out = oak(work.path(), home.path(), &args);
            let valid = revision.is_none() || revision == Some(serde_json::json!("opaque-token"));
            assert_eq!(code(&out), if valid { 0 } else { 1 }, "{}", stderr(&out));
            if valid {
                assert_eq!(stdout(&out).contains("opaque-token"), revision.is_some());
            }
            assert!(!stdout(&out).contains("PRIVATE"));
        }
    }
...
#[tokio::test]
async fn inventory_list_preserves_flag_placement_and_omits_unknown_fields() {
    let server = MockServer::start().await;
    let (work, home) = scene(&server);
    Mock::given(method("GET"))
        .and(path_matcher("/api/feedback/inventory"))
    ...
    for args in [
        ...
        assert_eq!(value["items"][0]["number"], 165);
        ...
        assert!(value["recommended_next_commands"][0]
            .as_str()
            .unwrap()
            .contains("--cursor abcd"));
    }
}

#[tokio::test]
async fn inventory_show_uses_direct_reference_and_effective_remote() {
    let origin = MockServer::start().await;
    let target = MockServer::start().await;
    let (work, home) = scene(&origin);
    login(home.path(), &target.uri(), "target-token");
    let mut detail = inventory_page();
    detail.as_object_mut().unwrap().remove("items");
    detail["item"] = inventory_item();
    Mock::given(method("GET"))
        .and(path_matcher("/api/feedback/fb-165"))
...
#[tokio::test]
async fn inventory_method_rejection_is_specific_without_disclosing_server_data() {
    for (args, endpoint) in [
        (
            vec!["feedback", "list", "--json"],
            "/api/feedback/inventory",
        ),
        (
            vec!["feedback", "show", "fb-165", "--json"],
            "/api/feedback/fb-165",
        ...
        // Classification uses only405, never inferred route age, auth status,
        // or Allow parsing. Even contradictory or malformed headers stay inert.
        for allow in [
            Some("PATCH"),
            Some("GET, PATCH"),
            Some("PRIVATE-MALFORMED;??"),
            None,
        ] {
            let server = MockServer::start().await;
            let (work, home) = scene(&server);
            let mut response = ResponseTemplate::new(405)
                .insert_header("X-Private", "PRIVATE-HEADER")
                .set_body_string("PRIVATE-BODY admin-token-for-this-server");
            if let Some(value) = allow {
                response = response.insert_header("Allow", value);
            }
            Mock::given(method("GET"))
            ...
            assert!(out.stdout.is_empty(), "JSON error stdout must stay empty");
            let error = stderr(&out);
            assert!(error.contains("feedback GET request rejected (HTTP 405 Method Not Allowed)"));
            assert!(error.contains("does not establish whether credentials are valid"));
            assert!(error.contains("No legacy fallback was attempted"));
            for secret in [
                "PRIVATE",
...
#[tokio::test]
async fn inventory_failure_diagnostics_never_echo_response_or_fall_back() {
    for (status, body) in [
        (404, "PRIVATE-TEXT"),
        (500, "PRIVATE-TEXT"),
        (200, "{\"items\":\"PRIVATE-TEXT\"}"),
    ] {
        let server = MockServer::start().await;
        let (work, home) = scene(&server);
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let out = oak(work.path(), home.path(), &["feedback", "list", "--json"]);
        assert_eq!(code(&out), 1, "{}", stderr(&out));
        assert!(stdout(&out).is_empty());
        assert!(!stderr(&out).contains("PRIVATE"));
        if status == 404 {
            assert!(stderr(&out).contains("unsupported or unauthorized"));
            assert!(!stderr(&out).contains("GET request rejected"));
        }
        if status == 500 {
            assert!(stderr(&out).contains("feedback inventory request failed: HTTP 500"));
            assert!(!stderr(&out).contains("GET request rejected"));
        }
...
#[tokio::test]
async fn inventory_rejects_usage_and_never_uses_checkout_credentials() {
    let server = MockServer::start().await;
    let work = tempfile::tempdir().unwrap();
    let home = tempfile::tempdir().unwrap();
    fixture_repo(work.path(), &server.uri());
    for args in [
        vec!["feedback", "list", "--limit", "101"],
        vec!["feedback", "list", "--status", "nope"],
        vec!["feedback", "list", "--cursor", "invalid"],
        vec!["feedback", "show", "0"],
        vec!["feedback", "list"],
    ] {
        let out = oak(work.path(), home.path(), &args);
