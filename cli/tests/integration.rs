...
#[test]
fn test_switch_create_carry_respects_live_workdir_lock_before_changing_refs() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    ...
    let branch = repo.get_current_branch_name().unwrap();
    let branch_count = repo.list_branches().unwrap().len();
    let lock = oak_cli::workdir_lock::WorkdirLock::acquire(&temp.path().join(".oak")).unwrap();

    let result =
        oak_cli::commands::switch::create(temp.path(), "locked-task", WorktreePolicy::Carry);

    assert!(matches!(result, Err(oak_core::OakError::RepoLocked)));
    assert_eq!(repo.get_head().unwrap(), head);
    assert_eq!(repo.get_current_branch_name().unwrap(), branch);
    assert_eq!(repo.list_branches().unwrap().len(), branch_count);
    assert!(repo.get_branch("locked-task").unwrap().is_none());
    assert_eq!(
    ...
    oak_cli::commands::switch::create(temp.path(), "locked-task", WorktreePolicy::Carry).unwrap();
    assert_eq!(
        repo.get_current_branch_name().unwrap().as_deref(),
        Some("locked-task")
    );
...
#[test]
fn test_switch_create_cli_serializes_all_modes_and_releases_the_lock() {
    for named in [false, true] {
        for discard in [false, true] {
            for main_available in [false, true] {
                ...
                oak_cli::commands::commit::run(temp.path()).unwrap();
                let repo = open_repo(temp.path());
                let old_head = repo.get_head().unwrap();
                let seed = if main_available {
                    Some(seed_main_commit(&repo, "file.txt", "main\n"))
                } else {
                    old_head.clone()
                };
                write_file(temp.path(), "file.txt", "dirty\n");
                let branch_count = repo.list_branches().unwrap().len();
                let mut args = vec!["switch", "-c"];
                if named {
                    args.push("serialized-task");
                }
                if discard {
                    args.push("--clean");
                }
                let lock =
                    oak_cli::workdir_lock::WorkdirLock::acquire(&temp.path().join(".oak")).unwrap();

                let blocked = oak_bin(temp.path(), &args);

                assert_eq!(
                    blocked.status.code(),
                    Some(3),
                    "{args:?}, main={main_available}"
                );
                assert_eq!(repo.get_head().unwrap(), old_head);
                assert_eq!(
                    repo.get_current_branch_name().unwrap().as_deref(),
                    Some(DEFAULT_BRANCH)
                );
                ...
                let completed = oak_bin(temp.path(), &args);
                assert!(
                    completed.status.success(),
                    "{}",
                    String::from_utf8_lossy(&completed.stderr)
                );
                assert_eq!(repo.get_head().unwrap(), seed);
                let current = repo.get_current_branch_name().unwrap().unwrap();
                assert_ne!(current, DEFAULT_BRANCH);
                if named {
                    assert_eq!(current, "serialized-task");
                }
                assert_eq!(repo.get_branch_head(&current).unwrap(), seed);
                assert_eq!(repo.list_branches().unwrap().len(), branch_count + 1);
...
async fn test_switch_create_locked_does_not_refresh_or_change_remote_metadata() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    write_file(temp.path(), "file.txt", "committed\n");
    oak_cli::commands::commit::run(temp.path()).unwrap();
    ...
    assert!(repo
        .get_metadata(MetadataKey::MainLastCheckedAt)
        .unwrap()
        .is_none());
...
#[tokio::test(flavor = "current_thread")]
async fn test_legacy_clone_branch_preserves_a_headless_branch_seeded_at_main() {
    let temp = TempDir::new().unwrap();
    ...
    Mock::given(method("GET"))
        .and(path("/api/oak/oak/pull"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "head": main.hash.as_str(),
            "branch": main_branch.clone(),
            "branches": [main_branch.clone(), feature_branch.clone()],
            "commits": [{
                "hash": main.hash.as_str(),
                "branch_name": "main",
                "parent_hash": null,
                "manifest_hash": manifest.hash.as_str(),
                "author": "tester",
                "message": "main seed",
                "timestamp": "2026-01-01T00:00:00Z",
                "files": []
            ...
            "blobs": [],
            "trees": [{"hash": manifest.hash.as_str(), "entries": []}]
        })))
        .with_priority(2)
        .expect(1)
        .mount(&server)
    ...
    assert!(
        output.status.success(),
        "legacy headless branch clone failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );

    let repo = open_repo(&temp.path().join("cloned"));
    assert_eq!(
        repo.get_current_branch_name().unwrap().as_deref(),
        Some("remote-feature")
    );
    assert_eq!(
        repo.get_branch_head("remote-feature").unwrap().as_ref(),
        Some(&main.hash),
        "a branch with no own commits must retain its authoritative main seed"
    );
    assert_eq!(repo.get_head().unwrap().as_ref(), Some(&main.hash));
}
