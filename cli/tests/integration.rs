...
fn test_diff_shows_changes() {
    ...
    oak_cli::commands::diff::run(temp.path(), &[], false).unwrap();
}
...
fn test_diff_omits_large_text_hunks_but_keeps_path() {
    ...
    fs::write(temp.path().join("large.txt"), vec![b'x'; 1024 * 1024 + 1]).unwrap();
    ...
    assert!(
        rendered.contains("Large files a/large.txt and b/large.txt differ"),
...
fn test_status_warns_on_now_ignored_but_not_on_real_deletions() {
    ...
    oak_cli::output::begin_capture();
    oak_cli::commands::status::run(temp.path()).unwrap();
    let out = oak_cli::output::end_capture();
    assert!(
...
#[test]
fn test_switch_create_without_name_preserves_clean_worktree_when_main_has_no_head() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    ...
    assert_eq!(
        ...
        None,
        "new branch should remain headless when main has no head"
    );
    assert_eq!(
    ...
    assert!(
        !changes.is_empty(),
        "preserved files should be visible as work to commit on the new branch"
    );
}
...
fn test_switch_create_without_name_discard_starts_clean() {
    ...
    assert!(
        ...
        "discard should materialize the clean main tree"
    );
    let (changes, _, _) = oak_cli::commands::commit::get_status(temp.path()).unwrap();
...
async fn test_switch_create_without_name_fetches_remote_main_when_not_recently_checked() {
    ...
    let remote_head = oak_core::hash_bytes(b"remote main head");
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/api/oak/oak/tree/{}", remote_head.as_str())))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!([{
            "name": "remote.txt",
            "path": "remote.txt",
            "kind": "blob",
            "blob_hash": remote_blob.hash.as_str()
        }])))
        .expect(1)
        .mount(&server)
        .await;
...
fn test_switch_without_name_noninteractive_requires_explicit_action() {
    ...
    assert!(
        err.contains("oak switch -c --discard"),
