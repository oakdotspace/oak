...
async fn merge_refuses_dirty_working_tree() {
    ...
    let err = oak_cli::commands::merge::run(temp.path(), false, false, None)
        .await
...
async fn merge_pushes_branch_before_server_merge() {
    ...
    oak_cli::commands::merge::run(temp.path(), false, false, None)
        .await
        .unwrap();
...
async fn merge_explicit_main_child_without_switching_checkout() {
    ...
    oak_cli::commands::merge::run(temp.path(), false, false, Some(DEFAULT_BRANCH))
        .await
        .unwrap();
...
async fn merge_continue_scopes_commit_to_merge_and_resolutions() {
    ...
    oak_cli::commands::merge::run(temp.path(), true, false, None)
        .await
        .unwrap();
...
async fn merge_continue_rejects_stray_conflict_marker_delimiter() {
    ...
    let err = oak_cli::commands::merge::run(temp.path(), true, false, None)
        .await
