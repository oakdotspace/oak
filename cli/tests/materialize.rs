...
use oak_core::{Commit, FileMode, Hash, ManifestEntry, MetadataKey};
...
async fn pull_fast_forward_applies_upstream_deletion() {
    ...
    let timestamp = chrono::DateTime::from_timestamp(1_700_000_400, 0).unwrap();
    let new_commit = Commit::with_timestamp(
        DEFAULT_BRANCH.to_string(),
        Some(old_head.clone()),
        None,
        new_manifest_hash.clone(),
        "peer".to_string(),
        None,
        Vec::new(),
        timestamp,
    ...
    assert_eq!(head_after_pull, new_head);
    drop(repo);
    oak_cli::commands::commit::run(temp.path()).unwrap();
    let repo = open_repo(temp.path());
    assert_eq!(
...
#[tokio::test(flavor = "current_thread")]
async fn sparse_pull_requests_cone_protocol_and_preserves_out_of_cone_identity() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());

    write_file(temp.path(), "src/app.rs", "local\n");
    write_file(temp.path(), "docs/guide.md", "old docs\n");
    oak_cli::commands::commit::run(temp.path()).unwrap();
    oak_cli::commands::sparse::run(
        temp.path(),
        oak_cli::commands::sparse::SparseAction::Set {
    ...
    repo.set_metadata(MetadataKey::RepoName, "oak").unwrap();
    let old_head = repo.get_branch_head(DEFAULT_BRANCH).unwrap().unwrap();
    let old_manifest = head_manifest(&repo, DEFAULT_BRANCH);
    let src_entry = old_manifest
        .entries
        .iter()
        .find(|entry| entry.path == "src/app.rs")
        .unwrap()
        .clone();
    let old_docs_hash = old_manifest
        .entries
        .iter()
        ...
        .blob_hash
        .clone();
    let upstream_docs_hash = oak_core::hash_bytes(b"upstream docs\n");
    let upstream_manifest_hash = repo
        .put_manifest(vec![
            src_entry,
            ManifestEntry {
                path: "docs/guide.md".to_string(),
                blob_hash: upstream_docs_hash.clone(),
                mode: FileMode::Regular,
            },
        ])
        .unwrap();
    let timestamp = chrono::DateTime::from_timestamp(1_700_000_450, 0).unwrap();
    let upstream = Commit::with_timestamp(
        DEFAULT_BRANCH.to_string(),
        Some(old_head.clone()),
        None,
        upstream_manifest_hash.clone(),
        "peer".to_string(),
        None,
        Vec::new(),
        timestamp,
    ...
    let connection = rusqlite::Connection::open(temp.path().join(".oak/oak.db")).unwrap();
    connection
        .execute(
            "DELETE FROM blobs WHERE hash = ?1",
            [old_docs_hash.as_str()],
        )
        .unwrap();
    drop(connection);
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            ...
            "commits": [{
                "hash": upstream.hash.as_str(),
                "branch_name": DEFAULT_BRANCH,
                "parent_hash": old_head.to_string(),
                "manifest_hash": upstream_manifest_hash.to_string(),
                "author": "peer",
                "timestamp": timestamp.to_rfc3339(),
                "files": [],
            }],
    ...
    let pulled_manifest = head_manifest(&repo, DEFAULT_BRANCH);
    assert_eq!(
        pulled_manifest
            .entries
            .iter()
            ...
            .blob_hash,
        upstream_docs_hash,
        "pull must retain the server's exact out-of-cone identity"
    );
    assert!(!repo.has_blob(&upstream_docs_hash).unwrap());
    assert!(!temp.path().join("docs/guide.md").exists());
    assert!(
    ...
    write_file(temp.path(), "src/app.rs", "local edit\n");
    oak_cli::commands::commit::run(temp.path()).unwrap();
    let repo = open_repo(temp.path());
    assert_eq!(
        head_manifest(&repo, DEFAULT_BRANCH)
            .entries
            .iter()
            .find(|entry| entry.path == "docs/guide.md")
            .unwrap()
            .blob_hash,
        upstream_docs_hash,
        "the next sparse commit must carry the pulled identity unchanged"
    );
}
...
#[tokio::test(flavor = "current_thread")]
async fn sparse_sync_carries_one_sided_out_of_cone_identity_without_its_bytes() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    let repo = open_repo(temp.path());
    repo.store_branch(&oak_core::Branch::new("dev".to_string(), None, None))
    ...
    let src = repo.put_blob(b"fn main() {}\n".to_vec()).unwrap();
    let base_docs = repo.put_blob(b"base docs\n".to_vec()).unwrap();
    let entry = |path: &str, hash: &Hash| ManifestEntry {
        path: path.to_string(),
    ...
    let base_manifest = repo
        .put_manifest(vec![
            entry("src/app.rs", &src),
            entry("docs/guide.md", &base_docs),
        ])
        .unwrap();
    let base = repo
        .put_commit(
    ...
    repo.set_head(&feature).unwrap();

    let upstream_docs = oak_core::hash_bytes(b"upstream docs\n");
    let parent_manifest = repo
        .put_manifest(vec![
            entry("src/app.rs", &src),
            entry("docs/guide.md", &upstream_docs),
        ])
        .unwrap();
    let parent = repo
        .put_commit(
        ...
        .unwrap();
    repo.set_branch_head("dev", &parent).unwrap();
    ...
    let connection = rusqlite::Connection::open(temp.path().join(".oak/oak.db")).unwrap();
    connection
        .execute("DELETE FROM blobs WHERE hash = ?1", [base_docs.as_str()])
        .unwrap();
    drop(connection);
    drop(repo);
    ...
    assert_eq!(
        merged
            ...
            .blob_hash,
        upstream_docs,
        "a one-sided upstream edit is merged by immutable identity"
    );
    assert!(!repo.has_blob(&upstream_docs).unwrap());
    assert!(!temp.path().join("docs/guide.md").exists());
}

#[tokio::test(flavor = "current_thread")]
async fn sparse_sync_fails_explicitly_when_out_of_cone_conflict_needs_missing_bytes() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    let repo = open_repo(temp.path());
    repo.store_branch(&oak_core::Branch::new("dev".to_string(), None, None))
    ...
    let src = repo.put_blob(b"fn main() {}\n".to_vec()).unwrap();
    let base_docs = repo.put_blob(b"base docs\n".to_vec()).unwrap();
    let feature_docs = repo.put_blob(b"feature docs\n".to_vec()).unwrap();
    let parent_docs = repo.put_blob(b"parent docs\n".to_vec()).unwrap();
    let entry = |path: &str, hash: &Hash| ManifestEntry {
        path: path.to_string(),
        blob_hash: hash.clone(),
        mode: FileMode::Regular,
    ...
    let feature_manifest = repo
        .put_manifest(vec![
            entry("src/app.rs", &src),
            entry("docs/guide.md", &feature_docs),
        ])
        .unwrap();
    let feature = repo
        .put_commit(
    ...
    let parent_manifest = repo
        .put_manifest(vec![
            entry("src/app.rs", &src),
            entry("docs/guide.md", &parent_docs),
        ])
        .unwrap();
    let parent = repo
        .put_commit(
    ...
    let err = oak_cli::commands::sync::sync_from_parent(temp.path())
        .await
        .expect_err("content-dependent conflict with missing bytes must fail");
    assert!(
        matches!(err, oak_core::OakError::IncompleteBlobData { .. }),
        "expected explicit incomplete content, got: {err}"
    );
    assert!(
        err.to_string()
            .contains("conflict resolution for 'docs/guide.md'"),
        "failure must identify the content-dependent conflict, got: {err}"
    );
    let repo = open_repo(temp.path());
    assert_eq!(repo.get_branch_head("feature").unwrap().unwrap(), feature);
    assert!(!temp.path().join(".oak/SYNC_HEAD").exists());
    assert!(!temp.path().join(".oak/SYNC_STATE").exists());
    let restored_base = repo.put_blob(b"base docs\n".to_vec()).unwrap();
    assert_eq!(restored_base, base_docs);
    drop(repo);

    let err = oak_cli::commands::sync::sync_from_parent(temp.path())
        .await
        .expect_err("an actual conflict outside the cone must not create hidden state");
    assert!(
        matches!(err, oak_core::OakError::MergeFailed(_)),
        "expected an explicit sparse conflict refusal, got: {err}"
    );
    assert!(
        err.to_string().contains("outside the active sparse cone"),
        "error must explain how to expose the conflict, got: {err}"
    );
    assert!(!temp.path().join(".oak/SYNC_HEAD").exists());
    assert!(!temp.path().join(".oak/SYNC_STATE").exists());
}
...
async fn merge_refuses_dirty_working_tree() {
    ...
    let err = oak_cli::commands::merge::run(temp.path(), false, false, None, None, false, false)
        .await
...
async fn merge_pushes_branch_before_server_merge() {
    ...
    oak_cli::commands::merge::run(temp.path(), false, false, None, None, false, false)
        .await
        .unwrap();
...
#[tokio::test(flavor = "current_thread")]
async fn merge_explicit_main_child_without_switching_checkout() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    ...
    let manifest_hash = repo
        ...
        .unwrap()
        .manifest_hash;
    let observer = oak_core::Branch::new("observer".to_string(), None, Some("main".to_string()));
    repo.store_branch(&observer).unwrap();
    repo.set_branch_head("observer", &branch_head).unwrap();
    repo.set_current_branch("observer").unwrap();
    drop(repo);
    ...
    let squash_hash = "0123012301230123012301230123012301230123012301230123012301230123";
    Mock::given(method("POST"))
        .and(path(format!(
            "/api/oak/oak/branches/{DEFAULT_BRANCH}/merge"
        )))
    ...
    assert!(
        repo.get_branch(DEFAULT_BRANCH).unwrap().is_none(),
        "explicitly merged branch is removed from local history"
    );
    assert_eq!(
        repo.get_branch_head("main").unwrap().unwrap(),
        Hash(squash_hash.to_string())
...
#[tokio::test(flavor = "current_thread")]
async fn sync_continue_rejects_stray_conflict_marker_delimiter() {
    let temp = setup_conflicted_sync().await;
    write_file(temp.path(), "conflict.txt", "resolved\n=======\n");

    let err = oak_cli::commands::sync::sync_continue(temp.path())
        .expect_err("stray separator delimiter must block sync continue");
    ...
    assert!(
        ...
        "failed continue must leave sync state intact"
    );
}

/// `oak merge --continue` commits the recorded merge result plus conflict
/// resolutions, not unrelated dirty files made while resolving.
#[tokio::test(flavor = "current_thread")]
async fn merge_continue_scopes_commit_to_merge_and_resolutions() {
    let temp = setup_conflicted_merge().await;

    write_file(temp.path(), "mine.txt", "dirty edit\n");
    write_file(temp.path(), "scratch.txt", "scratch\n");
    write_file(temp.path(), "conflict.txt", "resolved\n");

    oak_cli::commands::merge::run(temp.path(), true, false, None, None, false, false)
        .await
        .unwrap();

    let repo = open_repo(temp.path());
    let manifest = head_manifest(&repo, "dev");
    let by_path = |p: &str| manifest.entries.iter().find(|e| e.path == p);
    ...
    assert_eq!(
        repo.get_blob(&conflict.blob_hash).unwrap().unwrap().content,
        b"resolved\n",
        "the merge commit carries the user's resolution"
    );
    let clean = by_path("clean.txt").expect("parent's clean edit committed");
    assert_eq!(
        repo.get_blob(&clean.blob_hash).unwrap().unwrap().content,
    ...
    assert_eq!(
        repo.get_blob(&mine.blob_hash).unwrap().unwrap().content,
        b"tracked\n",
        "the unrelated dirty edit must NOT be committed by the merge"
    );
    assert!(
        by_path("scratch.txt").is_none(),
        "untracked scratch files must NOT be swept into the merge commit"
    );
    assert!(
        by_path("gone.txt").is_none(),
        "the parent's deletion lands in the merge commit"
    );
...
#[tokio::test(flavor = "current_thread")]
async fn merge_continue_rejects_stray_conflict_marker_delimiter() {
    let temp = setup_conflicted_merge().await;
    write_file(temp.path(), "conflict.txt", "resolved\n>>>>>>> main\n");

    let err = oak_cli::commands::merge::run(temp.path(), true, false, None, None, false, false)
        .await
        .expect_err("stray theirs delimiter must block merge continue");
    ...
    assert!(
        temp.path().join(".oak/MERGE_HEAD").exists(),
        "failed continue must leave merge state intact"
    );
}

async fn setup_conflicted_merge() -> TempDir {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    let repo = open_repo(temp.path());
    ...
    let err = oak_cli::commands::merge::run(temp.path(), false, false, None, None, false, false)
        .await
        .unwrap_err();
    assert!(matches!(err, oak_core::OakError::MergeConflict(1)));
    assert!(temp.path().join(".oak/MERGE_STATE").exists());
