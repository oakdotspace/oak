...
#[tokio::test(flavor = "current_thread")]
async fn mount_finish_unconfirmed_push_keeps_dirty_commit_state_without_blind_retry() {
    let server = MockServer::start().await;
    let temp_mnt = TempDir::new().unwrap();
    let dest = temp_mnt.path();
    let state_dir = build_mount_with_remote(dest, &server.uri());
    ...
    assert!(
        ...
        "should mention pending commits: {msg}"
    );
    assert!(
        msg.contains("unconfirmed"),
        "should name uncertainty: {msg}"
    );
    assert!(!msg.contains("SYNTHETIC_PRIVATE_ERROR_MARKER"));
    ...
    let page = oak_cli::publication_state::AttemptStore::mount(&state_dir)
        .page(None, 50)
        .unwrap();
    ...
    let restarted = oak_with_mounts_root(
        dest,
        &["agent", "state", "--json", "--compact"],
    ...
    assert_eq!(restarted["pending_publication_count"], 1);
    assert_eq!(restarted["publication_blocked"], true);
    assert_eq!(restarted["needs_push"], true);
    assert_eq!(restarted["finish_eligible"], false);
    assert!(!restarted["recommended_next_commands"]
        .as_array()
        .unwrap()
        .iter()
        .any(|command| command == "oak push" || command == "oak finish --json"));
}

#[tokio::test(flavor = "current_thread")]
async fn mount_finish_does_not_turn_an_unconfirmed_push_into_head_only_success() {
    ...
    let cfg = mount::state::load_config(&state_dir).unwrap();
    let cache = SqliteRepository::open_relaxed(&mount::state::cache_db_path(&state_dir)).unwrap();
    let local_head = cache.get_branch_head(&cfg.virtual_branch).unwrap().unwrap();
    drop(cache);
    ...
    Mock::given(method("GET"))
        ...
        .respond_with({
            ...
            let local_head = local_head.to_string();
            move |_: &wiremock::Request| {
                if accepted.load(std::sync::atomic::Ordering::SeqCst) {
                    ResponseTemplate::new(200).set_body_json(json!({ "head": local_head }))
                } else {
    ...
    let err = mount::finish_json(dest, "keep uncertain push evidence")
        .await
        .expect_err("an ambiguous publication must remain a failed push phase");
    let json = serde_json::to_value(output::JsonErrorEnvelope::from_error(&err)).unwrap();
    assert_eq!(json["error"]["finish"]["phase"], "push");
    assert_eq!(
        json["error"]["finish"]["completed_phases"],
    ...
    assert_eq!(
        mount::state::load_pushed_head(&state_dir),
        None,
        "head-only recovery must not manufacture an acknowledged mount receipt"
    );
    let page = oak_cli::publication_state::AttemptStore::mount(&state_dir)
        .page(None, 50)
        .unwrap();
...
async fn mount_finish_retry_after_push_failure_succeeds_and_ends() {
    ...
    Mock::given(method("POST"))
        .and(path("/api/oak/myrepo/push"))
        // An explicit rejection is safe for a deliberate later retry. The
        // separate unconfirmed-finish case covers ambiguous 5xx outcomes,
        // which retain durable evidence and must not be retried blindly.
...
#[tokio::test(flavor = "current_thread")]
async fn mount_finish_json_unconfirmed_metadata_sync_preserves_description() {
    let server = MockServer::start().await;
    let temp_mnt = TempDir::new().unwrap();
    let dest = temp_mnt.path();
    let state_dir = build_mount_with_remote(dest, &server.uri());
    ...
    assert_eq!(
        ...
        json!([
            format!("oak branch show {} --remote --json", cfg.virtual_branch),
            "oak mount status --json --compact"
        ])
    );
    assert!(!json.to_string().contains("SYNTHETIC_PRIVATE_ERROR_MARKER"));
    assert!(!json.to_string().contains("do-not-print"));
    let cache = SqliteRepository::open_relaxed(&mount::state::cache_db_path(&state_dir)).unwrap();
...
#[test]
fn end_refuses_to_erase_mount_publication_evidence() {
    let temp_mnt = TempDir::new().unwrap();
    let dest = temp_mnt.path();
    let state_dir = build_mount(dest);
    let cfg = mount::state::load_config(&state_dir).unwrap();
    ...
    let target = cache
        ...
        .to_string();
    let branch = cache.get_branch(&cfg.virtual_branch).unwrap().unwrap();
    let desired = oak_cli::publication_state::DesiredBranchState {
        head: target.clone(),
    ...
    let store = oak_cli::publication_state::store_for_work_tree(dest)
        .expect("registered mount selects the mount-state publication adapter");
    let _attempt = store
        .reserve(oak_cli::publication_state::NewAttempt {
            remote: &cfg.remote_url,
            owner: &cfg.owner,
            repo: &cfg.repo,
            branch: &cfg.virtual_branch,
            expected_predecessor: oak_cli::publication_state::ExpectedPredecessor::Absent,
            target_head: &target,
            desired_branch: &desired,
            transport: oak_cli::publication_state::PublicationTransport::Ordinary,
            request_body: b"{}",
            endpoint_path: "oak/myrepo",
        })
        .unwrap();
    assert_eq!(
        oak_cli::publication_state::AttemptStore::mount(&state_dir)
            .page(None, 50)
            .unwrap()
    ...
    let error = mount::end(dest, false).expect_err("retained evidence must block teardown");
    assert!(error.to_string().contains("publication record"));
    assert!(error.to_string().contains("oak agent state --refresh"));
    assert!(state_dir.exists());
    assert!(mount::state::lookup_id_for(dest).unwrap().is_some());
}
