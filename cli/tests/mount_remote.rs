...
async fn fetch_blob_sizes_batches_and_collects() {
    ...
    let sizes = remote::fetch_blob_sizes(&server.uri(), "oak", "myrepo", &hashes, None)
        .await
...
async fn fetch_blob_sizes_empty_input_is_noop() {
    ...
    let server = MockServer::start().await;
    let sizes = remote::fetch_blob_sizes(&server.uri(), "oak", "myrepo", &[], None)
        .await
...
async fn fetch_commit_into_cache_stores_commit_and_manifest() {
    ...
    let server = MockServer::start().await;
    let commit_hash = "deadbeef00000000".to_string();
    let manifest_hash = "abcd1234".to_string();
    let blob_hash = "f00f".to_string();
