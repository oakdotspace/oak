...
#[tokio::test]
async fn resolve_branch_head_encodes_explicit_branch_path_segment() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/api/oak/myrepo/branches/feature%2Fone%20two"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "name": "feature/one two",
            "description": null,
            "parent_branch": "main",
            "status": "open",
            "created_at": "2026-04-01T00:00:00Z",
    ...
    let (branch, head) = remote::resolve_branch_head(
        &server.uri(),
        "oak",
        "myrepo",
    ...
    .await
    .expect("should resolve explicit branch");
    assert_eq!(branch, "feature/one two");
    assert_eq!(head, "abc123def456");
}

#[tokio::test]
async fn resolve_branch_info_returns_full_record() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/api/oak/myrepo/branches/feature"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
    ...
    let info = remote::resolve_branch_info(&server.uri(), "oak", "myrepo", "feature", None)
        .await
        .expect("should resolve branch info");
    assert_eq!(info.name, "feature");
    assert_eq!(info.head.as_deref(), Some("abc123def456"));
    assert_eq!(info.parent_branch.as_deref(), Some("main"));
    assert_eq!(info.description.as_deref(), Some("fix the widget"));
    assert_eq!(info.status, "closed");
}

#[tokio::test]
async fn resolve_branch_info_defaults_status_open() {
    // Older servers omit status/description — parse leniently.
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/api/oak/myrepo/branches/feature"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
...
async fn fetch_commit_into_cache_stores_commit_and_manifest() {
    use oak_core::{Commit, FileMode, Hash, Tree, TreeEntry, TreeEntryKind};
    use oak_core::{Repository, SqliteRepository};
    use tempfile::TempDir;

    let server = MockServer::start().await;
    let blob_hash = Hash::from_hex(&"f00f".repeat(16)).unwrap();
    ...
    .unwrap();
    let manifest_hash = tree.hash.clone();
    let timestamp = chrono::DateTime::parse_from_rfc3339("2026-04-01T12:00:00Z")
        .unwrap()
    ...
    .unwrap()
    .hash;
    let tree_wire = oak_core::protocol::tree_to_wire(&tree);

    Mock::given(method("POST"))
        .and(path("/api/oak/myrepo/commits/info"))
        .and(body_partial_json(json!({"hashes": [commit_hash.as_str()]})))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "commits": [{
    ...
    assert_eq!(manifest.entries[0].blob_hash, blob_hash);

    assert!(
        cache.get_blob(&blob_hash).unwrap().is_none(),
