...
/// `NO_COLOR=1` *with* `CLICOLOR_FORCE=1`: the no-color spec wins, so this
/// also pins the precedence, not just the plain case.
fn oak_with_no_color(dir: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_oak"))
        .args(args)
        .current_dir(dir)
        .env("OAK_NO_UPDATE_CHECK", "1")
...
/// Subcommand help and `--version` are rendered by clap but re-emitted
/// through oak's output funnel (see `main`), so the ANSI-vs-plain choice is
/// oak's to make — and it must use oak's one color policy rather than a bare
/// TTY probe. Piped stdout with `CLICOLOR_FORCE=1` is exactly the case a TTY
/// probe gets wrong.
#[test]
fn intercepted_help_honors_the_color_environment_not_just_the_tty() {
    let temp = tempfile::TempDir::new().unwrap();

    // Piped, nothing forced: plain, like every other piped output.
    let plain = oak(temp.path(), &["diff", "--help"]);
    assert!(plain.status.success(), "stderr: {}", stderr(&plain));
    assert_no_ansi("diff --help", &stdout(&plain));

    // CLICOLOR_FORCE=1 with a piped (non-TTY) stdout: styled anyway.
    let forced = oak_with_forced_color(temp.path(), &["diff", "--help"]);
    assert!(forced.status.success(), "stderr: {}", stderr(&forced));
    assert!(
        stdout(&forced).contains('\u{1b}'),
        "CLICOLOR_FORCE=1 must style intercepted help even when piped, got: {:?}",
        stdout(&forced)
    );

    // NO_COLOR=1 wins over CLICOLOR_FORCE=1.
    let no_color = oak_with_no_color(temp.path(), &["diff", "--help"]);
    assert!(no_color.status.success(), "stderr: {}", stderr(&no_color));
    assert_no_ansi("NO_COLOR=1 diff --help", &stdout(&no_color));

    // The renderings differ only in styling — the plain one is what a piped
    // consumer sees, and the styled one is the same help with escapes woven
    // through it (so its words survive, even where escapes split them).
    assert_eq!(stdout(&plain), stdout(&no_color));
    assert!(
        stdout(&plain).contains("Usage: oak diff"),
        "{:?}",
        stdout(&plain)
    );
    assert!(
        stdout(&forced).contains("--exit-code"),
        "styled help must still be the same help: {:?}",
        stdout(&forced)
    );
}

/// Bare `oak mount` prints the mount subcommand's help through the same
/// funnel, so it follows the same color policy.
#[test]
fn bare_mount_help_honors_the_color_environment() {
    let temp = tempfile::TempDir::new().unwrap();

    let plain = oak(temp.path(), &["mount"]);
    assert!(plain.status.success(), "stderr: {}", stderr(&plain));
    assert_no_ansi("mount", &stdout(&plain));
    assert!(
        stdout(&plain).contains("Usage: mount"),
        "bare mount should print its help, got: {:?}",
        stdout(&plain)
    );

    let forced = oak_with_forced_color(temp.path(), &["mount"]);
    assert!(forced.status.success(), "stderr: {}", stderr(&forced));
    assert!(
        stdout(&forced).contains('\u{1b}'),
        "CLICOLOR_FORCE=1 must style bare mount help too, got: {:?}",
        stdout(&forced)
    );
}
...
#[test]
fn commit_message_like_positional_is_rejected_before_path_filter_noop() {
    let temp = fixture_repo();

    let out = oak(temp.path(), &["commit", "fix the thing"]);

    assert_eq!(out.status.code(), Some(2), "stderr: {}", stderr(&out));
    assert_eq!(stdout(&out), "", "usage errors should not print stdout");
    let err = stderr(&out);
    assert!(
        err.contains("takes path filters, not a message") && err.contains("oak desc --file"),
        "stderr: {err}"
    );
}
...
#[test]
fn diff_renders_multimegabyte_text_instead_of_binary_notice() {
    let temp = fixture_repo();
    let big = format!("{}\nchanged\n", "line\n".repeat(300_000));
    std::fs::write(temp.path().join("big.rs"), &big).unwrap();

    let text = stdout(&oak(temp.path(), &["diff", "--print", "big.rs"]));

    assert!(
        text.contains("diff --oak a/big.rs b/big.rs"),
        "missing path header: {}",
        &text[..text.len().min(200)]
    );
    assert!(
        text.contains("+changed"),
        "large text hunk was not rendered"
    );
    assert!(
        !text.contains("text diff omitted"),
        "large source file should not be treated as binary: {text}"
    );
}
...
#[test]
fn historical_diff_reports_missing_old_and_new_content_without_checkout_substitution() {
    for missing_content in [
        b"alpha\nbeta\n".as_slice(),
        b"alpha\nBETA\ngamma\n".as_slice(),
    ] {
        let (temp, first, second) = endpoint_fixture_repo();
        let conn = rusqlite::Connection::open(temp.path().join(".oak/oak.db")).unwrap();
        let missing = oak_core::hash_bytes(missing_content);
        assert_eq!(
            conn.execute("DELETE FROM blobs WHERE hash = ?1", [missing.as_str()])
                .unwrap(),
            1
        );
        drop(conn);
        std::fs::write(temp.path().join("base.txt"), "unrelated checkout\n").unwrap();
        for flags in [
            vec!["--print"],
            vec!["--stat"],
            vec![],
            ...
            let mut args = vec!["diff", &first, &second];
            args.extend(flags);
            let out = oak(temp.path(), &args);
            assert!(
                !out.status.success(),
                "missing historical object must fail: {}",
                stdout(&out)
            );
            assert!(
                stderr(&out).contains("Incomplete blob data"),
                "{}",
                stderr(&out)
            );
            assert!(stderr(&out).contains(missing.as_str()), "{}", stderr(&out));
            assert!(!stdout(&out).contains("unrelated checkout"));
            assert!(
                !stdout(&out).contains("@@"),
                "must not invent a partial patch: {}",
                stdout(&out)
            );
        }
    }
...
#[test]
fn diff_branch_json_covers_commits_and_dirty_worktree() {
    let temp = branch_fixture_repo();
    ...
    let before = oak(temp.path(), &["status", "--json"]);
    let out = oak(temp.path(), &["diff", "--branch", "--json", "--hunks"]);
    assert!(out.status.success(), "{}", stderr(&out));
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(json["kind"], "current_branch_diff");
    assert_eq!(json["diff_mode"], "contribution");
    assert_eq!(json["branch"], "feature");
    ...
    assert!(files
        .iter()
        .any(|f| f["path"] == "src/a.txt" && f["patch"].as_str().unwrap().contains("+B")));
    assert!(files
        .iter()
        .any(|f| f["path"] == "docs/x.md" && f["patch"].as_str().unwrap().contains("+y")));
    let lineage = &json["merge_lineage_evidence"];
    assert!(lineage["fork_point"].as_str().is_some());
    assert_eq!(lineage["comparison_source"], "working_tree_vs_fork_point");
    assert_eq!(
        before.stdout,
        oak(temp.path(), &["status", "--json"]).stdout
    );
}

#[test]
fn diff_branch_json_continuations_preserve_quoted_scope_and_rendering() {
    let temp = branch_fixture_repo();
    let scope = "work's dir";
    std::fs::create_dir(temp.path().join(scope)).unwrap();
    for name in ["a.txt", "b.txt"] {
        std::fs::write(temp.path().join(scope).join(name), "new text\n").unwrap();
    }
    let out = oak(
        temp.path(),
        &[
    ...
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(json["changed_file_count"], 2);
    let next = json["changed_files_page"]["next_page_command"]
        .as_str()
        .unwrap();
    assert_eq!(next, "oak diff --branch --json --changed-files-limit 1 --changed-files-offset 1 --hunks --max-bytes 0 -U 0 --text -- 'work'\\''s dir'");
    let patch = json["recommended_next_commands"]
        .as_array()
        .unwrap()
        .iter()
        .filter_map(|v| v.as_str())
        .find(|s| s.contains("a.txt"))
        .unwrap();
    assert_eq!(
        patch,
        "oak diff --branch --json --hunks -U 0 --text -- 'work'\\''s dir/a.txt'"
    );
    #[cfg(unix)]
    for command in [next, patch] {
        let result = Command::new("sh")
            .args([
                "-c",
                &format!(
                    "exec \"$OAK_TEST_BINARY\" {}",
                    command.strip_prefix("oak ").unwrap()
                ),
            ])
            .env("OAK_TEST_BINARY", env!("CARGO_BIN_EXE_oak"))
            .env("OAK_NO_UPDATE_CHECK", "1")
        ...
        assert_eq!(receipt["kind"], "current_branch_diff");
        for file in receipt["changed_files"].as_array().unwrap() {
            assert!(file["path"].as_str().unwrap().starts_with(scope));
        }
        if command == next {
            assert_eq!(receipt["changed_file_count"], 2);
            assert_eq!(receipt["changed_files"][0]["path"], "work's dir/b.txt");
            assert_eq!(receipt["hunks_truncated"], true);
        } else {
            assert!(receipt["changed_files"][0]["patch"]
                .as_str()
                .unwrap()
                .contains("+new text"));
        }
...
#[test]
fn diff_branch_json_keeps_sparse_scope_and_committed_changes() {
    ...
    drop(repo);
    std::fs::remove_file(temp.path().join("docs/x.md")).unwrap();
    let out = oak(temp.path(), &["diff", "--branch", "--json", "--hunks"]);
    assert!(out.status.success(), "{}", stderr(&out));
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(json["changed_file_count"], 1);
    assert_eq!(json["changed_files"][0]["path"], "src/a.txt");
    assert!(json["changed_files"][0]["patch"]
        .as_str()
        .unwrap()
        .contains("+B"));
...
#[test]
fn diff_branch_json_missing_fork_fails_without_snapshot_substitution() {
    use oak_core::{Repository, SqliteRepository};
    let temp = branch_fixture_repo();
    let repo = SqliteRepository::open(&temp.path().join(".oak/oak.db")).unwrap();
    let head = repo.get_branch_head("feature").unwrap().unwrap();
    let fork = repo
        .get_commit(&head)
        .unwrap()
        .unwrap()
    ...
    drop(repo);
    let conn = rusqlite::Connection::open(temp.path().join(".oak/oak.db")).unwrap();
    // Deliberately model an incomplete acquired graph in this owned fixture.
    conn.execute_batch("PRAGMA foreign_keys=OFF").unwrap();
    assert_eq!(
        conn.execute("DELETE FROM commits WHERE hash=?", [fork.as_str()])
            .unwrap(),
        1
    );
    drop(conn);
    ...
    assert!(!out.status.success());
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_ne!(json["kind"], "current_branch_diff");
    assert!(stdout(&out).contains("contribution"), "{}", stdout(&out));
    assert_eq!(
        std::fs::read_to_string(temp.path().join("docs/x.md")).unwrap(),
        "x\ny\n"
    );
}

#[test]
fn diff_branch_json_clean_branch_and_noncurrent_endpoint_remain_distinct() {
    let temp = branch_fixture_repo();
    assert!(oak(temp.path(), &["commit"]).status.success());
    let out = oak(temp.path(), &["diff", "--branch", "--json", "--exit-code"]);
    ...
    assert!(!json["recommended_next_commands"]
        ...
        .iter()
        .any(|v| v == "oak commit"));
    assert!(oak(temp.path(), &["switch", "-c", "other"])
        .status
        .success());
    std::fs::write(temp.path().join("other.txt"), "unrelated local change\n").unwrap();
    let named = oak(temp.path(), &["diff", "feature", "--json"]);
    assert!(named.status.success(), "{}", stderr(&named));
    assert!(!stdout(&named).contains("other.txt"));
    let invalid = oak(temp.path(), &["diff", "feature", "--branch", "--json"]);
    assert_eq!(invalid.status.code(), Some(2));
    let plain = oak(temp.path(), &["diff", "--json"]);
    let plain: serde_json::Value = serde_json::from_slice(&plain.stdout).unwrap();
    assert_eq!(plain["kind"], "working_tree_diff");
    assert_eq!(plain["changed_files"][0]["path"], "other.txt");
}

#[test]
fn diff_branch_json_retains_empty_binary_and_renamed_paths() {
    let temp = branch_fixture_repo();
    std::fs::rename(
    ...
    .unwrap();
    // Exact rename relative to the fork, not a tiny below-threshold similarity guess.
    std::fs::write(temp.path().join("src/renamed.txt"), "a\nb\n").unwrap();
    std::fs::write(temp.path().join("src/empty.txt"), b"").unwrap();
    std::fs::write(temp.path().join("src/binary.bin"), b"\0\xffbinary").unwrap();
    let out = oak(temp.path(), &["diff", "--branch", "--json", "--hunks"]);
    assert!(out.status.success(), "{}", stderr(&out));
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    let files = json["changed_files"].as_array().unwrap();
    assert!(files
        .iter()
        .any(|f| f["path"] == "src/empty.txt" && f["status"] == "added"));
    assert!(
        files
            .iter()
            .any(|f| f["path"] == "src/binary.bin" && f["binary_or_large"] == true),
        "{json}"
    );
    assert!(files
        .iter()
        .any(|f| f["path"] == "src/renamed.txt" && f["old_path"] == "src/a.txt"));
}

#[test]
fn diff_branch_json_empty_contribution_and_parentless_branch_do_not_invent_a_base() {
    use oak_core::{Repository, SqliteRepository};
    let temp = branch_fixture_repo();
    std::fs::write(temp.path().join("src/a.txt"), "a\nb\n").unwrap();
    std::fs::write(temp.path().join("docs/x.md"), "x\n").unwrap();
    let out = oak(temp.path(), &["diff", "--branch", "--json", "--exit-code"]);
    assert_eq!(out.status.code(), Some(0));
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(json["changed_file_count"], 0);
    let repo = SqliteRepository::open(&temp.path().join(".oak/oak.db")).unwrap();
...
#[test]
fn diff_branch_json_headless_branch_has_a_proven_empty_base() {
    let temp = tempfile::TempDir::new().unwrap();
    ...
    assert!(out.status.success(), "{}", stderr(&out));
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert!(json["branch_head"].is_null());
    assert!(json["merge_lineage_evidence"]["fork_point"].is_null());
    assert_eq!(json["changed_file_count"], 1);
    assert!(json["changed_files"][0]["patch"]
        .as_str()
        .unwrap()
...
#[cfg(unix)]
#[test]
fn diff_branch_json_mode_only_patch_retains_executable_transition() {
    use std::os::unix::fs::PermissionsExt;
    let temp = branch_fixture_repo();
    let file = temp.path().join("src/a.txt");
    // Remove the text edit so the only visible src change is mode metadata.
    std::fs::write(&file, "a\nb\n").unwrap();
    std::fs::set_permissions(&file, std::fs::Permissions::from_mode(0o755)).unwrap();
    let before = oak(temp.path(), &["status", "--json"]);
    let out = oak(
        temp.path(),
        &["diff", "--branch", "--json", "--hunks", "--", "src/a.txt"],
    );
    assert!(out.status.success(), "{}", stderr(&out));
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    let patch = json["changed_files"][0]["patch"].as_str().unwrap();
...
#[cfg(unix)]
#[test]
fn diff_branch_json_rename_mode_and_symlink_headers_share_captured_renderer() {
    ...
    for edited in [false, true] {
        ...
        assert!(oak(temp.path(), &["init", "."]).status.success());
        let content = (0..50)
            .map(|n| format!("unique important line {n}\n"))
            .collect::<String>();
        std::fs::write(temp.path().join("old.txt"), &content).unwrap();
        std::fs::set_permissions(
            temp.path().join("old.txt"),
            std::fs::Permissions::from_mode(0o644),
        )
        .unwrap();
        symlink("old.txt", temp.path().join("link")).unwrap();
        assert!(oak(temp.path(), &["commit"]).status.success());
        assert!(oak(temp.path(), &["switch", "-c", "feature"])
            .status
            .success());
        // Establish a branch-owned commit so this local-only fixture has a
        // recorded fork even though its server-owned main has no local head.
        std::fs::write(temp.path().join("branch.txt"), "branch\n").unwrap();
        assert!(oak(temp.path(), &["commit"]).status.success());
        std::fs::rename(
            temp.path().join("old.txt"),
        ...
        if edited {
            std::fs::write(
                temp.path().join("new name.txt"),
                content.replace("line 25", "line twentyfive"),
            )
            .unwrap();
            assert!(oak(temp.path(), &["commit"]).status.success());
            std::fs::OpenOptions::new()
                .append(true)
                .open(temp.path().join("new name.txt"))
                .unwrap()
        ...
        symlink("new name.txt", temp.path().join("link")).unwrap();
        let before = oak(temp.path(), &["status", "--json"]);
        let out = oak(temp.path(), &["diff", "--branch", "--json", "--hunks"]);
        assert!(out.status.success(), "{}", stderr(&out));
        let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
        let files = json["changed_files"].as_array().unwrap();
        let rename = files.iter().find(|f| f["path"] == "new name.txt").unwrap();
        let patch = rename["patch"].as_str().unwrap();
        for header in [
            "rename from old.txt",
            ...
            assert!(patch.contains(header), "edited={edited}; {header}: {json}");
        }
        if edited {
            assert!(patch.contains("+unique important line twentyfive"));
            assert!(patch.contains("+dirty tail"));
        }
        let link = files.iter().find(|f| f["path"] == "link").unwrap()["patch"]
            .as_str()
            .unwrap();
        assert!(
            link.contains("-old.txt") && link.contains("+new name.txt"),
            "{link}"
        );
        assert!(
            !link.contains("unique important line"),
            "must render symlink target, not referent"
        );
        assert_eq!(
            before.stdout,
            oak(temp.path(), &["status", "--json"]).stdout
...
#[cfg(unix)]
#[test]
fn diff_branch_human_footer_preserves_quoted_paths_in_json_advice() {
    let temp = branch_fixture_repo();
    std::fs::create_dir(temp.path().join("with space")).unwrap();
    std::fs::write(temp.path().join("with space/file.txt"), "scoped\n").unwrap();
    let out = oak(temp.path(), &["diff", "--branch", "--", "with space"]);
    assert!(out.status.success());
    let text = stdout(&out);
    let command = text
        .lines()
        .find_map(|line| {
            line.split_once(" · structured: ")
    ...
    assert_eq!(json["changed_file_count"], 1);
    assert_eq!(json["changed_files"][0]["path"], "with space/file.txt");
}

#[cfg(unix)]
#[test]
fn diff_branch_json_patch_advice_executes_from_a_subdirectory() {
    let temp = branch_fixture_repo();
    ...
    assert!(out.status.success(), "{}", stderr(&out));
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    let command = json["recommended_next_commands"]
        .as_array()
        .unwrap()
        .iter()
        .filter_map(|v| v.as_str())
    ...
    assert_eq!(receipt["changed_files"][0]["path"], "src/a.txt");
    assert!(receipt["changed_files"][0]["patch"]
        .as_str()
        .unwrap()
        .contains("+B"));
...
// ---------------------------------------------------------------------------
//
// The reader side of a pipeline is allowed to stop early — `oak diff --print
// | head -1` — and that is not an error. The contract: when stdout's reader
// goes away, oak silences further output, RUNS THE COMMAND TO COMPLETION,
// and exits with the operation's own outcome (0 on success). It must never
// surface Rust's default `println!` behavior (a "failed printing to stdout:
// Broken pipe" panic on stderr with a nonzero exit), and it must never skip
// the operation itself — mutating commands print before they mutate. Pinned
// shapes: a pipe closed *before* the first write, one that closes while the
// writer is blocked mid-stream on a full pipe (the original fb-106 repro),
// and mutators that must still mutate after the reader hangs up.

/// Spawn `oak` in `dir` with the given stdout, deterministic env (matching
/// [`oak`]), and piped stderr; return the finished [`Output`].
fn oak_with_stdout(
    dir: &Path,
    args: &[&str],
...
fn assert_broken_pipe_contract(label: &str, out: &Output) {
    assert_eq!(
        out.status.code(),
        Some(0),
        "{label}: a vanished stdout reader must not change the outcome \
         (no panic's 101, no SIGPIPE death)\nstderr:\n{}",
        stderr(out)
    );
    assert!(
        !stderr(out).contains("panic"),
        "{label}: no panic text may reach stderr, got:\n{}",
        stderr(out)
    );
    assert_eq!(
        stderr(out),
        "",
        "{label}: a broken pipe is the reader's choice, not a diagnostic"
    );
}

/// Pipe whose read end is already closed when oak starts: the very first
/// stdout write sees EPIPE. Deterministic for any output size.
fn run_against_closed_pipe(dir: &Path, args: &[&str]) -> Output {
    let (rx, tx) = std::io::pipe().expect("pipe");
    drop(rx);
    let child = oak_with_stdout(dir, args, tx);
    // The write end also lives in `child`'s Stdio only; wait_with_output
    // collects stderr (stdout was never piped to us).
    child.wait_with_output().expect("oak binary should run")
}

#[test]
fn diff_print_into_closed_pipe_exits_zero_without_panic() {
    let temp = fixture_repo();
    let out = run_against_closed_pipe(temp.path(), &["diff", "--print"]);
    assert_broken_pipe_contract("diff --print", &out);
}

#[test]
fn log_into_closed_pipe_exits_zero_without_panic() {
    let temp = fixture_repo();
    let out = run_against_closed_pipe(temp.path(), &["log"]);
    assert_broken_pipe_contract("log", &out);
}

#[test]
fn help_into_closed_pipe_exits_zero_without_panic() {
    let temp = tempfile::TempDir::new().unwrap();
    let out = run_against_closed_pipe(temp.path(), &["--help"]);
    assert_broken_pipe_contract("--help", &out);
}

/// The original fb-106 shape: `oak diff --print | head -1`. The diff is made
/// larger than any OS pipe buffer so oak is still blocked writing when the
/// reader quits after one line — the EPIPE lands mid-stream, not up front.
#[test]
fn diff_print_mid_stream_reader_exit_is_clean() {
    ...
    let mut child = oak_with_stdout(
        temp.path(),
        &["diff", "--print", "big.rs"],
        std::process::Stdio::piped(),
    );
    let mut reader = std::io::BufReader::new(child.stdout.take().expect("stdout should be piped"));
    let mut first_line = String::new();
    reader
        .read_line(&mut first_line)
        .expect("oak should produce at least one line");
    assert!(
        first_line.contains("diff --oak a/big.rs b/big.rs"),
        "got: {first_line}"
    );
    drop(reader); // hang up: oak's blocked write now fails with EPIPE

    let out = child.wait_with_output().expect("oak binary should run");
    assert_broken_pipe_contract("diff --print | head -1", &out);
}

/// The reason a vanished reader must not abort the process: state-changing
/// commands print before they mutate. `oak reset --force x | head -0` must
/// still reset `x` — the command runs to completion with output silenced.
#[test]
fn reset_with_closed_pipe_still_resets_the_file() {
    let temp = tempfile::TempDir::new().unwrap();
    let dir = temp.path();
    assert!(oak(dir, &["init", "."]).status.success());
    ...
    let out = run_against_closed_pipe(dir, &["reset", "--force", "tracked.txt"]);

    assert_broken_pipe_contract("reset --force", &out);
    assert_eq!(
        std::fs::read_to_string(dir.join("tracked.txt")).unwrap(),
        "committed\n",
        "the mutation must happen even though stdout's reader hung up"
    );
}

#[test]
fn restore_with_closed_pipe_still_restores_the_file() {
    let temp = tempfile::TempDir::new().unwrap();
    let dir = temp.path();
    assert!(oak(dir, &["init", "."]).status.success());
    std::fs::write(dir.join("tracked.txt"), "committed\n").unwrap();
    ...
    let out = run_against_closed_pipe(dir, &["restore", "--force", "tracked.txt"]);

    assert_broken_pipe_contract("restore --force", &out);
    assert_eq!(
        std::fs::read_to_string(dir.join("tracked.txt")).unwrap(),
        "committed\n",
        "the mutation must happen even though stdout's reader hung up"
...
/// A stdout that is genuinely unwritable — as opposed to merely unread.
///
/// The write end of the pipe is made non-blocking and the pipe is filled to
/// capacity before oak starts, with the read end kept open and unread: the
/// child's first stdout flush fails with EAGAIN (`WouldBlock`), a real
/// non-`BrokenPipe` I/O failure. (A read-only fd can't serve here: Rust's
/// std deliberately masks EBADF on the standard streams.) Contract under
/// test: genuine output loss is reported on stderr and a would-be success
/// exits nonzero — unlike a vanished reader, which stays a silent success.
#[cfg(unix)]
fn run_against_unwritable_stdout(dir: &Path, args: &[&str]) -> Output {
    use std::io::Write as _;
    ...
    loop {
        match tx.write(&chunk) {
            ...
            Err(e) => panic!("filling pipe: {e}"),
        }
    }

    let child = oak_with_stdout(dir, args, tx);
    let out = child.wait_with_output().expect("oak binary should run");
    drop(rx); // keep the reader attached (but idle) for the child's lifetime
    out
}

#[cfg(unix)]
fn assert_output_loss_reported(label: &str, out: &Output) {
    assert_ne!(
        out.status.code(),
        Some(0),
        "{label}: output was genuinely lost — that must not exit 0\nstderr:\n{}",
        stderr(out)
    );
    assert!(
        stderr(out).contains("failed writing to stdout"),
        "{label}: the loss must be reported on stderr, got:\n{}",
        stderr(out)
    );
    assert!(
        !stderr(out).contains("panic"),
        "{label}: reported as a structured error, not a panic:\n{}",
        stderr(out)
    );
}

/// Top-level help exits without reaching main's result epilogue; it must
/// still notice that the help never arrived.
#[cfg(unix)]
#[test]
fn help_with_unwritable_stdout_fails_loudly() {
    let temp = tempfile::TempDir::new().unwrap();
    let out = run_against_unwritable_stdout(temp.path(), &["--help"]);
    assert_output_loss_reported("--help", &out);
}

/// Subcommand help renders through the intercepted clap path (see main.rs);
/// same contract as top-level help.
#[cfg(unix)]
#[test]
fn subcommand_help_with_unwritable_stdout_fails_loudly() {
    let temp = tempfile::TempDir::new().unwrap();
    let out = run_against_unwritable_stdout(temp.path(), &["diff", "--help"]);
    assert_output_loss_reported("diff --help", &out);
}

/// `oak diff --exit-code` terminates through an explicit checked exit
/// (`output::exit_process`), not main's epilogue: its data code survives,
/// and the loss is still reported on stderr.
#[cfg(unix)]
#[test]
fn exit_code_diff_with_unwritable_stdout_reports_loss() {
    let temp = fixture_repo();
    let out = run_against_unwritable_stdout(temp.path(), &["diff", "--exit-code"]);
    assert_output_loss_reported("diff --exit-code", &out);
    assert_eq!(
        out.status.code(),
        Some(1),
        "the differences-exist data code must survive\nstderr:\n{}",
        stderr(&out)
    );
}

/// The `--json` error envelope is written to *stdout*, and it is the whole
/// error report a machine consumer gets. When stdout is genuinely unwritable
/// that envelope never arrives, so the failing exit must not be silent: the
/// envelope path leaves through the checked exit, which reports the loss on
/// stderr while keeping the error's own exit code.
#[cfg(unix)]
#[test]
fn json_error_envelope_with_unwritable_stdout_reports_loss() {
    // No repo here, so `oak log --json` fails and emits the envelope.
    let temp = tempfile::TempDir::new().unwrap();
    let out = run_against_unwritable_stdout(temp.path(), &["log", "--json"]);
    assert_output_loss_reported("log --json", &out);
    assert_eq!(
        out.status.code(),
        Some(1),
        "the error's own exit code must survive\nstderr:\n{}",
        stderr(&out)
    );
}

/// A vanished reader is still not an error on the envelope path: the JSON
/// consumer hung up, which is its right.
#[test]
fn json_error_envelope_into_closed_pipe_stays_quiet() {
    let temp = tempfile::TempDir::new().unwrap();
    let out = run_against_closed_pipe(temp.path(), &["log", "--json"]);
    assert_eq!(
        out.status.code(),
        Some(1),
        "the error's own exit code must survive\nstderr:\n{}",
    ...
    assert_eq!(
        stderr(&out),
        "",
        "a vanished reader is not a diagnostic, got:\n{}",
        stderr(&out)
    );
}

/// Bare `oak mount` prints help through the funnel rather than clap's own
/// stdout writer (whose discarded `io::Result` swallowed both halves of the
/// contract): a vanished reader stays a silent success, and genuine loss is
/// reported with a nonzero exit.
#[test]
fn bare_mount_help_into_closed_pipe_exits_zero_without_panic() {
    let temp = tempfile::TempDir::new().unwrap();
    let out = run_against_closed_pipe(temp.path(), &["mount"]);
    assert_broken_pipe_contract("mount", &out);
}

#[cfg(unix)]
#[test]
fn bare_mount_help_with_unwritable_stdout_fails_loudly() {
    let temp = tempfile::TempDir::new().unwrap();
    let out = run_against_unwritable_stdout(temp.path(), &["mount"]);
    assert_output_loss_reported("mount", &out);
}

/// `oak split` without `--plan` is an interactive crossterm TUI that writes
/// stdout directly, bypassing the output funnel — it must refuse to start
/// when stdout is not a terminal instead of writing escape sequences (or
/// panicking) into a pipe.
#[test]
fn split_refuses_non_tty_stdout_before_any_crossterm_write() {
    let temp = tempfile::TempDir::new().unwrap();
    let dir = temp.path();
    assert!(oak(dir, &["init", "."]).status.success());
    ...
    assert_eq!(stdout(&out), "", "no TUI bytes may reach a piped stdout");
    assert!(
        stderr(&out).contains("--plan"),
        "the refusal must name the non-interactive alternative:\n{}",
        stderr(&out)
    );
}

// ---------------------------------------------------------------------------
// Destructive confirmations need BOTH streams, not just a TTY stdin
// ---------------------------------------------------------------------------
//
// `oak reset` / `oak restore` print "Are you sure…? [y/N] " on *stdout* and
// read the answer from *stdin*. Gating the prompt on stdin alone leaves a real
// hazard: a TTY stdin with a redirected or dead stdout shows the user nothing
// while still accepting their "y", so a destructive reset can be confirmed
// blind. Before the output funnel, `print!` + `flush()?` at least aborted when
// stdout was unwritable; the funnel deliberately swallows that failure to keep
// commands running, so the gate has to be explicit (`output::can_prompt`).
// Contract: no prompt without both streams — refuse with a structured error
// naming `--force`, and never hang. `--force` itself is unchanged: it skips
// the question entirely and still mutates under a dead stdout (above).

/// A real pty pair. The slave end is a genuine terminal, so a child given it
/// as stdin sees `stdin().is_terminal() == true` — the half of the hazard a
/// plain pipe cannot reproduce.
#[cfg(unix)]
struct Pty {
...
/// `ptsname` returns a pointer into a static buffer, so the open sequence is
/// serialized across the (multi-threaded) test harness.
#[cfg(unix)]
...
fn open_pty() -> Pty {
    ...
    unsafe {
        ...
        assert_eq!(
            libc::grantpt(std::os::fd::AsRawFd::as_raw_fd(&master)),
            0,
            "grantpt failed"
        );
        assert_eq!(
            libc::unlockpt(std::os::fd::AsRawFd::as_raw_fd(&master)),
            0,
            "unlockpt failed"
        );
        ...
        assert!(slave >= 0, "opening pty slave failed");
        Pty {
            master,
            slave: std::os::fd::OwnedFd::from_raw_fd(slave),
        }
    }
}

/// Run `oak` with a **terminal stdin** (pty slave) and a **dead stdout** (pipe
/// whose reader is already gone), with `answer` pre-queued on the terminal.
///
/// Pre-queueing the answer is what keeps this test honest: a build that still
/// prompts finds a "y" waiting and proceeds, so the regression shows up as a
/// destroyed file and a zero exit — a failed assertion — instead of a hang.
#[cfg(unix)]
fn run_with_tty_stdin_and_dead_stdout(dir: &Path, args: &[&str], answer: &str) -> Output {
    use std::io::Write as _;

    let pty = open_pty();
    {
        let mut master = std::fs::File::from(pty.master.try_clone().expect("dup pty master"));
        master
            .write_all(answer.as_bytes())
            .expect("queueing the answer on the terminal");
        master.flush().expect("flush pty master");
    }

    let (rx, tx) = std::io::pipe().expect("pipe");
    drop(rx); // stdout's reader is gone before oak writes a byte
...
#[cfg(unix)]
fn dirty_repo_with_one_tracked_file() -> tempfile::TempDir {
    let temp = tempfile::TempDir::new().unwrap();
    let dir = temp.path();
    assert!(oak(dir, &["init", "."]).status.success());
    std::fs::write(dir.join("tracked.txt"), "committed\n").unwrap();
...
/// The blind-confirmation shape: the prompt is invisible, so oak must not ask.
#[cfg(unix)]
fn assert_refused_naming_force(label: &str, out: &Output, dir: &Path) {
    assert_eq!(
        out.status.code(),
        Some(4),
        "{label}: an unshowable confirmation must refuse with the dirty-tree \
         exit code, not proceed\nstderr:\n{}",
        stderr(out)
    );
    assert!(
        stderr(out).contains("--force"),
        "{label}: the refusal must name the non-interactive escape hatch:\n{}",
        stderr(out)
    );
    assert!(
        !stderr(out).contains("panic"),
        "{label}: structured error, not a panic:\n{}",
        stderr(out)
    );
    assert_eq!(
        std::fs::read_to_string(dir.join("tracked.txt")).unwrap(),
        "dirty\n",
        "{label}: nothing may be discarded on a confirmation the user never saw"
    );
}

#[cfg(unix)]
#[test]
fn reset_refuses_to_confirm_when_stdout_cannot_show_the_prompt() {
    let temp = dirty_repo_with_one_tracked_file();
    let out = run_with_tty_stdin_and_dead_stdout(temp.path(), &["reset", "tracked.txt"], "y\n");
    assert_refused_naming_force("reset", &out, temp.path());
}

#[cfg(unix)]
#[test]
fn restore_refuses_to_confirm_when_stdout_cannot_show_the_prompt() {
    let temp = dirty_repo_with_one_tracked_file();
    let out = run_with_tty_stdin_and_dead_stdout(temp.path(), &["restore", "tracked.txt"], "y\n");
    assert_refused_naming_force("restore", &out, temp.path());
}

/// The other half of the contract: `--force` asks nothing, so a dead stdout is
/// just silenced output — the mutation still happens and the exit stays 0,
/// even with a terminal stdin in the mix.
#[cfg(unix)]
#[test]
fn forced_reset_still_mutates_with_tty_stdin_and_dead_stdout() {
    let temp = dirty_repo_with_one_tracked_file();
    let out =
        run_with_tty_stdin_and_dead_stdout(temp.path(), &["reset", "--force", "tracked.txt"], "");
    assert_broken_pipe_contract("reset --force (tty stdin)", &out);
    assert_eq!(
        std::fs::read_to_string(temp.path().join("tracked.txt")).unwrap(),
        "committed\n",
        "--force skips the prompt, so a dead stdout must not block the mutation"
    );
}

#[cfg(unix)]
#[test]
fn forced_restore_still_mutates_with_tty_stdin_and_dead_stdout() {
    let temp = dirty_repo_with_one_tracked_file();
    let out =
        run_with_tty_stdin_and_dead_stdout(temp.path(), &["restore", "--force", "tracked.txt"], "");
    assert_broken_pipe_contract("restore --force (tty stdin)", &out);
    assert_eq!(
        std::fs::read_to_string(temp.path().join("tracked.txt")).unwrap(),
        "committed\n",
        "--force skips the prompt, so a dead stdout must not block the mutation"
