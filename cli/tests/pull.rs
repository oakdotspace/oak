...
use oak_cli::output;
use oak_core::{
    Branch, ChangeType, FileChange, FileMode, Hash, ManifestEntry, MetadataKey, OakError,
...
async fn mount_moved_seed_server(
    ...
    Mock::given(method("POST"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/api/oak/oak/raw/{seed}/{theirs_path}")))
...
async fn pull_force_parks_local_commits_instead_of_discarding() {
    ...
    // with an empty tree.
    let remote_commit: String = "c".repeat(64);
    Mock::given(method("GET"))
        .and(path("/api/oak/oak/pull"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "head": remote_commit,
