...
async fn run_linked_push_json_with_response(
    response: ResponseTemplate,
) -> (TempDir, std::process::Output, String, String, usize) {
    run_linked_push_json_with_response_and_receipt_obstruction(response, false).await
}

async fn run_linked_push_json_with_response_and_receipt_obstruction(
    response: ResponseTemplate,
    obstruct_receipt: bool,
) -> (TempDir, std::process::Output, String, String, usize) {
    let temp = TempDir::new().unwrap();
    oak_cli::commands::init::run(temp.path(), false).unwrap();
    std::fs::write(temp.path().join("hello.txt"), "preserve this work\n").unwrap();
    ...
    if obstruct_receipt {
        std::fs::create_dir(temp.path().join(".oak/LAST_PUSHED_HEAD.json")).unwrap();
    }
    ...
    assert_eq!(
        requests.len(),
        3,
        "durability tracking must not add normal-path HTTP requests"
    );
    let publication_requests = requests
        .into_iter()
        .filter(|request| {
            request.method.as_str() == "POST" && request.url.path().ends_with("/push")
        })
        .count();
    (temp, command, branch, head, publication_requests)
}

#[tokio::test]
async fn lost_publication_reply_leaves_durable_restart_evidence_without_replay() {
    ...
    let server = std::thread::spawn(move || {
        for _ in 0..4 {
            ...
            if method == "POST" && path == "/api/agents/widget/push" {
                server_posts.fetch_add(1, Ordering::SeqCst);
                // The server accepted and consumed the complete body, then the
                // transport disappeared before response headers.
                drop(stream);
                break;
            }
            let (status, body) =
                if method == "GET" && path.starts_with("/api/agents/widget/branches/") {
                    ("404 Not Found", "{}")
                } else if method == "GET" && path == "/api/agents/widget" {
                    ("200 OK", r#"{"head":null}"#)
                } else {
    ...
    .unwrap();
    server.join().unwrap();
    assert!(!output.status.success(), "lost reply must remain uncertain");
    assert_eq!(posts.load(Ordering::SeqCst), 1, "mutation must not replay");

    let page = oak_cli::publication_state::AttemptStore::checkout(&temp.path().join(".oak"))
        .page(None, 50)
        .unwrap();
    assert_eq!(page.pending_publication_count, 1);
    assert_eq!(
        page.pending_publications[0].state,
        oak_cli::publication_state::AttemptState::SentUnconfirmed
    );
    assert_eq!(page.pending_publications[0].owner, "agents");
    assert_eq!(page.pending_publications[0].repo, "widget");

    std::fs::write(
        temp.path().join("after.txt"),
        "new work after uncertainty\n",
    )
    .unwrap();
    oak_cli::commands::commit::run(temp.path()).unwrap();
    let head_after = repo
        .get_branch_head(&page.pending_publications[0].branch)
        .unwrap()
        .unwrap()
        .to_string();
    assert_ne!(head_after, page.pending_publications[0].target_head);

    let state = std::process::Command::new(env!("CARGO_BIN_EXE_oak"))
        .args(["agent", "state", "--json", "--compact"])
    ...
    assert_eq!(state["pending_publication_count"], 1);
    assert_eq!(
        state["pending_publications"][0]["acknowledgement"],
        "unconfirmed"
    );
    assert_eq!(state["finish_eligible"], false);
    assert_eq!(state["needs_push"], true);
    assert_eq!(state["head"], head_after);
    assert_eq!(state["publication_blocked"], true);
    assert_eq!(
    ...
    assert!(!state["recommended_next_commands"]
        ...
        .unwrap()
        .iter()
        .any(|command| command == "oak push"));

    let operation_id = page.pending_publications[0].operation_id.clone();
    let refused = std::process::Command::new(env!("CARGO_BIN_EXE_oak"))
        .args(["agent", "publication", "forget", &operation_id, "--json"])
        .current_dir(temp.path())
        .env("OAK_NO_UPDATE_CHECK", "1")
        .output()
        .unwrap();
    ...
    assert_eq!(
        oak_cli::publication_state::AttemptStore::checkout(&temp.path().join(".oak"))
            .page(None, 50)
            .unwrap()
            .pending_publication_count,
        1
    );
    let forgotten = std::process::Command::new(env!("CARGO_BIN_EXE_oak"))
        .args([
            "agent",
            "publication",
            "forget",
    ...
    let forgotten: serde_json::Value = serde_json::from_slice(&forgotten.stdout).unwrap();
    assert_eq!(forgotten["operation_id"], operation_id);
    assert_eq!(forgotten["outcome"], "unknown_acknowledged_by_user");
    assert_eq!(forgotten["remote_contacted"], false);
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn push_json_ambiguous_server_error_is_redacted_unconfirmed_and_not_retried() {
    let temp = TempDir::new().unwrap();
    oak_cli::commands::init::run(temp.path(), false).unwrap();
    std::fs::write(temp.path().join("hello.txt"), "preserve this work\n").unwrap();
    oak_cli::commands::commit::run(temp.path()).unwrap();
    ...
    assert_eq!(command.status.code(), Some(6));
    assert!(command.stderr.is_empty());
    let output: serde_json::Value = serde_json::from_slice(&command.stdout).unwrap();
    assert_eq!(output["error"]["code"], "publication_unconfirmed");
    assert!(output["error"]["message"]
        .as_str()
        .unwrap()
        .contains("outcome is unconfirmed"));
    assert!(output["error"]["recommended_next_commands"]
        .as_array()
        .unwrap()
        .iter()
        .any(|command| command == &format!("oak branch show {branch} --remote --json")));
    let diagnostics = format!(
        "{}{}",
        String::from_utf8_lossy(&command.stdout),
        String::from_utf8_lossy(&command.stderr)
    );
    assert!(!diagnostics.contains("SYNTHETIC_PRIVATE_ERROR_MARKER"));
    assert!(!diagnostics.contains("do-not-print"));
    assert!(!temp.path().join(".oak/LAST_PUSHED_HEAD.json").exists());
    assert_eq!(
        repo.get_branch_head(&branch).unwrap().unwrap().to_string(),
        head
    );
    ...
    let state: serde_json::Value = serde_json::from_slice(&state.stdout).unwrap();
    assert_eq!(state["needs_push"], true);
    assert!(state["current_branch_push_source"].is_null());
}

#[tokio::test(flavor = "current_thread")]
async fn push_json_redirect_timeout_and_rate_limit_are_unconfirmed_without_retry() {
    let cases = [
        ResponseTemplate::new(302).insert_header(
            "location",
            "https://private.example.invalid/?access_token=do-not-print",
        ),
        ResponseTemplate::new(408).set_body_string("SYNTHETIC_PRIVATE_ERROR_MARKER timeout detail"),
        ResponseTemplate::new(429).set_body_string("SYNTHETIC_PRIVATE_ERROR_MARKER rate detail"),
    ];
    for response in cases {
        let (_temp, command, _branch, _head, publication_requests) =
            run_linked_push_json_with_response(response).await;
        assert_eq!(command.status.code(), Some(6));
        let json: serde_json::Value = serde_json::from_slice(&command.stdout).unwrap();
        assert_eq!(json["error"]["code"], "publication_unconfirmed");
        let diagnostics = format!(
            "{}{}",
            String::from_utf8_lossy(&command.stdout),
            String::from_utf8_lossy(&command.stderr)
        );
        assert!(!diagnostics.contains("private.example.invalid"));
        assert!(!diagnostics.contains("SYNTHETIC_PRIVATE_ERROR_MARKER"));
        assert!(!diagnostics.contains("do-not-print"));
        assert_eq!(publication_requests, 1);
    }
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn push_json_oversized_success_receipt_is_unconfirmed_without_body_disclosure() {
    let temp = TempDir::new().unwrap();
    oak_cli::commands::init::run(temp.path(), false).unwrap();
    std::fs::write(temp.path().join("hello.txt"), "preserve this work\n").unwrap();
    oak_cli::commands::commit::run(temp.path()).unwrap();
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "success": true,
            "new_head": null,
            "message": format!("SYNTHETIC_PRIVATE_ERROR_MARKER{}", "x".repeat(70 * 1024))
        })))
        .expect(1)
        .mount(&server)
        .await;
    ...
    assert!(output["error"]["message"]
        .as_str()
        .unwrap()
        .contains("receipt exceeded 65536 bytes"));
    assert!(!String::from_utf8_lossy(&command.stdout).contains("SYNTHETIC_PRIVATE_ERROR_MARKER"));
    assert!(!temp.path().join(".oak/LAST_PUSHED_HEAD.json").exists());
}

#[tokio::test(flavor = "current_thread")]
async fn push_json_malformed_success_receipt_is_redacted_unconfirmed() {
    let (_temp, command, _branch, _head, publication_requests) =
        run_linked_push_json_with_response(
            ResponseTemplate::new(200)
                .set_body_string(r#"{"success":true,"new_head":"SYNTHETIC_PRIVATE_ERROR_MARKER""#),
        )
        .await;
    assert_eq!(command.status.code(), Some(6));
    let json: serde_json::Value = serde_json::from_slice(&command.stdout).unwrap();
    ...
    assert!(json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("valid receipt"));
    assert!(!String::from_utf8_lossy(&command.stdout).contains("SYNTHETIC_PRIVATE_ERROR_MARKER"));
    assert_eq!(publication_requests, 1);
}

#[tokio::test(flavor = "current_thread")]
async fn valid_legacy_null_ack_retires_attempt_without_extra_http() {
    let (temp, command, _branch, _head, publication_requests) = run_linked_push_json_with_response(
        ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "success": true,
            "new_head": null,
            "message": "ok"
...
#[tokio::test(flavor = "current_thread")]
async fn full_publication_record_capacity_refuses_before_any_mutation_post() {
    let temp = TempDir::new().unwrap();
    oak_cli::commands::init::run(temp.path(), false).unwrap();
    std::fs::write(temp.path().join("hello.txt"), "capacity guard\n").unwrap();
    oak_cli::commands::commit::run(temp.path()).unwrap();
    let repo = SqliteRepository::open(&temp.path().join(".oak/oak.db")).unwrap();
    let branch = repo.get_current_branch_name().unwrap().unwrap();
    let server = MockServer::builder().start().await;
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let records = temp.path().join(".oak/publication-attempts");
    ...
    assert!(json["error"]["message"]
        .as_str()
        .unwrap()
        .contains("publication_state_full"));
    assert!(server
        .received_requests()
        .await
        .unwrap()
...
#[tokio::test(flavor = "current_thread")]
async fn acknowledged_publication_with_failed_local_receipt_stays_durable_without_retry_advice() {
    let (temp, command, _branch, _head, publication_requests) =
        run_linked_push_json_with_response_and_receipt_obstruction(
            ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "success": true,
                "new_head": null,
                "message": "ok"
    ...
    assert!(diagnostics.contains("acknowledgement was valid"));
    assert!(diagnostics.contains("do not retry the mutation"));
    assert!(diagnostics.contains("durable operation"));
    ...
    assert_eq!(
        page.pending_publications[0].state,
        oak_cli::publication_state::AttemptState::Acknowledged
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn push_json_non_null_acknowledgement_must_match_the_submitted_target() {
    for acknowledged in ["not-a-commit-hash".to_string(), "a".repeat(64)] {
        let (temp, command, _branch, head, publication_requests) =
            run_linked_push_json_with_response(ResponseTemplate::new(200).set_body_json(
                serde_json::json!({
                    "success": true,
                    "new_head": acknowledged,
                    "message": "SYNTHETIC_PRIVATE_ERROR_MARKER"
                }),
            ))
            .await;
        ...
        assert!(output["error"]["message"]
            .as_str()
            .unwrap()
            .contains("acknowledged head did not match the submitted target"));
        assert!(
            !String::from_utf8_lossy(&command.stdout).contains("SYNTHETIC_PRIVATE_ERROR_MARKER")
        );
        assert_eq!(publication_requests, 1);
        assert!(!temp.path().join(".oak/LAST_PUSHED_HEAD.json").exists());
        let repo = SqliteRepository::open(&temp.path().join(".oak/oak.db")).unwrap();
        assert_eq!(
            repo.get_branch_head(&repo.get_current_branch_name().unwrap().unwrap())
                .unwrap()
                .unwrap()
                .to_string(),
            head
