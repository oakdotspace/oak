...
#[tokio::test]
async fn base_tree_hydration_does_not_require_unused_deleted_blob_content() {
...
async fn run(triage: bool, partial_tree: bool, missing_blob: bool) {
    ...
    if !triage {
        assert_eq!(
            json["merge_preview"]["prediction_available"], true,
            "{json}"
        );
        // Both heads identically deleted dir/file. No review content reader
        // needs its former bytes; metadata alone proves that deletion.
        oak_cli::output::begin_capture();
        oak_cli::commands::review::remote_branch_review_json(
            temp.path(),
            "feature",
