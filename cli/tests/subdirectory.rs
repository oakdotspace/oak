...
fn git_backed_read_commands_do_not_create_oak_sidecar() {
    ...
    oak_cli::commands::log::run(temp.path(), Some(1), false, true, &[], None, None).unwrap();
...
fn status_log_diff_from_subdirectory() {
    ...
    oak_cli::commands::log::run(&nested, None, false, false, &[], None, None).unwrap();
    oak_cli::commands::diff::run(&nested, &[], false, false, false).unwrap();
}
