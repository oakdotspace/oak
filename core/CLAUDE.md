...
oak commit          # local checkpoint only (no message; never pushes)
oak log             # show commit history
oak push            # publish checkpoints to the remote server
...
Commit freely — local checkpoints stay isolated until you `oak push`,
`oak commit --push`, `oak merge`, or open a PR on oak.space.
