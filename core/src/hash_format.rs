...
mod tests {
    ...
    fn v1_empty_golden() {
        ...
        // `Blob::empty_hash` is the sentinel the client matches on before it
        // will synthesize a blob the server never sent (`ensure_empty_blob`).
        // Pin it here so a hash-format change can't quietly turn that
        // synthesis into "reconstruct the wrong bytes for some other hash".
        assert_eq!(crate::Blob::empty_hash().as_str(), V1_EMPTY);
        assert_eq!(crate::Blob::empty().hash.as_str(), V1_EMPTY);
        assert!(crate::Blob::is_empty_hash(&crate::Blob::empty_hash()));
        assert!(!crate::Blob::is_empty_hash(&hash_blob(
            HashFormat::V1,
            b"not empty"
        )));
    }
