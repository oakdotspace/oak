...
impl BulkImporter {
    ...
    pub fn put_blob(&mut self, content: Vec<u8>) -> Result<Hash> {
        ...
        self.conn
            .execute(
                "INSERT OR IGNORE INTO blobs (hash, content, size) VALUES (?1, ?2, ?3)",
                params![blob.hash.as_str(), blob.content, blob.size as i64],
            )
            .map_err(|e| OakError::Database(e.to_string()))?;
        Ok(blob.hash)
    }
    ...
    pub fn put_tree(&mut self, entries: Vec<ManifestEntry>) -> Result<Hash> {
        ...
        for tree in &built.trees {
            ...
            for entry in &tree.entries {
                let mode_str = match (entry.kind, entry.mode) {
                ...
                self.conn
                    .execute(
                        "INSERT OR IGNORE INTO tree_entries (tree_hash, name, kind, hash, mode) VALUES (?1, ?2, ?3, ?4, ?5)",
                        params![
                            tree.hash.as_str(),
                            entry.name,
                            entry.kind.as_str(),
...
impl Repository for SqliteRepository {
    ...
    fn get_blob(&self, hash: &Hash) -> Result<Option<Blob>> {
        ...
        let result: Option<(Vec<u8>, i64)> = conn
            .query_row(
                "SELECT content, size FROM blobs WHERE hash = ?1",
                params![hash.as_str()],
        ...
        Ok(result.map(|(content, size)| Blob {
            hash: hash.clone(),
            content,
            size: size as u64,
        }))
    ...
    fn get_tree(&self, hash: &Hash) -> Result<Option<Tree>> {
        ...
        let entries: Vec<TreeEntry> = stmt
        ...
        // Re-sort to match Tree::new canonical ordering. The PRIMARY KEY guarantees
        // uniqueness but not the specific row order in the result.
