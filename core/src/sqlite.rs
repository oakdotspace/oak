...
impl Repository for SqliteRepository {
    ...
    fn put_blob_sources(&self, sources: Vec<BlobSource>) -> Result<Vec<Hash>> {
        ...
        let (producer_result, consumer_result) = std::thread::scope(|scope| {
            ...
            budget.close();
            (
                producer.join().expect("blob batch producer panicked"),
                consumer_result,
            )
        });
        consumer_result?;
    ...
    fn store_branch(&self, branch: &Branch) -> Result<()> {
        ...
        conn.execute(
            ...
            params![
                ...
                branch.close_reason.map(|r| r.as_str()),
    ...
    fn get_branch(&self, name: &str) -> Result<Option<Branch>> {
        let conn = self.conn.lock().unwrap();
        let result: Option<(Option<String>, Option<String>, String, Option<String>, String)> = conn
            .query_row(
    ...
    fn reconcile_remote_merge_branch_state(
        ...
        tx.execute(
            ...
            params![
                ...
                new_branch.close_reason.map(|r| r.as_str()),
