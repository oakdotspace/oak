...
use crate::{
    Blob, Branch, BranchStatus, CanonicalChangeSetV1, ChangeScopeV1, ChangeType, ChunkInfo,
    CloseReason, Commit, FileChange, FileMode, Hash, Manifest, ManifestEntry, MetadataKey,
    OakError, Result, Tag, Tree, TreeEntry, TreeEntryKind, CHANGE_SET_OBJECT_FORMAT_V1,
};
...
fn advance_description_generation(conn: &Connection) -> Result<i64> {
    conn.query_row(
        "UPDATE branch_description_clock SET generation = generation + 1
         WHERE singleton = 1 AND generation < 9223372036854775807 RETURNING generation",
        [],
        |row| row.get(0),
    )
    .map_err(|error| {
        OakError::Database(format!(
            "could not advance branch description generation: {error}"
        ))
    })
}
...
pub struct ChangeCaptureProvenance {
    pub owner: String,
    pub name: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct StoredChangeCapture {
    pub capture_id: String,
    pub change_set: CanonicalChangeSetV1,
    pub provenance: ChangeCaptureProvenance,
}

struct ChangeCaptureRow {
    change_id: String,
    format_version: i64,
    owner: String,
    name: String,
...
const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: "0012_pending_branch_description",
        sql: include_str!("migrations/sqlite/0012_pending_branch_description.sql"),
        required: true,
    },
    Migration {
        version: "0013_branch_description_generation",
        sql: include_str!("migrations/sqlite/0013_branch_description_generation.sql"),
        required: true,
    },
    Migration {
        version: "0014_change_captures",
        sql: include_str!("migrations/sqlite/0014_change_captures.sql"),
        required: true,
    },
];
...
fn migration_already_applied(version: &str, conn: &Connection) -> Result<bool> {
    match version {
        ...
        "0012_pending_branch_description" => column_exists(conn, "branches", "description_pending"),
        _ => Ok(false),
    }
}
...
fn capture_integrity(capture_id: &str, detail: &str) -> OakError {
...
fn validate_capture_id(capture_id: &str) -> Result<()> {
    let suffix = capture_id
        ...
        .ok_or_else(|| capture_integrity(capture_id, "invalid capture occurrence identifier"))?;
    if suffix.len() != 32
        || !suffix
            .bytes()
            .all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
    {
        return Err(capture_integrity(
            capture_id,
            "invalid capture occurrence identifier",
        ));
    }
    Ok(())
}

fn parse_native_hash(capture_id: &str, value: &str, label: &str) -> Result<Hash> {
    if value.len() != 64
        || !value
            .bytes()
            .all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
    ...
    Hash::from_hex(value)
        .map_err(|_| capture_integrity(capture_id, &format!("invalid {label} identity")))
}

fn validate_capture_segment(capture_id: &str, value: &str, label: &str) -> Result<()> {
    if value.is_empty()
        || value.len() > 255
        || value.trim() != value
        || value == "."
            ...
            .any(|c| c.is_whitespace() || c.is_control() || c == '/' || c == '\\')
    {
        return Err(capture_integrity(
            capture_id,
            &format!("invalid stored {label}"),
        ));
    }
    Ok(())
}

fn validate_capture_branch(capture_id: &str, branch: &str) -> Result<()> {
    if branch.is_empty()
        || branch.trim() != branch
        || branch
            .chars()
            .any(|c| c.is_whitespace() || c.is_control() || c == '/' || c == '\\')
    {
        return Err(capture_integrity(
            capture_id,
            "invalid stored source branch",
        ));
    }
    Ok(())
}

fn validate_capture_origin(capture_id: &str, origin: &str) -> Result<()> {
    if origin.len() > 2_048
        ...
        || origin.contains(['?', '#'])
    {
        return Err(capture_integrity(
            capture_id,
            "invalid stored repository origin",
        ));
    }
    let authority_and_path = origin
        .strip_prefix("https://")
        .or_else(|| origin.strip_prefix("http://"))
        .ok_or_else(|| capture_integrity(capture_id, "invalid stored repository origin"))?;
...
fn validate_capture_provenance(
    ...
    validate_capture_segment(capture_id, owner, "repository owner")?;
    validate_capture_segment(capture_id, name, "repository name")?;
    validate_capture_branch(capture_id, source_branch)?;
    if let Some(origin) = origin {
        validate_capture_origin(capture_id, origin)?;
    }
    if let Some(base_commit) = base_commit {
        parse_native_hash(capture_id, base_commit.as_str(), "base commit")?;
    }
    Ok(())
}

fn effective_head_in_connection(conn: &Connection, branch: &str) -> Result<Option<Hash>> {
    ...
    loop {
        ...
        if let Some(head) = head {
            return Ok(Some(Hash(head)));
        }
        let parent: Option<Option<String>> = conn
            .query_row(
                "SELECT parent_branch FROM branches WHERE name = ?1",
                [current.as_str()],
                |row| row.get(0),
            )
            .optional()
...
fn validate_capture_base_in_connection(
    conn: &Connection,
    capture_id: &str,
    base_commit: Option<&Hash>,
    base_tree: &Hash,
    ...
    let Some(base_commit) = base_commit else {
        if base_tree != &Tree::empty_hash() {
            return Err(capture_integrity(
                capture_id,
                "a headless capture must use the canonical empty base tree",
            ));
        }
        return Ok(());
    };
    let manifest_hash: Option<String> = conn
        .query_row(
            "SELECT manifest_hash FROM commits WHERE hash = ?1",
            [base_commit.as_str()],
            |row| row.get(0),
        )
        .optional()
    ...
    let manifest_hash = parse_native_hash(capture_id, &manifest_hash, "base commit tree")?;
    if &manifest_hash != base_tree {
        return Err(capture_integrity(
            capture_id,
            "the descriptor base tree does not match the recorded base commit",
        ));
    }
    Ok(())
}
...
impl SqliteRepository {
    /// Inspect an existing repository in one SQLite read transaction. Does not
    /// create a database, migrate schema, chmod, or change journal mode. Normal
    /// SQLite locking remains enabled (WAL coordination sidecars may be used).
    /// Mutating Repository methods fail on this connection.
    pub fn open_read_only(db_path: &Path) -> Result<Self> {
        ...
        conn.execute_batch("BEGIN DEFERRED")
            .map_err(|e| OakError::Database(e.to_string()))?;
        // Pin the read snapshot now. Refuse an unknown/partial schema rather
        // than migrating it or interpreting missing columns as empty state.
        {
            let mut stmt = conn
                .prepare("SELECT version, succeeded FROM schema_migrations LIMIT 1000")
                .map_err(|e| OakError::Database(e.to_string()))?;
            let rows = stmt
            ...
            if !versions
                .iter()
                .any(|(version, ok)| *ok && version == "0000_baseline")
                || versions
                    .iter()
                    .any(|(version, ok)| !ok || !MIGRATIONS.iter().any(|m| m.version == version))
            {
                return Err(OakError::Database(
                    "unsupported schema for read-only inspection".into(),
                ));
            }
        }
        Ok(Self {
    ...
    /// Atomically publish a complete canonical descriptor and one immutable
    /// local capture occurrence after rechecking the branch snapshot inside
    /// the write transaction. Identical portable changes share one descriptor;
    /// provenance rows are never replaced or deduplicated.
    #[allow(clippy::too_many_arguments)]
    pub fn store_change_capture(
        &self,
        capture_id: &str,
        ...
        validate_capture_provenance(capture_id, owner, name, origin, source_branch, base_commit)?;
        let (scope_kind, scope_paths_json, descriptor_json) =
            self.validate_change_set_objects(capture_id, change_set)?;
        ...
        let current_branch: Option<String> = tx
            .query_row(
                "SELECT value FROM metadata WHERE key = ?1",
                [MetadataKey::CurrentBranch.as_str()],
                |row| row.get(0),
            )
            .optional()
            .map_err(|error| OakError::Database(error.to_string()))?;
        let current_head = effective_head_in_connection(&tx, source_branch)?;
        if current_branch.as_deref() != Some(source_branch) || current_head.as_ref() != base_commit
        {
            return Err(OakError::WorktreeCaptureRaced {
                detail: "the current branch or effective HEAD changed before durable capture publication"
                    .to_string(),
            });
        }
        validate_capture_base_in_connection(&tx, capture_id, base_commit, change_set.base_tree())?;

        tx.execute(
            "INSERT OR IGNORE INTO change_sets
             (change_id, schema_version, object_format, base_tree_hash, result_tree_hash,
              scope_kind, scope_paths_json, descriptor_json, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)",
            params![
                change_set.id().as_str(),
                i64::from(change_set.schema_version()),
                change_set.object_format(),
                change_set.base_tree().as_str(),
                change_set.result_tree().as_str(),
                scope_kind,
                scope_paths_json,
                descriptor_json,
                now,
        ...
        let stored: (i64, String, String, String, String, String, String) = tx
            .query_row(
                "SELECT schema_version, object_format, base_tree_hash, result_tree_hash,
                        scope_kind, scope_paths_json, descriptor_json
                 FROM change_sets WHERE change_id = ?1",
                [change_set.id().as_str()],
                |row| {
                    Ok((
                        row.get(0)?,
            ...
            .map_err(|_| capture_integrity(capture_id, "stored descriptor row is unreadable"))?;
        if stored
            != (
                i64::from(change_set.schema_version()),
                change_set.object_format().to_string(),
                change_set.base_tree().to_string(),
                change_set.result_tree().to_string(),
                scope_kind.to_string(),
                scope_paths_json.clone(),
                descriptor_json.clone(),
            )
        {
            return Err(capture_integrity(
                capture_id,
                "change ID already names a different or corrupt descriptor",
            ));
        }

        tx.execute(
            "INSERT INTO change_captures
             (capture_id, change_id, format_version, repo_owner, repo_name, origin,
              source_branch, base_commit_hash, captured_at)
        ...
        .map_err(|_| {
            capture_integrity(
                capture_id,
                "capture occurrence ID already exists or provenance could not be stored",
            )
        })?;
        tx.commit()
            .map_err(|error| OakError::Database(error.to_string()))?;
    ...
    /// Resolve one durable capture occurrence and rederive its complete
    /// canonical descriptor from the immutable base/result tree objects.
    /// Working-tree bytes are never rescanned and inherited blobs are not
    /// hydrated by this lookup.
    pub fn get_change_capture(&self, capture_id: &str) -> Result<StoredChangeCapture> {
        if validate_capture_id(capture_id).is_err() {
            return Err(OakError::ChangeCaptureNotFound {
                capture_id: capture_id.to_string(),
            });
        }
        let conn = self.conn.lock().unwrap();
        let occurrence: Option<ChangeCaptureRow> = conn
            .query_row(
                "SELECT change_id, format_version, repo_owner, repo_name, origin,
                        source_branch, base_commit_hash, captured_at
                 FROM change_captures WHERE capture_id = ?1",
                [capture_id],
                |row| {
                    Ok(ChangeCaptureRow {
                        change_id: row.get(0)?,
            ...
            .optional()
            .map_err(|_| capture_integrity(capture_id, "capture occurrence row is unreadable"))?;
        let Some(ChangeCaptureRow {
            change_id,
            format_version,
            owner,
            name,
        ...
        if format_version != 1 {
            return Err(capture_integrity(
                capture_id,
                "unsupported capture occurrence format",
            ));
        }
        let base_commit = base_commit
            .as_deref()
            .map(|value| parse_native_hash(capture_id, value, "base commit"))
            .transpose()?;
        ...
        let captured_at = chrono::DateTime::parse_from_rfc3339(&captured_at)
            .map_err(|_| capture_integrity(capture_id, "invalid capture timestamp"))?
            .with_timezone(&chrono::Utc);

        let descriptor: Option<ChangeSetRow> = conn
            .query_row(
                "SELECT schema_version, object_format, base_tree_hash, result_tree_hash,
                        scope_kind, scope_paths_json, descriptor_json, created_at
                 FROM change_sets WHERE change_id = ?1",
                ...
                |row| {
                    Ok(ChangeSetRow {
                        ...
                        object_format: row.get(1)?,
                        base_tree: row.get(2)?,
                        result_tree: row.get(3)?,
                        scope_kind: row.get(4)?,
                        scope_paths_json: row.get(5)?,
                        descriptor_json: row.get(6)?,
        ...
        let Some(ChangeSetRow {
            ...
            return Err(capture_integrity(
                capture_id,
                "capture occurrence references a missing descriptor",
            ));
        };
        if schema_version != 1 || object_format != CHANGE_SET_OBJECT_FORMAT_V1 {
            return Err(capture_integrity(
                capture_id,
                "unsupported or corrupt change descriptor format",
            ));
        }
        chrono::DateTime::parse_from_rfc3339(&descriptor_created_at)
            .map_err(|_| capture_integrity(capture_id, "invalid descriptor timestamp"))?;
        let base_tree = parse_native_hash(capture_id, &base_tree, "base tree")?;
        let result_tree = parse_native_hash(capture_id, &result_tree, "result tree")?;
        let stored_change_id = parse_native_hash(capture_id, &change_id, "change ID")?;
        let paths: Vec<String> = serde_json::from_str(&scope_paths_json)
            .map_err(|_| capture_integrity(capture_id, "invalid descriptor scope"))?;
        let scope = match scope_kind.as_str() {
            "full" if scope_paths_json == "[]" => ChangeScopeV1::Full,
            "paths" => ChangeScopeV1::Paths { paths },
            _ => return Err(capture_integrity(capture_id, "invalid descriptor scope")),
        };
        let base_manifest = self
            .get_manifest(&base_tree)
            .map_err(|_| capture_integrity(capture_id, "base tree could not be validated"))?
            .ok_or_else(|| capture_integrity(capture_id, "base tree is missing"))?;
        let result_manifest = self
            .get_manifest(&result_tree)
            .map_err(|_| capture_integrity(capture_id, "result tree could not be validated"))?
            .ok_or_else(|| capture_integrity(capture_id, "result tree is missing"))?;
        let change_set =
            CanonicalChangeSetV1::from_manifests(&base_manifest, &result_manifest, scope)
                .map_err(|_| capture_integrity(capture_id, "descriptor could not be rederived"))?;
        let canonical_json = serde_json::to_string(&change_set)
            .map_err(|_| capture_integrity(capture_id, "descriptor could not be encoded"))?;
        let canonical_scope_json = match change_set.scope() {
            ChangeScopeV1::Full => "[]".to_string(),
            ChangeScopeV1::Paths { paths } => serde_json::to_string(paths).map_err(|_| {
                capture_integrity(capture_id, "descriptor scope could not be encoded")
            })?,
        };
        let expected_scope_kind = match change_set.scope() {
            ChangeScopeV1::Full => "full",
            ChangeScopeV1::Paths { .. } => "paths",
        };
        if change_set.id() != &stored_change_id
            || change_set.base_tree() != &base_tree
            || change_set.result_tree() != &result_tree
            || scope_kind != expected_scope_kind
            || scope_paths_json != canonical_scope_json
            || descriptor_json != canonical_json
        {
            return Err(capture_integrity(
                capture_id,
                "stored descriptor does not match its canonical tree-derived identity",
            ));
        }
        match base_commit.as_ref() {
            Some(base_commit) => {
                let commit = self
                    .get_commit(base_commit)
                    .map_err(|_| {
                        capture_integrity(capture_id, "the recorded base commit is unavailable")
                    })?
                    .ok_or_else(|| {
                        capture_integrity(capture_id, "the recorded base commit is unavailable")
                    })?;
                if commit.manifest_hash != base_tree {
                    return Err(capture_integrity(
                        capture_id,
                        "the descriptor base tree does not match the recorded base commit",
                    ));
            ...
            None if base_tree != Tree::empty_hash() => {
                return Err(capture_integrity(
                    capture_id,
                    "a headless capture must use the canonical empty base tree",
                ));
        ...
        Ok(StoredChangeCapture {
            capture_id: capture_id.to_string(),
            change_set,
            provenance: ChangeCaptureProvenance {
                owner,
                name,
                origin,
                source_branch,
    ...
    fn validate_change_set_objects(
        &self,
        capture_id: &str,
        change_set: &CanonicalChangeSetV1,
    ) -> Result<(&'static str, String, String)> {
        let base = self
            .get_manifest(change_set.base_tree())
            .map_err(|_| capture_integrity(capture_id, "base tree could not be validated"))?
            .ok_or_else(|| capture_integrity(capture_id, "base tree is missing"))?;
        let result = self
            ...
            .ok_or_else(|| capture_integrity(capture_id, "result tree is missing"))?;
        let rederived =
            CanonicalChangeSetV1::from_manifests(&base, &result, change_set.scope().clone())
                .map_err(|_| capture_integrity(capture_id, "descriptor could not be rederived"))?;
        if &rederived != change_set {
            return Err(capture_integrity(
                capture_id,
                "descriptor does not match its canonical tree-derived identity",
            ));
        }
        let (scope_kind, scope_paths_json) = match change_set.scope() {
            ChangeScopeV1::Full => ("full", "[]".to_string()),
            ChangeScopeV1::Paths { paths } => (
                "paths",
        ...
        let descriptor_json = serde_json::to_string(change_set)
            .map_err(|_| capture_integrity(capture_id, "descriptor could not be encoded"))?;
        Ok((scope_kind, scope_paths_json, descriptor_json))
    }
    ...
    /// Unfenced compatibility entry point: inserts missing branches and refreshes
    /// lifecycle fields, but conservatively preserves every existing description.
    /// This cannot safely refresh remote description text without a fetch-start token.
    #[deprecated(
        note = "capture description_generation before fetching and use upsert_branch_metadata_at_generation"
    )]
    pub fn upsert_branch_metadata(&self, branch: &Branch) -> Result<()> {
        self.reconcile_branch_metadata(branch, true, -1).map(|_| ())
    }

    /// Reconcile cached metadata with a remote row, retaining pending local descriptions.
    ///
    ...
    /// refresh lifecycle fields, but a locally edited description remains owned
    /// by the user until observed/published remotely with that exact value.
    /// Returns false when a newer local generation fenced out this response's
    /// description. Lifecycle reconciliation still follows the caller policy.
    pub fn upsert_branch_metadata_at_generation(
        &self,
        branch: &Branch,
        observed_generation: i64,
    ) -> Result<bool> {
        self.reconcile_branch_metadata(branch, true, observed_generation)
    }

    /// Pull preserves existing local lifecycle decisions (e.g. an unsynced
    /// close) while refreshing descriptions that have no pending local edit.
    /// Returns false if the response predates this branch's local generation.
    pub fn reconcile_pulled_branch_metadata(
        &self,
        branch: &Branch,
        observed_generation: i64,
    ) -> Result<bool> {
        self.reconcile_branch_metadata(branch, false, observed_generation)
    }

    /// Capture before issuing a remote metadata request; never reconstruct this
    /// token from local state after the response arrives.
    pub fn description_generation(&self) -> Result<i64> {
        self.conn
            .lock()
            .unwrap()
            .query_row(
                "SELECT generation FROM branch_description_clock WHERE singleton = 1",
                [],
                |row| row.get(0),
            )
            .map_err(|error| OakError::Database(error.to_string()))
    ...
    fn reconcile_branch_metadata(
        ...
        let tx = conn
            .savepoint()
            .map_err(|error| OakError::Database(error.to_string()))?;
        let generation = advance_description_generation(&tx)?;
        let fresh = tx
            .query_row(
                "SELECT description_generation <= ?2 FROM branches WHERE name = ?1",
                params![branch.name, observed_generation],
            ...
            .unwrap_or(true);
        tx.execute(
            "INSERT INTO branches (name, description, parent_branch, status, close_reason, created_at, description_generation) \
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?9) \
             ON CONFLICT(name) DO UPDATE SET \
               description = CASE WHEN branches.description_generation <= ?8 AND branches.description_pending = 0 THEN excluded.description ELSE branches.description END, \
               description_pending = CASE WHEN branches.description_generation <= ?8 AND branches.description IS excluded.description THEN 0 ELSE branches.description_pending END, \
               description_generation = CASE WHEN branches.description_generation <= ?8 AND (branches.description_pending = 0 OR branches.description IS excluded.description) THEN ?9 ELSE branches.description_generation END, \
               parent_branch = CASE WHEN ?7 THEN excluded.parent_branch ELSE branches.parent_branch END, \
               status = CASE WHEN ?7 THEN excluded.status ELSE branches.status END, \
               close_reason = CASE WHEN ?7 THEN excluded.close_reason ELSE branches.close_reason END, \
               created_at = CASE WHEN ?7 THEN excluded.created_at ELSE branches.created_at END",
            params![
                branch.name,
                branch.description,
    ...
    /// A description edited locally (or predating synchronization tracking)
    /// must not be replaced by a differing remote value.
    pub fn branch_description_pending(&self, name: &str) -> Result<bool> {
        let conn = self.conn.lock().unwrap();
        conn.query_row(
            "SELECT description_pending != 0 FROM branches WHERE name = ?1",
            [name],
            |row| row.get(0),
        )
        .optional()
...
impl Repository for SqliteRepository {
    ...
    fn store_branch(&self, branch: &Branch) -> Result<()> {
        ...
        let generation = advance_description_generation(&tx)?;
        tx.execute(
            "INSERT OR IGNORE INTO branches (name, description, parent_branch, status, close_reason, created_at, description_generation) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
            params![
                branch.name,
                branch.description,
                branch.parent_branch,
    ...
    fn update_branch_description(&self, name: &str, description: &str) -> Result<()> {
        ...
        let generation = advance_description_generation(&tx)?;
        tx.execute(
            "UPDATE branches SET description = ?1, description_generation = ?3 WHERE name = ?2",
            params![description, name, generation],
        )
        .map_err(|e| OakError::Database(e.to_string()))?;
        tx.commit()
            .map_err(|error| OakError::Database(error.to_string()))
    ...
    fn edit_branch_description(&self, name: &str, description: &str) -> Result<()> {
        let mut conn = self.conn.lock().unwrap();
        let tx = conn
            .savepoint()
            .map_err(|error| OakError::Database(error.to_string()))?;
        let generation = advance_description_generation(&tx)?;
        tx.execute(
            "UPDATE branches SET description = ?1, description_pending = 1, description_generation = ?3 WHERE name = ?2",
            params![description, name, generation],
        )
        .map_err(|error| OakError::Database(error.to_string()))?;
        tx.commit()
    ...
    fn description_publication_generation(&self) -> Result<Option<i64>> {
        self.description_generation().map(Some)
    }

    fn acknowledge_branch_description(
        &self,
        name: &str,
        ...
        let tx = conn
            .savepoint()
            .map_err(|error| OakError::Database(error.to_string()))?;
        // Publication changes which remote responses are safe to reconcile,
        // even when the text itself did not change. Fence fetches that began
        // before this acknowledgment, including while the POST was in flight.
        let acknowledged_generation = advance_description_generation(&tx)?;
        tx.execute(
            "UPDATE branches SET description_pending = 0, description_generation = ?4 WHERE name = ?1 AND description IS ?2 AND description_generation <= ?3",
            params![name, description, generation, acknowledged_generation],
        )
        .map_err(|error| OakError::Database(error.to_string()))?;
        tx.commit()
            .map_err(|error| OakError::Database(error.to_string()))
    ...
    fn rename_branch(&self, old_name: &str, new_name: &str) -> Result<()> {
        ...
        let generation = advance_description_generation(&tx)?;
        tx.execute(
            "INSERT INTO branches (name, description, parent_branch, status, close_reason, created_at, description_pending, description_generation) \
             SELECT ?1, description, parent_branch, status, close_reason, created_at, description_pending, ?3 FROM branches WHERE name = ?2",
            params![new_name, old_name, generation],
        )
        .map_err(|e| OakError::Database(e.to_string()))?;
...
mod tests {
    ...
    #[test]
    #[allow(deprecated)]
    fn unfenced_description_upsert_preserves_text_but_reconciles_lifecycle() {
        let repo = SqliteRepository::in_memory().unwrap();
        let mut remote = Branch::new("feature".into(), Some("initial".into()), None);
        repo.upsert_branch_metadata(&remote).unwrap();
        assert_eq!(
        ...
        remote.description = Some("unfenced replacement".into());
        remote.status = BranchStatus::Closed;
        repo.upsert_branch_metadata(&remote).unwrap();
        let actual = repo.get_branch("feature").unwrap().unwrap();
        assert_eq!(actual.description.as_deref(), Some("initial"));
        assert_eq!(actual.status, BranchStatus::Closed);
    }

    #[test]
    fn legacy_description_requires_matching_baseline_before_remote_refresh() {
        let temp = tempfile::tempdir().unwrap();
        let path = temp.path().join("oak.db");
        {
            ...
            .unwrap();
            // Fixture of the previous schema: no local/remote synchronization evidence.
            repo.conn
                .lock()
                .unwrap()
                .execute_batch(
                    "ALTER TABLE branches DROP COLUMN description_pending;
                 DELETE FROM schema_migrations WHERE version = '0012_pending_branch_description';",
                )
                .unwrap();
        }
        let repo = SqliteRepository::open(&path).unwrap();
        assert!(repo.branch_description_pending("feature").unwrap());
        let mut remote = Branch::new("feature".into(), Some("remote changed".into()), None);
        repo.upsert_branch_metadata_at_generation(&remote, repo.description_generation().unwrap())
            .unwrap();
        assert_eq!(
            repo.get_branch("feature")
                .unwrap()
        ...
        remote.description = Some("legacy local".into());
        repo.upsert_branch_metadata_at_generation(&remote, repo.description_generation().unwrap())
            .unwrap();
        assert!(!repo.branch_description_pending("feature").unwrap());
        ...
        repo.upsert_branch_metadata_at_generation(&remote, repo.description_generation().unwrap())
            .unwrap();
        assert_eq!(
            repo.get_branch("feature").unwrap().unwrap().description,
            None
        );
    }

    #[test]
    fn pending_description_survives_rename_and_stale_publish_ack() {
        let repo = SqliteRepository::in_memory().unwrap();
        repo.store_branch(&Branch::new("feature".into(), None, None))
            .unwrap();
        repo.edit_branch_description("feature", "pending").unwrap();
        repo.rename_branch("feature", "renamed").unwrap();
        assert!(repo.branch_description_pending("renamed").unwrap());
        repo.edit_branch_description("renamed", "newer").unwrap();
    ...
    #[test]
    fn description_fetch_fence_covers_aba_new_rows_and_overlapping_refreshes() {
        let repo = SqliteRepository::in_memory().unwrap();
        let mut remote = Branch::new("feature".into(), Some("A".into()), None);
        repo.store_branch(&remote).unwrap();
        let before = repo.description_generation().unwrap();
        repo.edit_branch_description("feature", "B").unwrap();
        repo.edit_branch_description("feature", "A").unwrap();
        // Matching text from an older fetch cannot acknowledge an ABA edit.
        assert!(!repo
            .reconcile_pulled_branch_metadata(&remote, before)
            .unwrap());
        assert!(repo.branch_description_pending("feature").unwrap());
        repo.acknowledge_branch_description("feature", Some("A"), Some(before))
            .unwrap();
        assert!(
            repo.branch_description_pending("feature").unwrap(),
            "old successful POST cannot acknowledge an ABA edit"
        );
        repo.acknowledge_branch_description(
            "feature",
            Some("A"),
        ...
        .unwrap();
        remote.description = Some("stale remote".into());
        assert!(!repo
            .reconcile_pulled_branch_metadata(&remote, before)
            .unwrap());
        assert_eq!(
        ...
        let fresh = repo.description_generation().unwrap();
        assert!(repo
            .reconcile_pulled_branch_metadata(&remote, fresh)
            .unwrap());
        remote.description = Some("overlapping old response".into());
        assert!(!repo
            .upsert_branch_metadata_at_generation(&remote, fresh)
            .unwrap());
        assert_eq!(
            repo.get_branch("feature")
                .unwrap()
        ...
        let before_creation = repo.description_generation().unwrap();
        let mut new_branch = Branch::new("new".into(), Some("locally created".into()), None);
        repo.store_branch(&new_branch).unwrap();
        new_branch.description = Some("older remote response".into());
        assert!(!repo
            .upsert_branch_metadata_at_generation(&new_branch, before_creation)
            .unwrap());
        assert_eq!(
            repo.get_branch("new")
                .unwrap()
                ...
                .description
                .as_deref(),
            Some("locally created")
        );

        // A sibling edit does not unnecessarily block this branch's refresh.
        let before_sibling = repo.description_generation().unwrap();
        repo.edit_branch_description("new", "sibling update")
            .unwrap();
        assert!(repo
            .reconcile_pulled_branch_metadata(&remote, before_sibling)
            .unwrap());
    }
