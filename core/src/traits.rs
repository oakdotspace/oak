...
pub trait Repository: Send + Sync {
    ...
    fn get_blob_size(&self, hash: &Hash) -> Result<Option<u64>> {
        Ok(self.get_blob(hash)?.map(|blob| blob.size))
    }
    ...
    /// Record user-authored text separately from server/import metadata updates.
    fn edit_branch_description(&self, name: &str, description: &str) -> Result<()> {
        self.update_branch_description(name, description)
    }
    /// Capture before reading description text for publication. Backends without
    /// local synchronization tracking return no token and do not acknowledge.
    fn description_publication_generation(&self) -> Result<Option<i64>> {
        Ok(None)
    }
    /// Acknowledge exact text and generation, without clearing a newer/ABA edit.
    fn acknowledge_branch_description(
        ...
        let _ = (name, description, generation);
        Ok(())
    }
    ...
    fn copy_chunk_to_writer(
        ...
        let Some(bytes) = self.get_chunk(hash)? else {
            return Ok(None);
        };
    ...
    /// Build/store a manifest, create a commit for it, and advance the branch
    /// head plus legacy HEAD to that commit. Backends that can transactionally
    /// publish the whole transition should override this method.
    #[allow(clippy::too_many_arguments)]
    fn put_commit_and_advance_refs(
        ...
        let manifest_hash = self.put_manifest(entries)?;
        let commit_hash = self.put_commit(
            branch_name.clone(),
            parent_hash,
            merge_parent_hash,
            manifest_hash,
        ...
        self.set_branch_head(&branch_name, &commit_hash)?;
