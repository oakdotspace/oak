...
Remote review pins source and target names, heads, and source-parent metadata
from its single branch-list response. It may cache verified immutable objects,
but it never updates local branch refs, status, descriptions, or parents; a
missing remote head remains unavailable rather than falling back to a local one.

For a single `branch review --remote`, preparation can also acquire missing
content through the existing raw-file endpoint at exact pinned commit/path
identities. It verifies plaintext hashes before caching and before admitting
content prediction. Acquisition shares the preparation's 30-second/128-MiB
budget and allows at most 256 distinct missing blobs; already-cached blobs do
not consume that request cap. Required content covers the top-level **target
snapshot → branch snapshot** summary and actual merge-conflict triples; the
preview's changed files describe **target → predicted result**. Neither view
should be relabeled as a fork-based contribution diff. Unchanged, unread base
blobs do not require hydration.

Unavailable required merge content, including an unsupported raw route on
older/local servers, leaves prediction unconfirmed. Snapshot statistics remain
available only when their own M/F content independently verifies; otherwise
the structural summary marks content evidence unavailable. There is no
ordinary-fetch/full-history fallback. Local refs and worktree state remain
untouched; shared local/batch review and mutating merge completeness guards
remain unchanged. These acquisition bounds do not impose a file-count limit on
warm reviews or claim a new bound on all cached graph/content processing.
