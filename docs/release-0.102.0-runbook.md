...
## Known residual risks in the signing pipeline (accepted, not mitigated)

Investigating the ad-hoc-signing incident turned up further weaknesses in the
release workflows. They are recorded here so they are not rediscovered from
scratch. **All of them are deliberately accepted** for a two-person team on a
private mirror — they are not TODOs, and none of them blocks a release.

- **The signing keychain is visible to processes started by earlier steps.**
  Steps in a job share a machine and a user; GitHub reaps orphaned processes
  at job *finalization*, not between steps. Anything an earlier step left
  running (a build script, a proc-macro, a `brew` daemon, a `setsid`
  descendant) is still alive while the Developer ID key is unlocked and in the
  keychain search list, and can invoke `codesign` with it.
- **Payloads inherit credentials through the environment.** Secrets exported
  at job or step scope are in the environment of every process the step
  spawns, transitively. Nothing sandboxes the compile from them.
- **The producer supplies the digests.** SHA256SUMS and the minisign
  signatures are produced by the same job that produced the binaries, so they
  prove *transport* integrity (the bytes on oak.space are the bytes that were
  uploaded) and not *provenance* (that those bytes came from this source at
  this commit). A compromised build job signs whatever it likes.
- **`workflow_dispatch` can run any ref with production secrets.** The
  dispatcher chooses the branch or tag, so every `if: github.ref == …` guard
  in these workflows is deletable by exactly the person it is meant to stop.
  Protected GitHub Environments would fix this and were evaluated and
  declined: the settings-side administration is not worth it at this size.
- **The macOS app compiles and signs in one `xcodebuild`.** The app path has
  no compile/sign separation at all — the certificate is in the keychain while
  Xcode is building — so it carries the first two risks above by construction.

The incident itself — a silent fallback to ad-hoc signing — *is* fixed. Every
item above instead requires an attacker who can already run code in the
release workflow: a different, much larger threat model.
