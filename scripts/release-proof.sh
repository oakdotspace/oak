...
check_signing() {
  ...
  # Key-agreement proof: signing working is not enough — the signature must
  # verify against the RELEASE_PUBKEY baked into the CLI (the single source of
  # truth clients check downloads with; cli/src/commands/upgrade.rs). Extract
  # that constant from source and verify the smoke signature against it. A
  # mismatch means the seckey is the wrong keypair: every artifact it signs
  # would be REFUSED by `oak upgrade` and first-run `oak mount` installs.
  # (Verifying the signature IS the derive-and-compare: minisign -V -P
  # cryptographically requires the signing key's public half to equal the
  # baked constant, and unlike `minisign -R` it needs no second password
  # prompt and no reliance on the seckey file's comment fields.)
  local baked_pubkey
  baked_pubkey="$(sed -n 's/^pub(crate) const RELEASE_PUBKEY: &str = "\([A-Za-z0-9+/=]*\)";$/\1/p' cli/src/commands/upgrade.rs)"
  if [[ -z "$baked_pubkey" ]]; then
    echo "error: could not extract RELEASE_PUBKEY from cli/src/commands/upgrade.rs (declaration moved or reformatted?)" >&2
    exit 1
  fi
  if ! "$minisign_bin" -V -P "$baked_pubkey" -m "$smoke" -x "$sig" >/dev/null; then
    echo "error: key agreement FAILED — the smoke signature from MINISIGN_SECKEY does not verify against the RELEASE_PUBKEY baked into cli/src/commands/upgrade.rs (${baked_pubkey}). This seckey is not the release keypair; artifacts it signs would be refused by 'oak upgrade'." >&2
    exit 1
  fi
  echo "key agreement OK: smoke signature verifies against the baked RELEASE_PUBKEY"
}
