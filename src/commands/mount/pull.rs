...
use super::{build_committed_manifest, config_for_dest, token_for};
use crate::output;

/// Conflict markers built from repeated chars so this source file never holds a
/// full 7-character marker (which the marker scanner / CI check would trip on).
fn marker_lt() -> String {
    "<".repeat(7)
}
fn marker_gt() -> String {
...
fn has_markers(s: &str) -> bool {
    s.contains(&marker_lt()) || s.contains(&marker_gt()) || s.lines().any(|l| l == "=======")
}
...
async fn fresh_pull(cfg: &MountConfig, state_dir: &Path) -> Result<()> {
    ...
    if plan.text_conflicts.is_empty() && plan.structural_conflicts.is_empty() {
        ...
        let commit = finalize_sync(cfg, state_dir, &cache, &plan.merged, &parent_head)?;
...
fn continue_pull(cfg: &MountConfig, state_dir: &Path) -> Result<()> {
    ...
    let commit = finalize_sync(cfg, state_dir, &cache, &merged, &parent_head)?;
...
async fn plan_merge(
    ...
    let base_map: HashMap<&str, &Hash> = base
        .entries
        .iter()
                    ...
                    (Some(be), Some(pe)) => {
                        ...
                        if let (Some(bb), Some(pb)) = (branch_blob, parent_blob) {
                            ...
                            match (branch_text, parent_text) {
                                (Ok(bt), Ok(pt)) => {
                                    let conflicted =
                                        crate::commands::merge::create_conflict_content(
                                            &base_text,
                                            &bt,
                                            &pt,
                                            &cfg.virtual_branch,
                                    ...
                                    let blob = Blob::new(conflicted.clone().into_bytes());
                                    ...
                                    text_conflicts.push((path.to_string(), conflicted, be.mode));
                                }
                                _ => {
...
fn finalize_sync(
    ...
    let new_manifest = build_committed_manifest(merged, &overlay, &dirty_blobs);
