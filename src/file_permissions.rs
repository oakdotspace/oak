...
/// `oak status`) sees it: follow symlinks via `fs::metadata` and map the unix
/// `0o111` execute bits to `Executable`, everything else to `Regular`. This
/// mirrors the walk in `commands::commit` exactly so `oak reset` / `oak
/// restore` change-detection agrees with `oak status` about mode. Returns
