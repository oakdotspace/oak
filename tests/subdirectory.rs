...
#[test]
fn status_log_hash_diff_from_subdirectory() {
    let temp = TempDir::new().unwrap();
    init_repo(temp.path());
    ...
    oak_cli::commands::status::run(&nested).unwrap();
    oak_cli::commands::log::run(&nested, None, false).unwrap();
    oak_cli::commands::hash::run(&nested).unwrap();
    oak_cli::commands::diff::run(&nested).unwrap();
}
